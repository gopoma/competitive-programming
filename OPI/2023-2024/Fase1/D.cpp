...
void solve() {
    ...
    if(is_sorted(all(a))) {
        ...
        //? move all to ccc then return to aaa
        reverse(all(a));

        const int OFFSET = 12;
        vb aa(n + OFFSET + 1, false), bb(n + OFFSET + 1, false);
        stack<int> aaa, bbb, ccc;

        for(auto& x: a) aa[x] = true;
        for(auto& x: a) aaa.push(x);

        const int upper_bound = int(3e5 + OFFSET);

        vpi res;
        int need = 1;
        int ops = 0;
        while(sz(ccc) < n && ops <= upper_bound) {
            if(aa[need]) {
                chk(!aaa.empty());
                while(aaa.top() != need) {
                    res.eb(mp(1, 2));
        ...
        reverse(all(aaaa));
        assert(is_sorted(all(aaaa)));

        cout << min(ops, upper_bound) << n_l;
        for(int i = 0; i < min(ops, upper_bound); i++) {
            cout << res[i].first << " " << res[i].second << n_l;
        }
    }
