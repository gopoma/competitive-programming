...
tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{LLONG_MAX}; T cmb(T a, T b) { return min(a, b); }
	int n; V<T> seg;
...
void solve() {
    ...
    for(int i = 0; i < N; i++) {
        ...
        if(go < st.query(0, N - 1)) {
            ...
            while(left + 1 < right) {
                ...
                if(st.query(fixedLeft, middle) <= mn) {
                    right = middle;
                } else {
                    left = middle;
                }
            ...
            go = arr[idx].f + arr[idx].s;

            order.eb(idx);
            when.eb(arr[idx].f);

            safeErase(aux, arr[idx].f);
            st.upd(idx, LLONG_MAX);
        } else {
            chk(go >= st.query(0, N - 1));
            ...
            while(left + 1 < right) {
                ...
                if(st.query(fixedLeft, middle) <= go) {
                    right = middle;
                } else {
                    left = middle;
                }
    ...
    ll ans = 0;
    for(int i = 0; i < N; i++) ckmax(ans, abs(when[i] - arr[order[i]].f));
