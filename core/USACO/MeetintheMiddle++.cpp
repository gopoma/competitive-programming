...
void solve() {
    ...
    vl left; for(int i = 0; i < n / 2; i++) left.eb(t[i]);
    vl right; for(int i = n / 2; i < n; i++) right.eb(t[i]);

    auto work = [&](vl& arr) {
        vl res;

        const int length = sz(arr);
    ...
    vl go_left = work(left);   sor(go_left);
    vl go_right = work(right); sor(go_right);

    dbg(go_left);
    dbg(go_right);

    ll ans = 0;
    for(auto& val: go_left) {
        ll tar = x - val;

        ll cnt = upper_bound(all(go_right), tar) - lower_bound(all(go_right), tar);
        dbg(val, tar, cnt);
