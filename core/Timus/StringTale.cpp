...
void solve() {
    ...
    if(S == T) ps("0");
        ...
        for(ll take = 1; take <= N - 1; take++) {
            //? from S
            const int L = 0;
            const int R = L + take - 1;
            //? from T
            const int R2 = N - 1;
            const int L2 = R2 - take + 1;
            ...
            //? from T
            const int L4 = 0;
            const int R4 = L4 + (R3 - L3 + 1) - 1;

            dbg(L, R, L2, R2, L3, R3, L4, R4);
            dbg(S.substr(L, R - L + 1), S.substr(L3, R3 - L3 + 1));
            dbg(T.substr(L2, R2 - L2 + 1), T.substr(L4, R4 - L4 + 1));
            RAYA;

            if(
                (SH.hash(L, R) == TH.hash(L2, R2))
                && (SH.hash(L3, R3) == TH.hash(L4, R4))
            ) {
                ps((N - 1) - take + 1);
                return;
            }
