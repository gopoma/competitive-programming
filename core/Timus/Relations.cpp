...
ll brute(ll N) {
    ...
    //? 1 -> '='
    dbg("Brute");
    set<vector<set<int>>> S;
    for(int mask = 0; mask < (1 << (N - 1)); mask++) {
        //?dbg(bitset<6>(mask), (1 << (N - 1)));
        vi config;
        for(int i = 0; i < (N - 1); i++) {
            if(mask & (1 << i)) {
                config.eb(1);
        ...
        do {
            ...
            set<int> current; current.emplace(p[0]);
            for(int i = 1; i < N; i++) {
                if(config[i - 1] == 0) {
                    if(!current.empty()) {
                        group.eb(current);
...
ll get(ll N) {
    ...
    for(int mask = 0; mask < (1 << (N - 1)); mask++) {
        ...
        ll contrib = F(N);
        each(x, lengths) contrib = fdiv(contrib, F(x));
        ans += contrib;
    }
    return dp[N] = ans;
}
