...
void solve() {
    ...
    auto F = [](ll x) -> E {
        E ans = 1;
        for(ll act = 1; act <= x; act++) ans *= E(act);
        return ans;
    };

    auto C = [&](ll N, ll K) -> E {
        dbg(K);
        if(K > N || K < 0) return E(0);
        assert(K <= N);
        assert(K >= 0);
        E ans = F(N) / (F(K) * F(N - K));
        return ans;
    ...
    auto power = [&](ll bb, ll exponent) -> E {
        E ans = 1;
        rep(exponent) ans *= bb;
        return ans;
    };
    ...
    E num = 0;
    for(ll x = max(n + p - m, 0LL); x <= p - n; x++) {
        num += C(n, x) * V(p - 1LL, 2LL * x) * power(2LL, n - x) * V(m - p, n - x) * F(n - x) * C(p - 2LL * x - 1LL, n - x - 1LL);
    }
    E denom = V(m, 2LL * n);
    if(num == E(0LL)) {
        denom = E(1LL);
