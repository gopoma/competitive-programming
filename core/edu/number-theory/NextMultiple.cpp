...
bool ok(ll X, ll Y, ll Z) {
    assert(Y <= Z);
    return (max(fdiv(Z, X) - cdiv(Y, X) + 1LL, 0LL) != 0LL);
}

pair<bool, ll> work(ll left, ll right, ll need) {
    bool ok = false;
    ll res = -1LL;
    if(left % need == 0LL) {
        ok = true;
...
void solve() {
    ...
    bool found = false;
    ll xx, yy;
    for(ll x = a + 1LL; x <= c; x += 1LL) {
        ll need = (a * b) / gcd(a * b, x);
        // need a multiple of 'need'

        pair<bool, ll> it = work(b + 1LL, d, need);
        if(it.first) {
            found = true;
            xx = x;
            yy = it.second;
