...
struct node {
	...
	node(): val(make_pair(-BIG, -79)) {}

	node(pair<long long, int> _val) : val(_val) {}
...
void solve() {
    ...
    V<ll> pref = get_prefix_sums(X);

    V<ll> ranges(n - k + 1);
    for(int i = 0; i < n - k + 1; i++) {
        ranges[i] = query(pref, i, i + k - 1);
    }
    ...
    const int OFFSET = 12;
    tree.init(n - k + 1 + OFFSET);

    for(int i = 0; i < n - k + 1; i++) {
        tree.upd(i, node(make_pair(ranges[i], -i))); //! -i
    }

    V<tuple<ll, int, int>> optimal;
    for(int i = 0; i < n - k + 1; i++) {
        if(i + k + k - 1>= n) break;

        node best = tree.query(i + k, n - k);
        optimal.eb(make_tuple(ranges[i] + best.val.first, -i, best.val.second));
    }

    sort(rall(optimal));

    // for(auto& it: optimal) DBG3(get<0>(it), get<1>(it), get<2>(it));
    auto it = optimal[0];
    cout << (-get<1>(it) + 1) << " " << (-get<2>(it) + 1) << n_l;
}
