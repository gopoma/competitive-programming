...
inline namespace RecursiveLambdas {
...
//? use [&] instead of [] if you want to use closures
//? void y_comb_demo() {
...
void solve() {
    ...
    V<pair<long long, long long>> a(n);
    for(auto& e: a) {
        cin >> e.first >> e.second;
    }

    vector<long long> memo(W + 5, 0LL);
    vector<bool> vis(W + 5, false);

    // solve(x): maximum value reachable with 'x' capacity
    long long ans = y_combinator([&](auto solve, long long x) -> long long {
...
signed main() {
    // https://codeforces.com/group/OyefIGO3t4/contest/447266/problem/H
    // H. Knapsack Problem
