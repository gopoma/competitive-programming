...
void solve() {
    ...
    bool can_build = true;
    for(auto& it: cntT) can_build &= (it.s <= cntS[it.f]);

    if(!can_build) cout << "need tree\n";
    else {
        if(issubsequence(T, S)) cout << "automaton\n";
        else {
            bool just_array = true;
            for(auto& it: cntT) just_array &= (it.s == cntS[it.f]);
            for(auto& it: cntS) just_array &= (it.s == cntT[it.f]);

            if(just_array) cout << "array\n";
            else cout << "both\n";
        }
    }
}
