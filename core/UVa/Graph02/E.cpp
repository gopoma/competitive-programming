...
void dfs(int v, int p = -1) {
    ...
    auto IS_CUTPOINT = [&](int node) -> void {
        is[node] = true;
    };
...
vpi brute(int N, int m, vpi edges) {

    vpi xd;
    for(int u = 0; u < N; u++) {
        vpi tar_edges;
        for(auto& [xx, yy]: edges) if(xx != u && yy != u) {
            tar_edges.eb(xx, yy);
        }
        vector<vector<int>> adj(N);
        for(auto& [x, y]: tar_edges) {
            adj[x].eb(y);
            adj[y].eb(x);
        }
        vb vis(N);
    ...
    for(int i = 0; i < m; i++) {
        response.eb(xd[i].s, -xd[i].f);
    }
    return response;
}

vpi slv(int N, int m, vpi edges) {
    n = N;
    ...
    dbg(DFSTreeEdges);
    cout << "DFSTreeEdges\n";
    for(auto& [xx, yy]: DFSTreeEdges) {
        cout << xx << " " << yy << "\n";
    }
    cout << "/DFSTreeEdges\n";
        
    vector<vector<int>> adjDFSTree(n);
    for(auto& [x, y]: DFSTreeEdges) {
        adjDFSTree[x].eb(y);
        adjDFSTree[y].eb(x);
    }
    vi pV(n);
    for(int u = 0; u < n; u++) {
        if(is[u]) pV[u] = sz(adjDFSTree[u]);
        else pV[u] = 1;
    }
    for(int u = 0; u < n; u++) dbg(u, is[u], pV[u]);
...
void solve() {
    //? <>
    while(true) {
        int N, m; cin >> N >> m; if(N == 0 && m == 0) return;
        vpi edges;
        while(true) {
        ...
        dbg(N, m);
        dbg(edges);
        vpi response = brute(N, m, edges);
        for(auto& [x, y]: response) cout << x << " " << y << "\n";
        cout << "\n";
        RAYA;
    }
}
...
signed main() {
    ...
    while(1) {
        ...
        dbg(edges);
        cout << "Edges\n";
        for(auto& [xx, yy]: edges) cout << xx << " " << yy << "\n";
        cout << "/Edges\n";
        dbg("Brute");
        vpi ans = brute(N, m, edges);
        dbg("/Brute");
        dbg("Greedy");
        vpi greedy = slv(N, m, edges);
        dbg("/Greedy");
        dbg(ans);
        dbg(greedy);
        chk(ans == greedy);
