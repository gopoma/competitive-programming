...
void solve() {
    ...
    //? <>
    str S; cin >> S;
    str N_str; each(c, S) if(isdigit(c)) N_str.pb(c);
    int N = stoi(N_str);
    int count = 0; each(c, S) count += (c == '!');
    dbg(N, count);

    i128 ans = 1;
    map<int, int> factors;
    for(int x = N; x >= 2; x -= count) {
        int current = x;
        while(current != 1) {
            factors[st.sp[current]]++;
            current /= st.sp[current];
        }
    }
    for(auto& [a, p]: factors) {
        ans *= (p + 1);
        if(ans > MAXN) {
            cout << "Case " << caso << ": Infinity\n";
            return;
        }
    }
    cout << "Case " << caso << ": " << ll(ans) << "\n";
}
