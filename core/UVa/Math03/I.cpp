...
void solve() {
    //? <>
    int prod_without_2_and_5 = 1;
    vi prod_2(MAXN + 5);
    prod_2[0] = 1;
    for(int i = 1; i < MAXN; i++) {
        prod_2[i] = 2 * prod_2[i - 1];
        prod_2[i] %= 10;
    }
    vi hist(MAXN + 5);
    auto upd = [&](int a, int p) -> void {
        if(hist[a] < p) {
            if(a == 2 || a == 5) {
                ckmax(hist[a], p);
            } else {
                chk(a != 0);
                rep(p - hist[a]) {
                    prod_without_2_and_5 *= a;
                    prod_without_2_and_5 %= 10LL;
                }
                hist[a] = p;
            }
        }
    ...
    for(int x = 2; x < MAXN; x++) {
        ...
        int current = x;
        while(current != 1) {
            factors.eb(st.sp[current]);
            current /= st.sp[current];
        }
        int a = factors.ft;
        int p = 1;
        ...
        upd(a, p);
        ans[x] = prod_without_2_and_5 * prod_2[hist[2] - hist[5]];
        ans[x] %= 10;
    }
    while(1) {
        ll x; cin >> x; if(x == 0) return;
        cout << ans[x] << "\n";
    }
}
