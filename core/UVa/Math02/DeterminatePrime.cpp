...
void solve() {
    ...
    {
        vl primes;
        for(ll x = 1; x <= MAXN; x++) if(st.is_prime[x]) primes.eb(x);

        const int N = sz(primes);
        for(int window_size = 5; window_size >= 3; window_size--) {
            for(int i = 0; i < N; i++) {
                const int L = i;
                const int R = i + window_size - 1;
                if(R >= N) break;
                vl window;
                for(int j = L; j <= R; j++) {
                    window.eb(primes[j]);
                ...
                set<ll> diff;
                const int M = sz(window);
                for(int j = 1; j < M; j++) {
                    const ll d = window[j] - window[j - 1];
                    diff.emplace(d);
                }
                if(sz(diff) != 1) continue;
                else {
                    bool canExtend = (primes[L] - primes[L - 1] == (*diff.begin())) || (primes[R + 1] - primes[R] == (*diff.begin()));
        ...
        dbg(sz(sequences));
        while(true) {
            ll x, y; cin >> x >> y; if(x == 0 && y == 0) return;
            ll L = min(x, y);
            ll R = max(x, y);
