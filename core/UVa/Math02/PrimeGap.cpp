...
void solve() {
    vi primes;
    for(int x = 2; x <= MAXN; x++) if(st.is_prime[x]) primes.eb(x);
    //? <>
    ll n;
    while(true) {
        RAYA;
        cin >> n; if(n == 0) return;
        if(n == 1 || st.is_prime[n]) {
            cout << "0\n";
            continue;
        }
        dbg(n);
        ll L_idx = lower_bound(all(primes), n) - primes.begin();
        L_idx--;
        ll R_idx = upper_bound(all(primes), n) - primes.begin();
        ll k = primes[R_idx] - primes[L_idx];
        cout << k << "\n";
    }
}
