...
void solve() {
    ...
    for(int _ = 0; _ < q; _++) {
        ...
        if(w1 < words.front() && w2 < words.front()) {
            cout << "0\n";
            continue;
        }
        if(w1 > words.back() && w2 > words.back()) {
            cout << "0\n";
            continue;
        }

        if(w1 < words.front()) w1 = words.front();
        if(w2 > words.back()) w2 = words.back();

        auto left = lower_bound(all(words), w1);
        auto right = upper_bound(all(words), w2);

        if(left == right) {
            if(w1 == w2 && there[w1]) cout << "1\n";
            else cout << "0\n";
        } else {
            int ans = (right - words.begin()) - (left - words.begin());
            cout << ans << n_l;
        }
    }
    RAYA;
