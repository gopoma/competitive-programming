...
void solve() {
    ...
    if(N == 1) {
        ...
        auto [x1, y1] = mp(0LL * K - L, 0LL * K - L);
        auto [x2, y2] = mp(0LL * K + L, 0LL * K + L);

        auto [x3, y3] = mp(1LL * K - L, 1LL * K - L);
        auto [x4, y4] = mp(1LL * K + L, 1LL * K + L);

        //? [x1, x2] [x3, x4]
        //? [y1, y2] [y3, y4]
        bigint between = 0;
        if(x3 < x2 && y3 < y2) {
            between = bigint(x2 - x3) * bigint(y2 - y3);
        }
        bigint ans = bigint(sqr(2LL * L));
        bigint use = ans - between;
        ans += bigint(N - 1LL) * use;
        dbg(ans);
        cout << ans << "\n"; 
    }
}
