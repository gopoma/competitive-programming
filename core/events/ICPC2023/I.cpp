...
void solve() {
    ...
    auto count_inversions = [&](string& SS) {

    };

    auto gen = [&](string base, int times) {
        string res = "";
    ...
    auto brute = [&]() {
        for(int x = 1; x <= N; x++) {
            ...
            assert(is_sorted(all(SS)));
            DBG3(SS_tmp, inversions, count_inversions(SS_tmp));
            // assert(inversions == count_inversions(SS_tmp));
            // DBG4(x, SS_tmp, inversions, count_inversions(SS_tmp));
            RAYA;
        }
    };
    brute();
