...
void solve() {
    int H, W, M; cin >> H >> W >> M;
    vpi targets(M);
    for(auto& [h, w]: targets) cin >> h >> w;

    vvi groups(H + 1);
    vi hist(W + 1);
    for(auto& [h, w]: targets) {
        groups[h].eb(w);
        hist[w]++;
    }
    ...
    auto remove = [&](int w) -> void {
        safeErase(ms, hist[w]);
        hist[w]--;
        if(hist[w] > 0) ms.emplace(hist[w]);
    };

    auto add = [&](int w) -> void {
        if(hist[w] > 0) safeErase(ms, hist[w]);
        hist[w]++;
        ms.emplace(hist[w]);
    };
    ...
    for(int h = 1; h <= H; h++) {
        ...
        int local_response = sz(groups[h]) + get();
