...
void solve() {
    ...
    vi b(k); for(auto& x: b) cin >> x;

    map<int, int> where;
    for(int i = 0; i < n; i++) where[a[i]] = i;

    set<int> should;
    for(auto& x: b) should.emplace(x);
    ...
    for(int i = 0; i < k; i++) {
        ...
        safeErase(should, b[i]);

        int neighbors = 0;
        if(check(where[b[i]] - 1)) neighbors++;
        if(check(where[b[i]] + 1)) neighbors++;

        int free = 0;
        if(check(where[b[i]] - 1) && !should.count(a[where[b[i]] - 1])) free++;
        if(check(where[b[i]] + 1) && !should.count(a[where[b[i]] + 1])) free++;
