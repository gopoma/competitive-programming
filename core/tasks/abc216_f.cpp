...
bool vis[5000 + 1][5000 + 1][2];
mi  memo[5000 + 1][5000 + 1][2];
void solve() {
    int n; cin >> n;
    ...
    dbg(a);
    dbg(b);
    vpi c(n); for(int i = 0; i < n; i++) c[i] = mp(a[i], b[i]);
    sort(rall(c));
    dbg(c);
    ...
    for(int i = 0; i <= 5000; i++)
        for(int j = 0; j <= 5000; j++)
            for(int k = 0; k < 2; k++)
                vis[i][j][k] = false;
    auto dp = [&](auto&& self, int i, int S, bool taken) -> mi {
        if(i == n) return mi(int(taken));
        if(vis[i][S][taken]) return memo[i][S][taken];
        vis[i][S][taken] = true;

        mi ans = self(self, i + 1, S, taken);
        auto [A, B] = c[i];

        if(!taken) {
            if(A >= B) ans += self(self, i + 1, A - B, true);
        } else {
            if(S - B >= 0) ans += self(self, i + 1, S - B, true);
        }
        return memo[i][S][taken] = ans;
    };
    mi ans = dp(dp, 0, n, false);
    cout << ans << "\n";
}
