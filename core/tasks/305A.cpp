...
void solve() {
    ...
    bool used3 = false;
    for(auto& x: a) {
        if(!used1 && sz(to_string(x)) == 1 && x != 0) {
            goods1.eb(x);
            used1 = true;
        }
        if(!used3 && sz(to_string(x)) == 2 && (x % 10 == 0)) {
            goods1.eb(x);
            used3 = true;
        }

        if(!used2 && sz(to_string(x)) == 2) {
            goods2.eb(x);
            used2 = true;
        }
    }

    cout << max(sz(goods1), sz(goods2)) << n_l;
    if(sz(goods1) > sz(goods2)) {
        for(auto& x: goods1) cout << x << " ";
    } else {
        for(auto& x: goods2) cout << x << " ";
    }
    cout << n_l;
