...
void solve() {
    ...
    vi hist(n + 1); for(auto& x: a) hist[x]++;
    vector<vector<int>> divs(n + 1);

    vl dp(n + 1); // amount of numbers divisible by i
    for(int i = 1; i <= n; i++) {
        for(int j = i; j <= n; j += i) {
            dp[i] += hist[j];
    ...
    dbg("amount of numbers divisible by i");
    for(int i = 1; i <= n; i++) {
        dbg(i, dp[i]);
    }
    ...
    dbg("Next 2: amount of pairs (x, y) such that (x < y) gcd(a[x], a[y]) is i");
    for(int i = 1; i <= n; i++) {
        dbg(i, dp[i]);
    }

    ll response = ((n - 1LL) * n) / 2LL;
    for(int i = 1; i <= n; i++) {
        bool there = false;
        for(auto& d: divs[i]) {
            if(hist[d] > 0) {
