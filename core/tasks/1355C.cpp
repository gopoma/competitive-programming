...
ll slv(ll A, ll B, ll C, ll D) {
    ...
    for(ll z = C; z <= D; z++) {
        ...
        if(B + B > z) {
            ...
            ll left = A - 1; // always bad
            ll right = B; // always good
            while(left + 1 < right) {
                ll middle = fdiv(left + right, 2);
                if(middle + B > z) right = middle;
                else left = middle;
            }
            ...
            final_x_right = right - 1;
        }

        if(A <= final_x_right && final_x_right + C > z) {
            ll x_left = -1;
            {
                ll left = A - 1; // always bad
                ll right = final_x_right; // always good
                while(left + 1 < right) {
                    ll middle = fdiv(left + right, 2);
                    if(middle + C > z) right = middle;
                    else left = middle;
                }
                x_left = right;
            }
            ll y_left = -1;
            {
                ll left = B - 1; // always bad
                ll right = C; // always good
                while(left + 1 < right) {
                    ll middle = fdiv(left + right, 2);
                    if(middle + final_x_right > z) right = middle;
                    else left = middle;
                }
                y_left = right;
            }
            //* dbg(x_left, final_x_right);
            //* dbg(y_left);

            ll sz_x = final_x_right - x_left + 1;
            ll sz_y = C - y_left + 1;
            //* dbg("regular", sz_x, sz_y);

            if(sz_x < sz_y) swap(sz_x, sz_y);

            ll xx = fdiv(sz_x * (sz_x + 1LL), 2LL);
            ll yy = fdiv((sz_x - sz_y) * (sz_x - sz_y + 1LL), 2LL);

            contrib2 = xx - yy;

        }

        //* dbg(z, final_x_right, contrib, contrib2);
...
int main() {
    ...
    while(0) {
        ...
        dbg(A, B, C, D);
        dbg("Brute");
        ll ans = brute(A, B, C, D);
        dbg("/Brute");
        dbg("Greedy");
        ll greedy = slv(A, B, C, D);
        dbg("/Greedy");
        dbg(ans, greedy);
        chk(ans == greedy);
    }
