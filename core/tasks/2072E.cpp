...
vi forwards(int(1e6) + 5);
set<int> a;
void precompute() {
    for(ll x = 2; fdiv(x * (x - 1LL), 2LL) <= ll(1e5); x++) {
        ll current = fdiv(x * (x - 1LL), 2LL);
        a.emplace(int(current));
        forwards[current] = int(x);
    }
    dbg(a);
    dbg(sz(a));
}
...
void solve() {
    ...
    int yy = -1000000;
    while(cur >= 0) {
        auto it = a.upper_bound(cur); it--;

        auto d = *it;
        dbg(cur, d);
        cur -= d;

        int should = forwards[d];
        dbg(should);
        tot_points += should;
        rep(should) {
            response.eb(xx, yy);
