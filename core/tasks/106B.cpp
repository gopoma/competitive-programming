...
void solve() {
    def(int, n);
    vector<tuple<int, int, int, int>> go(n); re(go);

    dbg(go);
    vector<tuple<int, int, int, int>> consider;
    for(int i = 0; i < n; i++) {
        bool ok = true;
        for(int j = 0; j < n; j++) {
            if(i == j) continue;
            ok &= !(
                (get<0>(go[i]) < get<0>(go[j]))
                && (get<1>(go[i]) < get<1>(go[j]))
                && (get<2>(go[i]) < get<2>(go[j]))
            );
        }
        if(ok) consider.eb(go[i]);
    }
    ...
    int ans = -1;
    for(int i = 0; i < n; i++) if(get<3>(go[i]) == cost) { ans = i; break; }
    assert(ans != -1);
