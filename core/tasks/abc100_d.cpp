...
void solve() {
    ll n, m; cin >> n >> m;
    V<Info> info(n);
    for(auto& [x, y, z]: info) cin >> x >> y >> z;

    auto get = [&](ll x, bool flip) -> ll {
        if(flip) return -x;
        return +x;
    };
    ...
    for(int mask = 0; mask < (1 << 3); mask++) {
        ...
        auto dp = [&](auto&& self, int i, int taken) -> ll {
            if(i == n) {
                if(taken == m) return 0;
                else return -BIG;
            }

            if(vis[i][taken]) return memo[i][taken];
            vis[i][taken] = true;

            ll ans = -BIG;

            //* skip
            ckmax(ans, self(self, i + 1, taken));

            //* take
            auto [x, y, z] = info[i];
            ckmax(ans, self(self, i + 1, taken + 1) + get(x, flip[0]) + get(y, flip[1]) + get(z, flip[2]));

            return memo[i][taken] = ans;
        };
        ll local_response = dp(dp, 0, 0);
