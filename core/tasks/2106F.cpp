...
void solve() {
    ...
    for(ll i = 0; i < n; i++) {
        if(S[i] == '0') {
            ll tot_up = i;
            ll tot_down = n - (tot_up + 1);
            dbg(i, tot_up, tot_down);
    ...
    for(int i = 0; i < n; i++) dbg(i, S[i], dp_up[i], dp_down[i]);

    ll MX_UP = *max_element(all(dp_up));
    ll MX_DOWN = *max_element(all(dp_down));
    ckmax(res, max(MX_UP, MX_DOWN));
    cout << res << "\n";
}
