...
void solve() {
    ...
    for(int i = 0; i < N; i++) {
        if(A[i] == 0) {
            ...
            //? A[i] = u * (p ** 2)
            map<ul, int> factors = factor(ul(A[i]));

            ll u = 1;
            ll p = 1;
            for(auto& [ff, cnt]: factors) {
                ll tmp = fdiv(ll(cnt), 2LL);
                ll need = 2LL * tmp;
                p *= binpow(ff, need);
    ...
    ll contrib1 = zeros * (N - zeros);
    ll contrib2 = fdiv(zeros * (zeros - 1LL), 2LL);
    ll ans = contrib1 + contrib2;
    dbg(ans);

    for(auto& [u, cnt]: uu) {
        dbg(u, cnt);
        ans += fdiv(cnt * (cnt - 1LL), 2LL);
    }
