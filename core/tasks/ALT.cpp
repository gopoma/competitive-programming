...
void solve() {
    ...
    for(auto& x: B) {
        if(x % 2 == 0) evens.emplace(x);
        else odds.emplace(x);
    }

    sor(B); dbg(B);
    dbg(evens, odds);
    if((sz(evens) % 2 != 0) || (sz(odds) % 2 != 0)) cout << "-1\n";
    else {
        V<ll> left, right;

        auto work = [&](multiset<ll>& S) {
            chk(sz(S) % 2 == 0);
            ...
            while(!S.empty()) {
                ...
                auto partial = calc(mx, mn);
                left.eb(partial.first);
                right.eb(partial.second);
