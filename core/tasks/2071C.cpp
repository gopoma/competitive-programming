...
void solve() {
    int n, from, to; cin >> n >> from >> to; from--; to--;
    vpi edges(n - 1);
    for(auto& [u, v]: edges) {
        cin >> u >> v;
        u--; v--;
...
//*        dbg(u, adj[u]);
    ...
    dbg(N, path);
    for(int i = 0; i < N; i++) {
        if(0 <= i - 1) banned.emplace(path[i - 1]);
        if(i + 1 < N) banned.emplace(path[i + 1]);
        work(path[i]);
    }
