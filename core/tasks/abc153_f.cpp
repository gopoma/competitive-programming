...
void solve() {
    ...
    const ll BOMB_ENTRY = 0;
    const ll MONSTER = 1;
    const ll BOMB_END = 2;

    using Event = tuple<ll, ll, ll>;
    multiset<Event> events; // position - type - value
    for(auto& [X, H]: monsters) events.emplace(X, MONSTER, H);

    ll cnt = 0;
    ll bombs = 0;
    while(!events.empty()) {
        auto it = *events.begin(); safeErase(events, it);
        auto [position, type, par] = it;
        ...
        if(type == BOMB_ENTRY) {
            ...
            ll H = par;
            if(cnt * A >= H) continue;
            ll adi = cdiv(H - cnt * A, A);
            bombs += adi;
            events.emplace(position, BOMB_ENTRY, adi);
            events.emplace(position + 2LL * D, BOMB_END,   adi);
        } else {
            assert(type == BOMB_END);
            ll value = par;
            cnt -= value;
        }
    }
