...
void solve() {
    ...
    const int R = int(1e9);
    vi digits;
    each(c, ts(R)) digits.eb(int(c - '0'));
    const int NONE = 10;
    dbg(digits, sz(digits));
    ll memo[15][15][5][90];
    memset(memo, -1, sizeof(memo));
    function<ll(int, int, bool, int)> dp = [&](int idx, int last, bool reached_lower, int sum) -> ll {
        if(sum > S) return 0;
        if(idx == sz(digits)) {
        ...
        ll ans = 0;
        if(last == NONE) {
            ans += dp(idx + 1, NONE, false, 0);
            if(idx == 0) {
                for(int d = 1; d <= digits[idx]; d++) {
                    ans += dp(idx + 1, d, d < digits[idx], sum + d);
                }
            } else {
                for(int d = 1; d <= 9; d++) {
