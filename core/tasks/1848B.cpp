...
void solve() {
    ...
    for(auto& it: there) where[it.first].eb(n);

    dbg(where);
    dbg(there);

    map<int, multiset<int>> diffs;
    for(auto& it: where) {
        for(int i = 1; i < sz(it.second); i++) {
            diffs[it.first].ins(it.second[i] - it.second[i - 1]);
        }
    }

    for(auto& it: diffs) dbg(it.first, it.second);
    for(auto& it: diffs) {
        chk(sz(it.second) >= 1);
        int largest = *it.second.rbegin();
        erase(it.second, largest);
        it.second.emplace(int(fdiv(ll(largest), 2LL)));
        it.second.emplace(int(cdiv(ll(largest), 2LL)));
    }
    RAYA;
    for(auto& it: diffs) dbg(it.first, it.second);

    int ans = INT_MAX;
    for(auto& it: diffs) ckmin(ans, *it.second.rbegin());
    cout << (--ans) << n_l;
