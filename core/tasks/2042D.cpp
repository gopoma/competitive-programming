...
using Segment = tuple<ll, ll, ll>;
void solve() {
    auto work = [&](vpl _segments) -> vl {
        const ll n = sz(_segments);

        V<Segment> segments(n);
        for(int id = 0; id < n; id++) {
            auto [l, r] = _segments[id];
            segments[id] = make_tuple(l, r, id);
        }
        sort(all(segments), [&](Segment& A, Segment& B) {
            auto [l, r, id] = A;
            auto [l2, r2, id2] = B;
            if(l != l2) return l < l2;
            if(r != r2) return r > r2;
        ...
        for(auto& [l, r, id]: segments) dbg(l, r, id);

        map<pl, ll> memo;
        multiset<ll> ms;
        for(int i = 0; i < n; i++) {
            auto [l, r, id] = segments[i];
            // already lj < li for all j < i
    ...
    vl right = work(segments);
    dbg(right);

    {
        ll mx = -BIG;
        for(auto& [l, r]: segments) ckmax(mx, r);
        mx += 5;
        vpl new_segments;
        for(auto& [l, r]: segments) {
            ll new_l = mx + 1LL - l;
            ll new_r = mx + 1LL - r;
            swap(new_l, new_r);
            new_segments.eb(new_l, new_r);
        }
        swap(segments, new_segments);
    }

    vl left = work(segments);
    dbg(left);

    vl response(n);
    for(int i = 0; i < n; i++) response[i] = right[i] + left[i];
    dbg(response);
