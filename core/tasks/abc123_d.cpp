...
void solve() {
    ...
    vl A(X); for(auto& x: A) cin >> x;
    vl B(Y); for(auto& x: B) cin >> x;
    vl C(Z); for(auto& x: C) cin >> x;
    sor(A);
    sor(B);
    sor(C);

    multiset<pl> ms; //* current sum - current id in C
    for(int i = 0; i < X; i++) {
        for(int j = 0; j < Y; j++) {
            ms.emplace(A[i] + B[j] + C[Z - 1], Z - 1);
        }
    }
    ...
    rep(K) {
        auto it = *ms.rbegin();
        auto [sum, i] = it; safeErase(ms, it);
        response.eb(sum);
        i--;
        if(i >= 0)
            ms.emplace(sum - C[i + 1] + C[i], i);
    }
