...
void solve() {
    ...
    auto check = [&]() -> pair<bool, pl> {
        ...
        rep(sz(C)) {
            {
                auto it = ms.lower_bound(mp(w, 0));
                if(it != ms.end()) {
                    auto [cw, ch] = *it;
                    if(cw == w) {
                        safeErase(ms, mp(cw, ch));
                        safeErase(ms2, mp(ch, cw));
                        h -= ch;
                        continue;
                    }
                }
            ...
            {
                auto it = ms2.lower_bound(mp(h, 0));
                if(it != ms2.end()) {
                    auto [ch, cw] = *it;
                    if(ch == h) {
                        safeErase(ms2, mp(ch, cw));
...

    {
        ...
        auto [ok2, it2] = check();
        swap(it2.f, it2.s);
        if(ok2) response.eb(it2.f, it2.s);
    }

    remDup(response);
    cout << sz(response) << "\n";
    for(auto& [a, b]: response) cout << a << " " << b << "\n";
}
