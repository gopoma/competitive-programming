...
ll slv(ll N) {
    ...
    S.emplace(1);
    squares.emplace(1);
    for(ll base = 2; base <= min(ll(1e6+10), N); base++) {
        ll act = base * base;
        for(ll exponent = 3; exponent <= 70; exponent++) {
            if((i128(act) * i128(base)) > i128(N)) break;
            act *= base;
            S.emplace(act);
            if(isPerfectSquare(act)) squares.emplace(act);
        }
    }
    //?dbg(S);
    //?dbg(sz(S));
    ...
    E num;
    {
        E left = 1; //? always good
        E right = ll(1e10); //? always bad
        while((left + E(1)) < right) {
            E middle = (left + right) / E(2);
            if(middle * middle <= E(N)) left = middle;
            else right = middle;
        }
        num = left;
    }
    dbg(num);
    ll ans = sz(S) + (stoll(ts(num)) - sz(squares));
    return ans;
} //? <>
