...
vl slv(int n, vl a, int q, vl queries) {
    vl pref = a;
    for(int i = 1; i < n; i++) {
        pref[i] += pref[i - 1];
    }
    ...
    vpl info;
    for(ll i = 0; i < n; i++) {
        ll sumLeft = query(0, i);
        ll sumRight = query(i + 1, n - 1);
        ll adi = cdiv(max(0LL, sumRight - (should[i] * (i + 1LL) - sumLeft)), (i + 1LL));
        dbg(i, sumLeft, sumRight, should[i], should[i] * (i + 1), adi);
        should[i] += adi;
        info.eb(should[i], i + 1);
    }
    sor(info);
    dbg(info);
    ...
    for(auto& t: queries) {
        ...
        while(left + 1 < right) {
            ...
            if(values[middle] <= t) left = middle;
            else right = middle;
        }
        if(left == -1) {
            response.eb(-1);
