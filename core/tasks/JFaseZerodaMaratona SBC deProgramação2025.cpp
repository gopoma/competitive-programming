...
void solve() {
    ...
    AngelBeats seg(b); // TODO [l, r[
    vl response(n);
    for(int i = 0; i < n; i++) {
        seg.range_add(i + 1, sz(b), -a[i]);

        int left = i; //? always bad
        int right = sz(b) - 1; //? always good
        while(left + 1 < right) {
            int middle = (left + right) >> 1LL;
            if(seg.range_min(i + 1, middle + 1) < 0) right = middle;
            else left = middle;
        }
        if(right >= n) right -= n;
        response[i] = right + 1;

        seg.range_add(i + 1, sz(b), +k + a[i]);
    }
