...
int slv(int n, vi a, vi b) {
    ...
    int mn_k = b.bk + 1;
    multiset<int> aa, bb;
    for(auto& x: a) aa.emplace(x);
    for(auto& x: b) bb.emplace(x);
    for(int i = 0; i < n; i++) {
        if(a[i] < mn_k) {
            safeErase(aa, a[i]);
    ...
    vi new_a;
    vi new_b;
    for(auto& x: aa) new_a.eb(x);
    for(auto& x: bb) new_b.eb(x);
    swap(a, new_a);
    swap(b, new_b);
    ...
    for(int i = 0; i < M; i++) {
        ...
        //* mx = x * k + B[i]
        //* mx - B[i] = x * k
        //* k should be at least mn_k

        if(vis[mx - B[i]]) continue;
        vis[mx - B[i]] = true;

        for(auto& k: divs[mx - B[i]]) {
            if(k >= mn_k) {
                candidates.eb(k);
            }
        }
    ...
    hash_map<int, bool> already;
    vi magic;
    rep(1) {
        int j = rng_int(0, sz(A) - 1);
        if(!already[j]) continue;
        already[j] = true;
        magic.eb(A[j]);
    }

    hash_map<int, int> hist_B;
    for(auto& x: B) hist_B[x]++;
...
int main() {
    ...
    while(0) {
        RAYA;
        int n = rng_int(int(1e4), int(1e4));
        int k = rng_int(int(9e5), int(1e6) + 5);
        vi a(n); for(auto& x: a) x = rng_int(int(8e5), int(1e6));
        vi b(n); for(int i = 0; i < n; i++) b[i] = a[i] % k;
