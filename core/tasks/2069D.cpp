...
int brute(str S) {
    ...
    for(int l = 0; l < n; l++) {
        for(int r = l; r < n; r++) {
            str partial = S.substr(l, r - l + 1);
            sor(partial);

            str left = S.substr(0, (l - 1) - 0 + 1);
            str right = S.substr(r + 1, (n - 1) - (r + 1) + 1);
            do {
                str cur = left + partial + right;
                if(isPalindrome(cur)) ckmin(ans, sz(partial));
            } while(next_permutation(all(partial)));
        }
    }
    return ans;
}
...
int slv(str S) {
    ...
    auto work = [&](str S) -> int {
        ...
        while(pleft < pright) {
            if(S[pleft] != S[pright]) {
                const int tot = pright - pleft + 1;
                assert(tot % 2 == 0);

                const int mid_tot = tot / 2;

                auto check = [&](int middle) -> bool {
                    const int cur_size = middle - pleft + 1;

                    vi hist(26);
                    for(int i = pleft; i <= middle; i++)
                        hist[S[i] - 'a']++;

                    if(cur_size >= mid_tot) {
                        int r = pright;
                        while(r > middle) {
                            if(hist[S[r] - 'a'] == 0) return false;
                            hist[S[r] - 'a']--;
                            r--;
                        }
                        return true;
                ...
                int left = pleft; // always bad
                int right = pright; // always good
                while(left + 1 < right) {
                    int middle = (left + right) >> 1;
                    if(check(middle)) right = middle;
                    else left = middle;
                }
                int response = right - pleft + 1;
                return response;
            }
            pleft++;
            pright--;
...
int main() {
    ...
    while(0) {
        ...
        for(int i = 0; i < 3; i++) {
            ...
            rep(hist[i]) SS.pb('a' + i);
        }
        shuf(SS);
        str S; for(auto& c: SS) S.pb(c);
