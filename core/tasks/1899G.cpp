...
void solve() {
    ...
    vector children(n, set<int>());
    for(int i = 0; i < n; i++) {
        children[i].emplace(where[i]);
    }
    ...
    auto dfs = [&](auto&& self, int src, int par) -> void {
        for(auto& nxt: adj[src]) {
            ...
            if(sz(children[src]) < sz(children[nxt])) {
                swap(children[src], children[nxt]);
            }
            for(auto& node: children[nxt]) {
                children[src].emplace(node);
            }
            //* children[nxt].clear();
        }

        for(auto& [l, r, i]: queries[src]) {
            auto it = children[src].lower_bound(l);
            if(it != children[src].end() && ((*it) <= r)) {
                response[i] = true;
            }
        }
    }; dfs(dfs, 0, -1);
