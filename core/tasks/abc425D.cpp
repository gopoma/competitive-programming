...
void solve() {
    ...
    auto check_white_cell = [&](int i, int j) -> bool {
        assert(S[i][j] == '.');
        int cnt = 0;
        for(int k = 0; k < 4; k++) {
            const int ni = i + dx[k];
            ...
            if(check_bounds(ni, nj) && (S[ni][nj] == '#')) cnt++;
        }
        return (cnt == 1);
    };
    ...
    while(!q.empty()) {
        ...
        for(auto& [i, j]: q) {
            ...
            for(int k = 0; k < 4; k++) {
                ...
                if(check_bounds(ni, nj) && !vis[ni][nj] && S[ni][nj] == '.' && check_white_cell(ni, nj)) {
                    vis[ni][nj] = true;
                    nxt_q.emplace_back(ni, nj);
                }
            }
        }
        for(auto& [i, j]: nxt_q) {
