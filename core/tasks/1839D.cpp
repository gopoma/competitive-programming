...
int dp[500 + 5][500 + 5][2];
int ndp[500 + 5][500 + 5][2];
void slv(int n, vi a) {
    for(int i = n; i >= 0; i--) {
        for(int cnt = n + 1; cnt >= 0; cnt--) {
            for(int prv = 0; prv <= n; prv++) {
                for(int deleting = 0; deleting < 2; deleting++) {
                    if(cnt > n) {
                        ndp[cnt][prv][deleting] = INF;
                        continue;
                    }

                    if(i == n) {
                        ndp[cnt][prv][deleting] = 0;
                        continue;
                    }
                    ...
                    // take
                    if(prv < a[i]) {
                        ckmin(ndp[cnt][prv][deleting], dp[cnt][a[i]][false]);
                    }

                    if(deleting) {
                        ckmin(ndp[cnt][prv][deleting], dp[cnt][prv][true] + 1);
                    } else {
                        ckmin(ndp[cnt][prv][deleting], ndp[cnt + 1][prv][true]);
                    }
                }
            }
        }
        ...
        for(int cnt = n + 1; cnt >= 0; cnt--) {
            for(int prv = 0; prv <= n; prv++) {
                for(int deleting = 0; deleting < 2; deleting++) {
                    dp[cnt][prv][deleting] = ndp[cnt][prv][deleting];
                }
            }
        }
    }
