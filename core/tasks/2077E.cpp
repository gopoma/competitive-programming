...
void slv(const ll _x, const ll _y, const ll _m) {
    dbg(_x, _y, _m);
    ...
    if(msb == -1) {
        ...
        ll n2 = 0;
        for(ll bit = msb; bit >= 0; bit -= 2) {
            n2 += (1LL << bit);
        }
        dbg(n2);
        ll oo = query(n2);
        dbg(oo);
        ll E = 2LL * n2 + x_plus_y - oo;
        dbg(E);
        ...
        ll E2 = x_plus_y - E;
        for(ll bit = msb - 1; bit >= 0; bit -= 2) {
            if(E2 & (1LL << bit)) {
                x += (1LL << bit);
            } else if(E2 & (1LL << (bit + 1LL))) {
                x += (1LL << bit);
                y += (1LL << bit);
            } else {
                continue;
...
int main() {
    ...
    while(0) {
        RAYA;
        ll x = rng_ll(0, ll(1LL << 30) - 1LL);
        ll y = rng_ll(0, ll(1LL << 30) - 1LL);
        ll m = rng_ll(0, ll(1LL << 30) - 1LL);
        slv(x, y, m);
    }
