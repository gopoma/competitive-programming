...
bigint brute(ll K) {
    ll L = binpow(10LL, K - 1LL);
    ll R = binpow(10LL, K) - 1LL;
    bigint ans = 0;
    for(ll A = L; A <= R; A++) {
        for(ll B = L; B <= R; B++) {
            if(ds(A+B) == ds(A)+ds(B)) {
...
bigint slv(ll K) {
    vector<bigint> memo(K+5, bigint(-1));
    function<bigint(ll)> dp = [&](ll idx) -> bigint {
        if(idx == K) return bigint(1LL);
        if(memo[idx] != bigint(-1)) return memo[idx];
        bigint ans = 0;
        ll L = (idx == 0)? 1 : 0;
        ll R = 9;
        for(ll d = L; d <= R; d++) {
            for(ll other = L; other <= R; other++) {
                if(d + other <= R) {
                    ans += dp(idx + 1LL);
                }
            }
