...
void solve() {
    ...
    V<ll> A(N), ColorA(N), B(N), ColorB(N);
    for(auto& x: A) cin >> x;
    for(auto& x: ColorA) cin >> x;
    for(auto& x: B) cin >> x;
    for(auto& x: ColorB) cin >> x;

    dbg(A, ColorA);
    dbg(B, ColorB);

    map<ll, multiset<ll>> use;
    map<ll, bool> thereB;
    for(int i = 0; i < N; i++) {
        use[ColorA[i]].emplace(A[i]);
        use[ColorB[i]].emplace(B[i]);

        thereB[ColorB[i]] = true;
    }

    V<bool> estatico(N, false);
    for(int i = 0; i < N; i++)
        if(!thereB[ColorA[i]])
            estatico[i] = true;
    dbg(estatico);
    ...
    RAYA;
    for(auto& it: use) dbg(it.first, it.second);
    ...
    for(int i = 0; i < N; i++) {
        ...
        if(estatico[i]) {
            ...
            if(use[ColorA[i]].lower_bound(komp) == use[ColorA[i]].end()) {
                cout << "No\n";
                return;
            } else {
                komp = *use[ColorA[i]].lower_bound(komp);
                erase(use[ColorA[i]], komp);
            }
        }
    }
    cout << "Yes\n";
