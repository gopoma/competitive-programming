...
const ll MAXA = int(2e7) + 5;
int dp[MAXA + 5]{};
bool is_composite[MAXA + 5]{};
...
void slv(int c, int d, int x) {
    auto get = [&](int g) -> int {
        int e = x / g + d;
        if(e % c != 0) return 0;
        // assert(1 <= (e / c) && (e / c) <= int(1e7));
        dbg(g, x / g + d, e / c, dp[e / c]);
        return (1 << dp[e / c]);
    };
...
int main() {
    ...
    while(isDebugging && 0) {
        ...
        RAYA;
        RAYA;
        int c = rng_int(int(1), int(100));
        int d = rng_int(int(1), int(100));
        int x = rng_int(int(1), int(100));
        dbg(c, d, x);
        slv(c, d, x);
    }
