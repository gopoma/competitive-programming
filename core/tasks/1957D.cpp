...
ll brute(ll n, vi a) {
    ...
    for(int x = 0; x < n; x++) {
        for(int y = x; y < n; y++) {
            for(int z = y; z < n; z++) {
                response += int((query(x, y) ^ query(y, z)) > (query(x, z)));
            }
        }
    }
    return response;
...
ll slv(ll n, vi a) {
    ...
    vvi cntOdd(MXBIT, vi(n)), cntEven(MXBIT, vi(n));
    for(int i = 0; i < n; i++) {
        for(int bit = 0; bit < MXBIT; bit++) {
            if(dp[bit][i] & 1) cntOdd[bit][i]++;
            else               cntEven[bit][i]++;

            if(0 <= i - 1) {
                cntOdd[bit][i]  += cntOdd[bit][i - 1];
                cntEven[bit][i] += cntEven[bit][i - 1];
            }
        }
    }

    auto query = [&](vi& pref, int L, int R) -> int {
    ...
    for(int i = 0; i < n; i++) {
        ...
        ll evenLeft = 1 + query(cntEven[msb], 0, i - 1);
        ll evenRight = query(cntEven[msb], i, n - 1);

        ll oddLeft = query(cntOdd[msb], 0, i - 1);
        ll oddRight = query(cntOdd[msb], i, n - 1);

        //* dbg(cntEven[msb], cntOdd[msb], evenLeft, evenRight, oddLeft, oddRight);

        ll contrib = 0;
        contrib += evenLeft * evenRight;
        contrib += oddLeft * oddRight;
...
int main() {
    ...
    while(0) {
        RAYA;
        ll n = rng_ll(1, 500);
        vi a(n); for(auto& x: a) x = rng_ll(1, ll(1e9));
        ll ans = brute(n, a);
        ll greedy = slv(n, a);
        dbg(ans, greedy);
        chk(ans == greedy);
