...
void solve() {
    ...
    rep(q) {
        ...
        bool should_u = false;
        bool should_v = false;
        if(t == 1) should_u = true;
        else       should_v = true;
        if(parent[u] == v) {
            swap(u, v);
            swap(should_u, should_v);
        }
        // u is parent
        dbg(u, v, should_u, should_v);

        if(should_u) {
            diff_array[0] += x;
            diff_array[0 + subtree_size[0]] -= x;

            diff_array[where[v]] += -x;
            diff_array[where[v] + subtree_size[v]] -= -x;
        } else {
            diff_array[where[v]] += x;
            diff_array[where[v] + subtree_size[v]] -= x;
        }
    ...
    for(int i = 1; i < sz(diff_array); i++)
        diff_array[i] += diff_array[i - 1];

    vl response(n);
    for(int i = 0; i < n; i++) response[order[i]] = diff_array[i];
    dbg(response);
