...
//    }
    ...
    // lst: amount of zeros taken previously including a one
//    auto dp = [&](auto&& self, int lst, int count) -> mi {
//        if(count == n) return 1;
//        if(vis[lst][count]) return memo[lst][count];
//        vis[lst][count] = true;
//
//        mi re = 0;
//
...
//            }
//
//            if((lst + z - 1 <= k) && (count + (lst * (z - 1)) + z <= n)) {
//                re += self(self, z, count + (lst * (z - 1)) + z);
//                nxt = true;
//            }
//
//            if(!nxt) break;
//        }
//        return memo[lst][count] = re;
//    };
    ...
    for(int count = n; count >= 1; count--) {
        for(int lst = 1; lst <= k; lst++) {
            ...
            for(int z = 1; ; z++) {
                ...
                if((lst + z - 1 <= k) && (count + (lst * (z - 1)) + z <= n)) {
                    dp[lst][count] += dp[z][count + (lst * (z - 1)) + z];
                    nxt = true;
                }
