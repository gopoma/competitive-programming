...
mi brute(ll n, ll m, ll L, ll R) {
    auto work = [&](ll nm, ll e, ll o) -> mi {
        mi re = 0;
        for(ll cnt = 0; cnt <= nm; cnt += 2) {
            assert(nm >= cnt);
            re += F.C(nm, cnt) * pow(mi(o), cnt) * pow(mi(e), nm - cnt);
        }
        return re;
    };
    ...
    return work(T, evens, odds);
}

void slv(ll n, ll m, ll L, ll R) {
    const ll T = n * m;
    const ll count = R - L + 1;
    dbg(T, count);

    mi response = 0;

    if(count % 2 == 0) {
        response = pow(mi(count / 2), T) * pow(mi(2), T - 1);
    } else {
        ll oo = (count - 1) / 2;
        if(T % 2 == 0) {
            response = (pow(mi(2 * oo) + 1, T) + 1) / 2;
        } else {
            mi delta = 0;
            if(oo & 1) delta = -1;
            else delta = +1;

            response = (pow(mi(2 * oo) + 1, T) + delta) / 2;
            dbg(count, oo, int(delta), int(response));
    ...
    if(isDebugging && 1) {
        dbg("Brute");
        mi ans = brute(n, m, L, R);
        dbg("/Brute");
        dbg(int(response), int(ans));
        assert(response == ans);
    }
}
