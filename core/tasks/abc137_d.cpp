...
ll slv(ll n, ll m, vpl info) {
    sort(all(info), [&](pl& X, pl& Y) {
        if(X.s != Y.s) return X.s > Y.s;
        return X.f > Y.f;
    });
    set<ll> days;
    for(int i = 0; i < m; i++) days.emplace(i);
    ll response = 0;
    for(auto& [A, B]: info) {
        ll from = 0;
        ll to   = m - A;
        auto it = days.upper_bound(to);
...
int main() {
    ...
    while(0) {
        ...
        dbg(n, m);
        dbg(info);
        ll ans = brute(n, m, info);
        ll greedy = slv(n, m, info);
        dbg(ans, greedy);
        chk(ans == greedy);
    }
