...
ll brute(ll n, ll m, vvl a) {
    vl p(n); iota(all(p), 0);

    auto get = [&]() {
        vl temp;
        for(auto& i: p) for(auto& x: a[i]) temp.eb(x);
        return work(temp);
    };
...
ll slv(ll n, ll m, vvl& a) {
    vl sums(n); for(int i = 0; i < n; i++) sums[i] = accumulate(all(a[i]), 0LL);
    vl order(n); iota(all(order), 0);
    sort(all(order), [&](ll i, ll j) {
        return sums[i] > sums[j];
    });
    vl temp; for(auto& i: order) for(auto& x: a[i]) temp.eb(x);
    return work(temp);
}
...
int main() {
    ...
    while(1) {
        ...
        ll n = rng_ll(1, 5);
        ll m = rng_ll(1, 5);
        vvl a(n, vl(m)); for(auto& vec: a) for(auto& x: vec) x = rng_ll(1, 5);
        dbg(n, m);
        for(auto& vec: a) dbg(vec, accumulate(all(vec), 0LL));
        dbg("Brute");
        ll ans = brute(n, m, a);
        dbg("/Brute");
        dbg("Greedy");
