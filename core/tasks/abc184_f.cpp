...
void solve() {
    ...
    int tot_left  = cdiv(ll(n), 2LL);
    int tot_right = n / 2;

    vi left, right;
    for(int i = 0; i < tot_left; i++) left.eb(a[i]);
    for(int i = tot_left; i < n; i++) right.eb(a[i]);

    set<int> S;
    for(int mask = 0; mask < (1 << tot_right); mask++) {
        bool ok = true;
        int sum = 0;
        for(int i = 0; i < tot_right; i++) {
            if(mask & (1 << i)) {
                sum += right[i];
                if(sum > T) {
                    ok = false;
    ...
    int ans = 0;
    for(int mask = 0; mask < (1 << tot_left); mask++) {
        int sum = 0;
        bool ok = true;
        for(int i = 0; i < tot_left; i++) {
            if(mask & (1 << i)) {
                sum += left[i];
                if(sum > T) {
                    ok = false;
