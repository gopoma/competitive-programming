...
void solve() {
    ...
    vpl a(d); for(auto& x: a) cin >> x.f >> x.s;

    ll sum_mn = 0, sum_mx = 0;
    for(auto& x: a) {
        sum_mn += x.f;
        sum_mx += x.s;
    }

    if(sumTime < sum_mn || sum_mx < sumTime) cout << "NO\n";
    else {
        cout << "YES\n";
        vl ans(d);
        for(int i = 0; i < d; i++) ans[i] = a[i].f;

        ll need = sumTime - sum_mn;
        int current = 0;
        for(ll _ = 0; _ < need; _++) {
            while(ans[current] == a[current].s) current++;
            ans[current]++;
        }
