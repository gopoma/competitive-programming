...
void solve() {
    ...
    AngelBeats st(A);
    //? for(int i = 0; i < N; i++) dbg(i, st.range_sum(i, i + 1), A[i]);

    each(idx, B) {
        //? RAYA;
        //? dbg(idx);

        ll k = st.range_sum(idx, idx + 1);
        st.range_update(idx, idx + 1, 0LL);

        ll L = get(idx + 1);
        ll R = get(L + k - 1LL);
        //? dbg(L, R, k, st.range_sum(idx, idx + 1));

        {
            ll xx = fdiv(k, N);
            st.range_add(0, N - 1 + 1, xx);
        }

        if(k % N == 0) continue;

        if(L <= R) st.range_add(L, R + 1, 1LL);
        else {
            //? L > R
            st.range_add(0, R + 1, 1LL);
            st.range_add(L, N - 1 + 1, 1LL);
        }
    }

    vl ans(N);
    for(int i = 0; i < N; i++) {
        ans[i] = st.range_sum(i, i + 1);
    }
    ps(ans);
}
