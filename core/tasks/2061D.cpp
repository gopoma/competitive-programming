...
bool brute(vl a, vl b) {
    ll n = sz(a);
    ll m = sz(b);

    multiset<ll> A, B;
    for(auto& x: a) A.emplace(x);
    for(auto& x: b) B.emplace(x);

    map<multiset<ll>, bool> vis; vis[A] = true;
    deque<multiset<ll>> q; q.eb(A);
    while(!q.empty()) {
        auto cur = q.ft; q.pop_front();
        if(cur == B) return true;
        vl arr_cur; for(auto& x: cur) arr_cur.eb(x);

        for(int i = 0; i < sz(arr_cur); i++) {
            for(int j = i + 1; j < sz(arr_cur); j++) {
                if(abs(arr_cur[i] - arr_cur[j]) <= 1) {
                    multiset<ll> C;
                    for(int k = 0; k < sz(arr_cur); k++) {
                        if(k != i && k != j) C.emplace(arr_cur[k]);
                    }
                    C.emplace(arr_cur[i] + arr_cur[j]);
                    if(!vis[C]) {
                        vis[C] = true;
                        q.eb(C);
                    }
...
bool slv(vl a, vl b) {
    ...
    while(!A.empty() && !B.empty()) {
        ll mx_A = *A.rbegin(); safeErase(A, mx_A);
        ll mx_B = *B.rbegin(); safeErase(B, mx_B);
        if(mx_A == mx_B) continue;
        else {
            if(mx_B < mx_A || sz(A) < sz(B)) return false;
            ll x = fdiv(mx_B, 2LL);
            ll y = fdiv(mx_B, 2LL) + (mx_B & 1LL);
...
int main() {
    ...
    if(isDebugging && false) {
        auto get = [&](ll n, ll MIN_VAL, ll MAX_VAL) -> vvl {
            vvl response;
            vl cur;
            auto backtrack = [&](const auto& backtrack) -> void {
                ...
                if(!cur.empty()) response.eb(cur);
                for(ll x = MIN_VAL; x <= MAX_VAL; x++) {
                    cur.eb(x);
                    backtrack(backtrack);
                    cur.pop_back();
                }
    ...
    while(0) {
        RAYA;
        ll n = ll(2e5);
        ll m = rng_ll(ll(1e4), n);
        vl a(n); for(auto& x: a) x = rng_ll(1, ll(1e3));
        vl b(m); for(auto& x: b) x = rng_ll(1, ll(1e3));
        dbg(n, m);
