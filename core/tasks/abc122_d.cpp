...
bool vis[100 + 1][5 + 1][5 + 1][5 + 1];
mi  memo[100 + 1][5 + 1][5 + 1][5 + 1];

void solve() {
    int n; cin >> n;

    auto check = [&](vi& cur) -> bool {
        bool ok = true;
        for(int i = 0; i < sz(cur); i++) {
            int j = i + 2;
            if(j >= sz(cur)) break;
            ok &= !(cur[i] == 1 && cur[i + 1] == 2 && cur[i + 2] == 3);
        }
        return ok;
    };
    ...
    auto dp = [&](auto&& self, int i, int lst, int lstlst, int lstlstlst) -> mi {
        if(i == n) return 1;

        if(vis[i][lst][lstlst][lstlstlst]) return memo[i][lst][lstlst][lstlstlst];
        vis[i][lst][lstlst][lstlstlst] = true;

        mi ans = 0;
        for(int nxt = 0; nxt <= 3; nxt++) {
            vi cur{lstlstlst, lstlst, lst, nxt};
            bool ok = check(cur);
            for(int j = 0; j + 1 < sz(cur); j++) {
                swap(cur[j], cur[j + 1]);
                ok &= check(cur);
                swap(cur[j], cur[j + 1]);
            }
            if(ok) {
                ans += self(self, i + 1, nxt, lst, lstlst);
            }
        }
        return memo[i][lst][lstlst][lstlstlst] = ans;
    };
    mi response = dp(dp, 0, 4, 4, 4);
    cout << response << "\n";
}
