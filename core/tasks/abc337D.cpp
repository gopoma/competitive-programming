...
vector<vector<char>> rotate(vector<vector<char>>& mtx) {
    const int rows = sz(mtx);
    const int cols = sz(mtx[0]);
...
void solve() {
    ...
    auto work = [&]() {
        RAYA; each(row, grid) dbg(row); RAYA;
        ...
        for(int i = 0; i < H; i++) {
            ...
            vl go(W);
            for(int j = 0; j < W; j++) {
                if(grid[i][j] == 'x') go[j] = 1e12;
                else if(grid[i][j] == '.') go[j] = 1;
                else {
                    assert(grid[i][j] == 'o');
                    go[j] = 0;
                }
            ...
            for(int j = 1; j < W; j++) go[j] += go[j - 1];

            auto query = [&](int left, int right) {
                ll sum = go[right];
                if(left - 1 >= 0) sum -= go[left - 1];
