...
void solve() {
    ...
    const int MAX_DIGITS = sz(S);
    vi S_digits;
    for(auto& c: S) S_digits.eb(c - '0');

    dbg(MAX_DIGITS, S_digits);

    const int LEADING_ZERO = 10;

    bool vis[MAX_DIGITS + 5][11][2][5];
    memset(vis, 0, sizeof(vis));
    bigint memo[MAX_DIGITS + 5][11][2][5];
    function<bigint(int, int, int, int)> dp = [&](int idx, int last, int reached_lower, int amount_non_zero_digits) -> bigint {
        if(amount_non_zero_digits > K) return bigint(0);
        if(idx == MAX_DIGITS) {
            if(last == LEADING_ZERO) return bigint(0);
            else return bigint(amount_non_zero_digits == K);
        }

        if(vis[idx][last][reached_lower][amount_non_zero_digits]) return memo[idx][last][reached_lower][amount_non_zero_digits];
        vis[idx][last][reached_lower][amount_non_zero_digits] = true;

        bigint ans = bigint("0");
        if(last == LEADING_ZERO) {
            ans += dp(idx + 1, LEADING_ZERO, false, 0);

            if(0 < idx) {
                for(int d = 1; d <= 9; d++) {
                    ans += dp(idx + 1, d, true, 1);
                }
            } else {
                for(int d = 1; d <= S_digits[idx]; d++) {
                    ans += dp(idx + 1, d, d < S_digits[idx], 1);
                }
            }
        } else {
            if(reached_lower) {
                for(int d = 0; d <= 9; d++) {
                    ans += dp(idx + 1, d, true, amount_non_zero_digits + (d != 0));
                }
            } else {
                for(int d = 0; d <= S_digits[idx]; d++) {
                    ans += dp(idx + 1, d, d < S_digits[idx], amount_non_zero_digits + (d != 0));
                }
            }
        }
        return memo[idx][last][reached_lower][amount_non_zero_digits] = ans;
    };

    bigint ans = dp(0, LEADING_ZERO, false, 0);
    dbg(ans);
    ps(ans);
}
