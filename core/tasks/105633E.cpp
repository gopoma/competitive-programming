...
void solve() {
    ...
    auto work = [&](auto&& self, int i, int j, int parent_i, int parent_j) -> ll {
        if(S[i][j] == 'P' || S[i][j] == '#') {
            for(int k = 0; k < 4; k++) {
                int ni = i + dx[k];
                int nj = j + dy[k];
                if((mp(ni, nj) != mp(parent_i, parent_j)) && check(ni, nj) && S[ni][nj] != '.') {
                    return self(self, ni, nj, i, j);
                }
            }
            ...
            for(int k = 0; k < 4; k++) {
                ...
                int nj = j + dy[k];
                if((mp(ni, nj) != mp(parent_i, parent_j)) && check(ni, nj) && S[ni][nj] != '.') {
                    nxt.eb(self(self, ni, nj, i, j));
                }
            }
            assert(sz(nxt) == 2);

            ll mn = *min_element(all(nxt));
            ll mx = *max_element(all(nxt));
            if(S[i][j] == '+') return mn + mx;
            else if(S[i][j] == '-')  return mx - mn;
            else if(S[i][j] == '*') return mn * mx;
            else return mx / mn;
        }
    };

    ll response = work(work, px, py, -1, -1);
    dbg(response);
    cout << response << "\n";
}
