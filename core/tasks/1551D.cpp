...
void slv(int n, int m, int k) {
    ...
    if(n % 2 == 0 && m % 2 == 0) {
        ...
        const int mx = (n * (m - 1)) >> 1;
        ok &= (k % 2 == 0) && (k <= mx);

        for(int i = 0; i + 1 < n; i += 2) {
            re[i][m - 1] = re[i + 1][m - 1] = cid++;
        }
    } else {
        assert(n & 1);
        assert(m % 2 == 0);

        const int mx = ((n - 1) * m) >> 1;
    ...
    for(int i = 0; i + 1 < n; i += 2) {
        for(int j = 0; j + 1 < m; j += 2) {
            ...
            if(k > 0) {
                ...
                re[i][j] = re[i][j + 1] = cid++;;
                re[i + 1][j] = re[i + 1][j + 1] = cid++;
            } else {
                re[i][j] = re[i + 1][j] = cid++;
                re[i][j + 1] = re[i + 1][j + 1] = cid++;
            }
        }
    }
    assert(k == 0);
    ...
    vector<vector<char>> response(n, vector<char>(m, '$'));
    for(int i = 1; i <= ((n * m) >> 1); i++) {
        ...
        for(auto& [x, y]: G[i]) {
            for(int d = 0; d < 4; d++) {
                ...
                if(check(nx, ny) && !vis.count({nx, ny})) {
                    nxt.eb(nx, ny);
                }
            }
        }
    ...
    for(auto& vec: response) {
        str current; for(auto& c: vec) current.pb(c);
        cout << current << "\n";
    }
}
