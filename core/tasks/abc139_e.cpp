...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j + 1 < n - 1; j++) {
            ...
            adj[id[u][v]].eb(id[u2][v2]);
            tp.ae(id[u][v], id[u2][v2]);
        }
    }
    ...
    vb  vis(MAXN);
    vi memo(MAXN);
    auto dp = [&](auto&& self, int src) -> int {
        if(vis[src]) return memo[src];
        vis[src] = true;

        int ans = 1;
        for(auto& nxt: adj[src]) {
            ckmax(ans, self(self, nxt) + 1);
        }
    ...
    int response = 0;
    for(int u = 0; u < MAXN; u++) ckmax(response, dp(dp, u));
    cout << response << "\n";
}
