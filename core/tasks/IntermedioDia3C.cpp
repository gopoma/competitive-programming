...
void solve() {
    ...
    ll ans = modPow(2LL, fdiv(n * (n - 1LL), 2LL), p);
...
void brute() {
    ...
    //? 0 -> first  - second
    //? 1 -> second - first
    const int m = sz(unordered_edges);
    dbg(sz(unordered_edges));
    dbg(m);
    ...
    for(int i = 0; i < (1 << m); i++) {
        ...
        for(int j = 0; j < m; j++) {
            int u = unordered_edges[j].f;
            int v = unordered_edges[j].s;
