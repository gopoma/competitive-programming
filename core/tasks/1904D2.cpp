...
bool brute(int n, vi a, vi b) {
    ...
    while(!q.empty()) {
        vi x = q.ft; q.pop_front();
        if(x == b) return true;
        for(int L = 0; L < n; L++) {
            for(int R = L; R < n; R++) {
                vi newx = x;
                int mx = -int(1e9);
                for(int k = L; k <= R; k++) ckmax(mx, newx[k]);
                for(int k = L; k <= R; k++) newx[k] = mx;
                if(!vis[newx]) {
                    vis[newx] = true;
                    q.eb(newx);
                }
...
bool slv(int n, vi a, vi b) {
    map<int, vi> where;
    for(int i = 0; i < n; i++) where[b[i]].eb(i);

    vl aa; for(auto& x: a) aa.eb(ll(x));
    AngelBeats st(aa);
    BIT<int> already; already.init(n);

    auto ddd = [&]() {
        vi ab, bit;
        for(int i = 0; i < n; i++) {
            ab.eb(st.range_max(i, i + 1));
            bit.eb(already.sum(i, i));
        }
        dbg(ab);
        dbg(bit);
    };
    ...
    for(auto& [val, positions]: where) {
        for(auto& i: positions) {
            ...
            if(true) { // left
                ...
                int right = i + 1; // always good
                while(left + 1 < right) {
                    int middle = (left + right) >> 1;
                    if(st.range_max(middle, i + 1) <= val - 1) right = middle;
                    else left = middle;
                }
                dbg(right);
                if(0 <= right - 1 && right - 1 <= i && already.sum(right - 1, i) == 0 && st.range_max(right - 1, i + 1) == val) {
                    st.range_update(right - 1, i + 1, val);
                    found = true;
                }
            }
            if(!found) { // right
                int left = i - 1; // always good
                int right = n;    // always bad
                while(left + 1 < right) {
                    int middle = (left + right) >> 1;
                    if(st.range_max(i, middle + 1) <= val - 1) left = middle;
                    else right = middle;
                }
                dbg(left);
                if(i <= left + 1 && left + 1 < n && already.sum(i, left + 1) == 0 && st.range_max(i, (left + 1) + 1) == val) {
                    st.range_update(i, (left + 1) + 1, val);
                    found = true;
                }
            }
            if(!found) {
                dbg("not found", i, a[i], b[i]);
                // ddd();
                return false;
            }
        }
...
int main() {
    ...
    while(0) {
        ...
        int n = rng_int(1, 10);
        vi a(n), b(n);
        for(auto& x: a) x = rng_int(1, n);
        for(auto& x: b) x = rng_int(1, n);
        dbg(n);
        dbg(a);
        dbg(b);
        bool ans = brute(n, a, b);
