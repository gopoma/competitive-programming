...
void solve() {
    ...
    auto get = [&](string T, string P) {
        string S = P + '#' + T;
        vector<int> pi = goes(S);

        vector<int> res;
        for(int i = 0; i < sz(pi); i++) {
            if(pi[i] == sz(P)) {
                res.emplace_back(i - 2 * sz(P));
            }
        }
        return res;
    };

    vector<int> goi = get(s, a);
    vector<int> tmp = get(s, b);
    set<int> goj; for(auto& x: tmp) goj.emplace(x);

    vector<int> ans;
    for(auto& i: goi) {
        auto left = goj.lower_bound(i);
        auto right = goj.lower_bound(i);
