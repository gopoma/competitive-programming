...
void solve() {
    ...
    set<int> zeros, ones;
    for(int i = 0; i < n; i++) {
        if(S[i] == '0') zeros.emplace(i);
        else if(S[i] == '1') ones.emplace(i);
        else assert(false);
    }
    ...
    for(; k < n; k++) {
        if(T[k] != S[k]) continue;
        if(S[k] == '0') {
            auto aux = ones.lower_bound(k);
            if(aux == ones.end()) break;
            int val = *aux;
            ...
            assert(S[k] == '1');
            auto aux = zeros.lower_bound(k);
            if(aux == zeros.end()) break;
            int val = *aux;
