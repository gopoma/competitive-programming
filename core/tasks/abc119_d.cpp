...
void solve() {
    ...
    vl queries(q); for(auto& x: queries) cin >> x;
    ...
    for(auto& x: queries) {
        ll ans = BIG;
        auto leftS = a[upper_bound(all(a), x) - a.begin() - 1];
        auto leftT = b[upper_bound(all(b), x) - b.begin() - 1];
        auto rightS = a[upper_bound(all(a), x) - a.begin()];
        auto rightT = b[upper_bound(all(b), x) - b.begin()];
        //* from a from b
        //* left   left
        ckmin(ans, custom_abs(min(leftS, leftT) - x));
        //* left   right
        ckmin(ans, custom_abs(leftS - x) + custom_abs(leftS - rightT));
        ckmin(ans, custom_abs(rightT - x) + custom_abs(rightT - leftS));
        //* right right
        ckmin(ans, custom_abs(max(rightS, rightT) - x));
        //* right left
        ckmin(ans, custom_abs(rightS - x) + custom_abs(rightS - leftT));
        ckmin(ans, custom_abs(leftT - x) + custom_abs(leftT - rightS));
        response.eb(ans);
    }
