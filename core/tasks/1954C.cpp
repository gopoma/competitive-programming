...
bigint brute(str x, str y) {
    const int n = sz(x);
    const int m = sz(y); chk(n == m);
    ...
    for(int i = 0; i < (1 << n); i++) {
        ...
        for(int j = 0; j < n; j++) {
            if(i & (1 << j)) {
                swap(act_x[j], act_y[j]);
            }
        }

        bigint go_x = bigint(act_x);
        bigint go_y = bigint(act_y);
    ...
    for(int i = 0; i < (1 << n); i++) {
        ...
        if(go_x * go_y == mx) {
            dbg(bitset<15>(i), "answer: ", go_x, go_y);
        }
    }
    dbg(mx);
...
bigint solve2(str x, str y) {
    ...
    int go = 0;
    while(go < n && mx[go] == mn[go]) {
        go++;
    }

    dbg("Greedy", mx, mn);
    if(go == n) {
        return bigint(mx) * bigint(mn);
    } else {
        if(mx[go] < mn[go]) swap(mx[go], mn[go]);
...
void solve() {
    ...
    //? chk(brute(x, y) == bigint(mx) * bigint(mn));
}
...
signed main() {
    ...
    while(false) {
        ...
        rep(n) {
            x.pb(ts(rng_int(1, 9)).ft);
            y.pb(ts(rng_int(1, 9)).ft);
        }

        //? while(x.ft == y.ft) {
        //?     x.ft = ts(rng_int(1, 9)).ft;
        //?     y.ft = ts(rng_int(1, 9)).ft;
        //? }
        ...
        bigint ans = brute(x, y);
        bigint greedy = solve2(x, y);
