...
void solve() {
    ...
    vs B(n); for(auto& x: B) cin >> x;
    auto apply_vertical = [&](vs& C, int j) -> void {
        for(int i = 0; i < n; i++) {
            C[i][j] = (C[i][j] == '0')? '1' : '0';
        }
    };
    auto apply_horizontal = [&](vs& C, int i) -> void {
        for(int j = 0; j < n; j++) {
            C[i][j] = (C[i][j] == '0')? '1' : '0';
        }
    };
    auto check = [&](vs& C, vs& D) {
        for(int j = 0; j < n; j++) {
            if(C[0][j] != D[0][j]) apply_vertical(C, j);
        }
        chk(C[0] == D[0]);
        for(int i = 1; i < n; i++) {
            if(C[i] != D[i]) apply_horizontal(C, i);
        }
        return (C == D);
    };
    bool ok = check(A, B);
    apply_horizontal(A, 0);
    ok |= check(A, B);
    cout << (ok?"YES":"NO") << "\n";
}
