...
void solve() {
    ...
    vl pref(n); pref[0] = a[0];
    for(int i = 1; i < n; i++) pref[i] = pref[i - 1] + a[i];

    auto query = [&](int left, int right) {
        ll sum = pref[right];

        if(left - 1 >= 0) sum -= pref[left - 1];
    ...
    vector<tuple<ll, int, int>> go;
    for(int i = 0; i < n; i++) {
        for(int j = i; j < n; j++) {
            //? range [i, j]
            go.eb(make_tuple(query(i, j), i, j));
        }
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(auto& [val, left, right]: go) {
            if(left <= i && i <= right) afectados.eb(val);
            else libres.eb(val);
        }

        //?dbg(i, a[i], afectados, libres);
        //?RAYA;

        ll ans = findSmallestDifference(afectados, libres);
