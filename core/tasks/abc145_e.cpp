...
void solve() {
    ...
    sort(all(items), [&](pi& x, pi& y) {
        if(x.f != y.f) return x.f < y.f;
        return x.s > y.s;
    });

    //* for(auto& [A, B]: items) cerr << "A, B: " << A << " " << B << "\n";

    for(int i = 0; i <= n; i++)
        for(int j = 0; j <= t; j++)
            vis[i][j] = false;
    auto dp = [&](auto&& self, int i, int timestamp) -> int {
        if(timestamp == t) return 0;
        if(i == n) return 0;

        if(vis[i][timestamp]) return memo[i][timestamp];
        vis[i][timestamp] = true;

        int ans = -INF;
        //* ignore
        ckmax(ans, self(self, i + 1, timestamp));

        //* take
        auto [A, B] = items[i];
        ckmax(ans, self(self, i + 1, min(t, timestamp + A)) + B);

        return memo[i][timestamp] = ans;
    };
