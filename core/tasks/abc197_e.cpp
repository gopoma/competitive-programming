...
bool vis[int(2e5) + 5][2];
ll  memo[int(2e5) + 5][2];
void solve() {
    ll n; cin >> n;
    vpl a(n); for(auto& [X, C]: a) cin >> X >> C;
    map<ll, vl> where;
    for(auto& [X, C]: a) where[C].eb(X);
    vpl intervals;
    for(auto& [val, positions]: where) {
        sor(positions);
        intervals.eb(positions.ft, positions.bk);
    }
    const int N = sz(intervals);
    for(int i = 0; i <= N; i++) for(int j = 0; j < 2; j++) vis[i][j] = false;
    auto dp = [&](auto&& self, int i, bool fst) -> ll {
        if(i == N) {
            ll lst = -BIG;
            if(fst) lst = intervals[i - 1].f;
            else lst = intervals[i - 1].s;
            return abs(lst - 0);
        }
        if(vis[i][fst]) return memo[i][fst];
        vis[i][fst] = true;
        ll ans = BIG;
        ll lst = -BIG;
        if(i == 0) lst = 0;
        ...
        ckmin(ans, self(self, i + 1, true) + abs(lst - intervals[i].s) + abs(intervals[i].s - intervals[i].f));
        ckmin(ans, self(self, i + 1, false) + abs(lst - intervals[i].f) + abs(intervals[i].s - intervals[i].f));
        return memo[i][fst] = ans;
    };
    ll ans = dp(dp, 0, false);
    dbg(ans);
    cout << ans << "\n";
}
