...
void solve() {
    ...
    ll ans = 0;
    for(int i = 1; i < sz(a); i++) ans += a[i].second - a[i].first + 1;

    dbg(ans);
    for(int i = 1; i < sz(a); i++) {
        int l_anterior = a[i - 1].second + 1;
        int r_actual = a[i].first;
        int salto = r_actual - l_anterior;
        dbg(l_anterior, r_actual, salto, salto % x);
        ans += salto % x;
    }
    cout << ans << n_l;
}
