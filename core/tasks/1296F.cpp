...
void solve() {
    ...
    vector edges_values(n + 1, vector<int>(n + 1, 1));
    dbg("Processing...");
    for(auto& [a, b, g]: constraints) {
        dbg(a, b, g);
        ...
        while(current != -1) {
            if(parent[current] != -1) {
                ckmax(edges_values[min(current, parent[current])][max(current, parent[current])], g);
            }
    ...
    for(auto& [a, b, g]: constraints) {
        ...
        while(current != -1) {
            if(parent[current] != -1) {
                ckmin(mn, edges_values[min(current, parent[current])][max(current, parent[current])]);
            }
    ...
    for(int i = 0; i < n - 1; i++) {
        ...
        cout << edges_values[min(u, v)][max(u, v)] << " ";
    }
    cout << "\n";
}
