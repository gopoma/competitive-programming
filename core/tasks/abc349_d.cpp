...
ll brute(ll L, ll R) {
 ...
 pair<ll, vpl> check(ll L, ll R) {
     ...
     while(act != R) {
         ...
         for(ll bit = 0; bit < 62; bit++) {
             ll hop = (1LL << bit);
             if(act % hop == 0 && act + hop <= R) {
                 ckmax(mx_hop, hop);
             }
         }
         assert(mx_hop != 0);

         intervals.eb(act, act + mx_hop);

         ans++;
         act += mx_hop;
     }
     return mp(ans, intervals);
 }
...
signed main() {
    ...
    while(true) {
        RAYA;
        ll L = rng_ll(1, 1LL << 40LL);
        ll R = rng_ll(L + 1, 1LL << 60LL);
        chk(0 <= L && L < R);

        dbg(L, R);

        ll ans = check(L, R).f;
        //? ll greedy = solve(L, R).f;
        dbg("Starting Greedy");
        ll greedy = solve(L, R);
        dbg("End Greedy");
