...
mi brute(int n, vi a) {
    ...
    do {
        ...
        remDup(pU);
        bool ok = true;
        for(auto& x: pU) ok &= isPrime[x];
        if(ok && sz(pU) == n && is_sorted(all(p))) {
            info.emplace(p, k);
        }
    } while(next_permutation(all(a)));

    //* for(auto& it: info) dbg(it);
    return mi(sz(info));
}

bool vis[4044 + 5][4044 + 5];
mi  memo[4044 + 5][4044 + 5];
...
mi slv(int n, vi a) {
    ...
    auto dp = [&](auto&& self, int i, int cntPrimes) -> mi {
        if(i == m) {
            //* dbg(i, cntPrimes);
            if(cntPrimes == n) return F.fac[n];
            else return 0;
        }

        if(vis[i][cntPrimes]) return memo[i][cntPrimes];
        vis[i][cntPrimes] = true;

        mi ans = 0;
        if(isPrime[values[i]]) {
            //* take
            ans += self(self, i + 1, cntPrimes + 1) * F.ifac[hist[i] - 1];
        }

        //* don't take
        ans += self(self, i + 1, cntPrimes) * F.ifac[hist[i]];

        //* dbg(i, values[i], isPrime[values[i]], hist[i], ans);
        return memo[i][cntPrimes] = ans;
    };

    //* dbg(F.ifac[0]);
    mi response = dp(dp, 0, 0);
    return response;
}

void solve() {
    int n; cin >> n;
    vi a(2 * n); for(auto& x: a) cin >> x;
    dbg(n);
    dbg(a);
...
int main() {
    ...
    while(0) {
        ...
        vi a(2 * n); for(auto& x: a) {
            if(true) {
                x = primes[rng_int(0, sz(primes) - 1)];
            } else {
                x = rng_int(1, 10);
            }
        }
