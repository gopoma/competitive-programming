...
bool brute(ll n, vl a, vl b) {
    ...
    map<vl, map<vl, bool>> memo;
    auto dp = [&](const auto& dp, vl arr, vl arr2) -> bool {
        if(arr == arr2) {
            return true;
        }
        if(vis[arr][arr2]) return memo[arr][arr2];
        vis[arr][arr2] = true;
...
bool slv(ll n, vl a, vl b) {
    ...
    ll countA = count_inv(a);
    ll countB = count_inv(b);
