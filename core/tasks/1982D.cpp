...
template<class T> struct PrefixSums {
	...
	T get(int X1, int X2, int Y1, int Y2) { // all are inclusive
		X2 ++, Y2 ++;
...
void solve() {
    ...
    PrefixSums<ll> st; st.init(b);
    ll g = 0;
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            const int nxt_i = i + k - 1;
            const int nxt_j = j + k - 1;
            if(nxt_i >= n || nxt_j >= m) continue;

            ll par = st.get(i, nxt_i, j, nxt_j);
            if(par != 0) g = gcd(g, custom_abs(par));
        }
    }
