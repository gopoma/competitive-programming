...
void solve() {
    ...
    multiset<int> ms; for(auto& x: a) ms.emplace(x);
    map<int, int> hist; for(auto& x: a) hist[x]++;
    set<int> twos; for(auto& [val, cnt]: hist) if(cnt >= 2) twos.emplace(val);

    auto upd = [&](int v) -> void {
        if(twos.count(v)) safeErase(twos, v);
        if(hist[v] >= 2) twos.emplace(v);
    };
    ...
    while(n > 0) {
        ...
        if(mx == mx_twos) {
            ...
            safeErase(ms, mx_twos);
            ms.emplace(mx_twos + 1);
            hist[mx_twos]--;
            hist[mx_twos + 1]++;
            upd(mx_twos);
            upd(mx_twos + 1);
