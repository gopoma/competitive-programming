...
void solve() {
    ...
    vvi dist(H, vi(W, INF)); dist[Ch][Cw] = 0;
    deque<pi> q; q.eb(Ch, Cw);
    while(!q.empty()) {
        ...
        for(int k = 0; k < 4; k++) {
            int nx = x + dx[k];
            int ny = y + dy[k];
            if(check(nx, ny) && S[nx][ny] != '#' && dist[x][y] < dist[nx][ny]) {
                dist[nx][ny] = dist[x][y];
        ...
        for(int i = -2; i <= +2; i++) {
            for(int j = -2; j <= +2; j++) {
                int nx = x + i;
                int ny = y + j;
                if(check(nx, ny) && S[nx][ny] != '#' && dist[x][y] + 1 < dist[nx][ny]) {
    ...
    int response = dist[Dh][Dw];
    if(response >= INF) response = -1;
    cout << response << "\n";
}
