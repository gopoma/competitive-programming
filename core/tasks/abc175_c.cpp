...
ll brute(ll X, ll K, ll D) {
    map<ll, map<ll, bool>> vis;
    map<ll, map<ll, ll>> memo;
    function<ll(ll, ll)> solve = [&](ll moves, ll act) {
        if(moves == 0) return abs(act);
        if(vis[moves][act]) return memo[moves][act];
        vis[moves][act] = true;

        ll ans = min(solve(moves - 1, act + D), solve(moves - 1, act - D));
        return memo[moves][act] = ans;
    };
    return solve(K, X);
}

ll slv(ll X, ll K, ll D) {
    bigint xx  = bigint(X);
    bigint mul = bigint(K) * bigint(D);
    const bigint zero = bigint("0");

    if(X > 0 && (xx - mul >= zero)) return X - K * D;
    else if(X < 0 && (xx + mul <= zero)) return abs(X + K * D);
    else {
        ll moves_to_reduce = fdiv(abs(X), D);
        K -= moves_to_reduce; assert(K >= 0);

        if(X > 0) X -= moves_to_reduce * D;
        else X += moves_to_reduce * D;

        ll ans = BIG;
        for(ll act = 0; act <= 5; act++) {
            if(K >= act && ((K - act) % 2LL) == 0LL) {
                ckmin(ans, brute(X, act, D));
            }
        }
        assert(ans != BIG);
