...
void solve() {
    ...
    bool ok = true;
    for(auto& [val, count]: cnt) ok &= (count <= k);
    ...
    if(!ok) cout << "NO\n";
        ...
        if(n < k) cout << "NO\n";
            ...
            cout << "YES\n";
            vector<vector<int>> coloring(k);
            vector<set<int>> already(k);

            for(int i = 0; i < k; i++) {
                coloring[i].eb(i);
                already[i].emplace(a[i]);
            }
            for(int i = k; i < n; i++) {
                for(int j = 0; j < k; j++) {
                    if(!already[j].count(a[i])) {
                        coloring[j].eb(i);
                        already[j].emplace(a[i]);
                        break;
                    }
                }
            }
