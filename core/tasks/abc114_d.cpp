...
void solve() {
    ...
    map<ll, ll> factors;
    for(ll x = 1; x <= N; x++) {
        map<ll, ll> partial_factors = factorize(x);
        for(auto& it: partial_factors) {
    ...
    map<ll, ll> cnt;
    //! 3 -> 2 + 1 | 5 -> 4 + 1 | 15 -> 14 + 1 | 25 -> 24 + 1 | 75 -> 74 + 1
    for(auto& it: factors) {
        if(it.second >= 2)  cnt[2]++;
        if(it.second >= 4)  cnt[4]++;
        if(it.second >= 14) cnt[14]++;
        if(it.second >= 24) cnt[24]++;
        if(it.second >= 74) cnt[74]++;
    }
    ...
    ll res = 0;
    //? 5 * 5 * 3 = (4 + 1) * (4 + 1) * (2 + 1)
    res += ((cnt[4] * (cnt[4] - 1LL)) / 2LL) * (cnt[2] - 2LL);
    //? 15 * 5
    res += cnt[14] * (cnt[4] - 1LL);
    //? 25 * 3 -> (24 + 1) * (2 + 1)
    res += cnt[24] * (cnt[2] - 1LL);
    //? 75 -> 74 + 1
    res += cnt[74];
