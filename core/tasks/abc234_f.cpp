...
int cnt[26];
mi  memo[26 + 1][5000 + 1];
bool vis[26 + 1][5000 + 1];

void solve() {
    str S; cin >> S;
    const int n = sz(S);
    for(int i = 0; i < 26; i++) cnt[i] = 0;
    for(int i = 0; i <= 26; i++) for(int j = 0; j <= 26; j++) vis[i][j] = false;

    for(auto& c: S) cnt[c - 'a']++;
    auto dp = [&](auto&& self, int i, int j) -> mi {
        if(i == 26) return mi(int(j > 0));
        ...
        for(int k = 0; k <= cnt[i]; k++) {
            if(k > 0) num *= (j + k);
            ans += self(self, i + 1, j + k) * num * F.ifac[k];
        }
        return memo[i][j] = ans;
    };
    mi ans = dp(dp, 0, 0);
