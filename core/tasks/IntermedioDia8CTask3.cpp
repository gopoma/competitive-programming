...
void solve() {
    ...
    //? for(ll x = 2; x <= 10; x++) dbg(x, get_divisors(x));
    vl divisors = get_divisors(n);
    const int m = sz(divisors);
    ...
    auto check = [](vl& go) {
        ...
        bool ok = true;
        for(int i = 1; i < l; i++) {
            ll mn = min(go[i - 1], go[i]);
            ll mx = max(go[i - 1], go[i]);
            ok &= ((mx % mn == 0) && prime(fdiv(mx, mn)));
        }

        if(l >= 2) {
            ll mn = min(go.ft, go.bk);
            ll mx = max(go.ft, go.bk);
            ok &= ((mx % mn == 0) && prime(fdiv(mx, mn)));
        }
        return ok;
    };
...
//?        }
//?
//?        assert(is_sorted(all(go)));
//?        if(go.empty() || go.ft != 1) continue;
//?        do {
//?            if(check(go)) {
//?                if(ckmax(k, ll(sz(go)))) {
//?                    ans = go;
...
//?    ps(ans);
    ...
    ll val = -1;
    for(int x = 2; x <= 150; x++) {
        vl xd = get_divisors(x);
        if(ckmax(mx, ll(sz(xd)))) val = x;
        dbg(x, sz(xd), xd);
    }
    assert(val != -1);
    dbg(mx, val);
