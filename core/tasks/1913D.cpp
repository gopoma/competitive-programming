...
tcT> struct SegTree { // cmb(ID,b) = b
    ...
    T ID{}; T cmb(T a, T b) { return (a+b)%MOD; }
	int n; V<T> seg;
...
ll brute(ll n, vl a) {
    ...
    set<vl> aux; aux.emplace(a);
    deque<vl> q; q.eb(a);

    while(!q.empty()) {
        vl v = q.ft; q.pop_front();
        ...
        for(int l = 0; l < N; l++) {
            for(int r = l; r < N; r++) {
                ...
                vl temp; for(int k = l; k <= r; k++) temp.eb(v[k]);
                nxt.eb(*min_element(all(temp)));
...
ll brute2(ll n, vl a) {
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < i; j++) {
            ll current_mn = BIG;
            for(int k = j; k < i; k++) {
                ckmin(current_mn, a[k]);
            }

            if(current_mn < min(a[i], a[j])) {
...
void slv(ll n, vl a) {
    ...
    vl dp(n);
    vpl stk;
    stk.emplace_back(-BIG, -1);
    SegTree<ll> st; st.ID = 0LL; st.init(n);

    ll window_sum = 0;
    for(int i = 0; i < n; i++) {
        while(stk.back().first > a[i]) {
            window_sum -= dp[stk.back().second];
            window_sum += MOD;
            window_sum %= MOD;
        ...
        if(stk.back().second + 1 < i) {
            dp[i] += st.query(stk.back().second + 1, i - 1);
            dp[i] %= MOD;
        }

        if(stk.back().first == -BIG) {
            dp[i] += 1;
            dp[i] %= MOD;
        } else {
            dp[i] += window_sum;
        ...
        // dbg(i, dp[i], stk, window_sum);

        window_sum += dp[i];
        window_sum %= MOD;

        // dbg(i, dp[i], window_sum);
    }
    // dbg(dp);
