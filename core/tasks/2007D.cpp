...
ll brute(ll n, vpi edges, str S) {
    ...
    vi leafs;
    for(int u = 0; u < n; u++) if(u != ROOT && sz(adj[u]) == 1) {
        leafs.eb(u);
    }
    //? dbg(leafs);
    //? dbg(par);

    map<str, map<ll, bool>> vis;
    map<str, map<ll, ll>> memo;
    auto dp = [&](const auto& dp, str config, bool turn) -> ll {
        //? dbg(config, turn);
        if(vis[config][turn]) return memo[config][turn];
        vis[config][turn] = true;
        ...
        if(rem == 0) {
            ...
            each(x, leafs) {
                ...
                while(node != -1) {
                    chk(0 <= node && node < n);
                    current.pb(config[node]);
                    node = par[node];
                }
                reverse(all(current));
                int cnt_10 = 0;
                int cnt_01 = 0;
                for(int i = 0; i + 1 < sz(current); i++) {
                    cnt_10 += (current[i] == '1' && current[i + 1] == '0');
                    cnt_01 += (current[i] == '0' && current[i + 1] == '1');
                }
                //? dbg(x, current, cnt_10, cnt_01);
                non_zero += (cnt_10 - cnt_01 != 0);
            }
            return memo[config][turn] = non_zero;
        } else {
            //? dbg("?");
            if(turn) { //? max
                ll ans = -BIG;
                for(int i = 0; i < n; i++) if(config[i] == '?') {
                    {
                        str new_config = config;
                        new_config[i] = '0';
                        ckmax(ans, dp(dp, new_config, !turn));
                    }
                    {
                        str new_config = config;
                        new_config[i] = '1';
                ...
                for(int i = 0; i < n; i++) if(config[i] == '?') {
                    {
                        str new_config = config;
                        new_config[i] = '0';
                        ckmin(ans, dp(dp, new_config, !turn));
                    }
                    {
                        str new_config = config;
                        new_config[i] = '1';
...
ll slv(ll n, vpi edges, str S) {
	...
	dbg(leafs_0, leafs_1, leafs_xd, other);
	ll ans = 0;
	if(S[ROOT] != '?') {
		if(S[ROOT] == '0') {
			ans += leafs_1;
		...
		ans += cdiv(leafs_xd, 2LL);
	} else {
		if(leafs_0 == leafs_1) {
			ans += max(leafs_0, leafs_1);
			if(other % 2LL == 0LL) {
				ans += fdiv(leafs_xd, 2LL);
			} else {
				ans += cdiv(leafs_xd, 2LL);
			}
		} else {
...
signed main() {
	...
	while(0) {
		RAYA;
		ll n = rng_ll(2, 10);
		vpi tree = treeRand(n, rng_int(0, n));
		dbg(n);
		dbg(tree);

		str alphabet = "01?";
		str S; rep(n) S.pb(alphabet[rng_int(0, sz(alphabet) - 1)]);
		dbg(n);
		dbg(tree);
		dbg(S);

		ll ans = brute(n, tree, S);
		ll greedy = slv(n, tree, S);
		dbg(ans, greedy);
		chk(ans == greedy);
	}
