...
ll slv(ll n, ll m, vpl a) {
    ...
    for(auto& [x, cnt]: a) {
        ...
        auto f = [&](ll middle) -> ll {
            ll sum = tot + middle * S;
            sum += fdiv(middle * (middle + 1LL), 2LL) * x;
            // dbg(tot, middle, sum);
            return sum;
        };

        auto ternarySearch = [&](ll low, ll high) { // O(log n)
            while (high - low > 2) {
        ...
        ll local_response = ternarySearch(1, cnt);
        // dbg(x, cnt, S, local_response);
...
void solve() {
    ll n, m; cin >> n >> m;
    vpl a(n); for(auto& [x, cnt]: a) cin >> x >> cnt;
    dbg(n, m);
    dbg(a);
