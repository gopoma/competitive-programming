...
ll need_mx;
ll need_mn;
ll brute(ll n, const vl a, bool ddd) {
    ll ans = BIG;
    map<vl, bool> vis;
    bool getAnswers = false;
    auto dp = [&](const auto& dp, vl arr) -> void {
        ll local_ans = *max_element(all(arr)) - *min_element(all(arr));
        ckmin(ans, local_ans);
        if(ddd && getAnswers && (local_ans == ans)) {
            chk(*min_element(all(arr)) > 0);
            need_mx = *max_element(all(arr));
            need_mn = *min_element(all(arr));
            dbg(arr);
        }
        ...
        if(*min_element(all(arr)) < -2) return;
...
ll slv(ll n, vl a) {
    ...
    if(n == 1) return 0;
        ...
        ll mx;
        {
            auto check = [&](ll tar_mx) -> ll {
                ll keep = 0;
                each(x, a) {
                    if(x >= tar_mx) keep += x - tar_mx;
                    else {
                        ll take = min(keep, tar_mx - x);
                        keep -= take;
                    }
                }
                return (keep == 0);
...
signed main() {
    ...
    while(0) {
        ...
        vl a(n); each(x, a) x = rng_ll(ll(1), ll(1e12));
        dbg(n);
        //? dbg(a);
        //? ll ans = brute(n, a);
        ll greedy = slv(n, a);
