...
bool vis[2000 + 1][2000 + 1][2];
int memo[2000 + 1][2000 + 1][2];
void solve() {
    int H, W; cin >> H >> W;
    ...
    auto get = [&](int i, int j, bool isFPTurn) -> int {
        if(isFPTurn) return (S[i][j] == '+')? +1 : -1;
        else return (S[i][j] == '+')? -1 : +1;
    };

    for(int i = 0; i <= H; i++) for(int j = 0; j <= W; j++) for(int k = 0; k < 2; k++) vis[i][j][k] = false;
    auto dp = [&](auto&& self, int i, int j, bool isFPTurn) -> int {
        if(i == H - 1 && j == W - 1)
            return 0;
        if(vis[i][j][isFPTurn]) return memo[i][j][isFPTurn];
        vis[i][j][isFPTurn] = true;

        int ans = INF;
        if(isFPTurn) {
            ans = -INF;
            if(check(i + 1, j)) ckmax(ans, self(self, i + 1, j, !isFPTurn) + get(i + 1, j, true));
            if(check(i, j + 1)) ckmax(ans, self(self, i, j + 1, !isFPTurn) + get(i, j + 1, true));
        } else {
            ans = INF;
            if(check(i + 1, j)) ckmin(ans, self(self, i + 1, j, !isFPTurn) + get(i + 1, j, false));
            if(check(i, j + 1)) ckmin(ans, self(self, i, j + 1, !isFPTurn) + get(i, j + 1, false));
        }
        return memo[i][j][isFPTurn] = ans;
    };
    int ans = dp(dp, 0, 0, true);
    dbg(ans);
