 ...
 */
 ...
 tcT> struct SegTree { // cmb(ID,b) = b
    ...
    T ID{1LL}; T cmb(T a, T b) { return a * b; }
	int n; V<T> seg;
...
void solve() {
    ...
    Info res = make_tuple(pref[n - 1], 1, 1);
    for(ll i = 0; i < sz(b); i++) {
        for(ll j = i; j < sz(b); j++) {
            const ll l = b[i];
            const ll r = b[j];
            ll sumLeft = query(0, l - 1);
            ll sumRight = query(r + 1, n - 1);
            ll middle = st.query(l, r);
            ll tot = sumLeft + sumRight + middle;
            ckmax(res, make_tuple(tot, l + 1, r + 1));
        }
    }
    auto [_, l, r] = res;
    assert(l <= r);
