...
void solve() {
    ...
    auto dp = [&](auto&& dp, int i, int carry) -> mi {
        if(i == n - 1 && carry == 0) {
            return mi(0);
        }
        if(i == n) {
        ...
        int nxt_carry = (int(S[i] - '0') + carry) / 2;
        int cur = (int(S[i] - '0') + carry) % 2;

        // floor
        re += (dp(dp, i + 1, nxt_carry) + mi(1)) / mi(2);

        // ceil
        re += (dp(dp, i + 1, nxt_carry + (cur == 1)) + mi(1)) / mi(2);

        return memo[i][carry] = re;
    };
