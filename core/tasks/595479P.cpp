...
void solve() {
    const int n = 4;
    V<str> stms(n);
    for(auto& x: stms) cin >> x;

    V<int> good;
    for(int i = 0; i < n; i++) {
        bool ok1 = true;
        bool ok2 = true;
        int current_length = sz(stms[i]) - 2;
        for(int j = 0; j < n; j++) {
            if(i == j) continue;
            int comparison_length = sz(stms[j]) - 2;
            ok1 &= (2 * current_length <= comparison_length);
            ok2 &= (current_length >= 2 * comparison_length);
        }
        if(ok1 || ok2) good.eb(i);
    }

    for(auto& ss: stms) DBG2(ss, sz(ss) - 2);
    for(auto& x: good) DBG(x);
    if(sz(good) == 1) cout << char('A' + good[0]) << n_l;
    else cout << "C\n";
}
