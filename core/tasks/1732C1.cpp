...
void brute(ll n, vl a) {
    for(int L = 0; L < n; L++) {
        ...
        // dbg(L, partial);
        chk(is_sorted(all(partial)));
    }
}

using Info = tuple<ll, ll, ll, ll>;
void slv(ll n, vl a) {
    vl pref_sum, pref_xor;
    pref_sum = pref_xor = a;
    for(int i = 1; i < n; i++) {
    ...
    auto query_sum = [&](ll L, ll R) -> ll {
        ll sum = pref_sum[R];
        if(0 <= L - 1) sum -= pref_sum[L - 1];
        return sum;
    };
    auto query_xor = [&](ll L, ll R) -> ll {
        ll sum = pref_xor[R];
        if(0 <= L - 1) sum ^= pref_xor[L - 1];
        return sum;
    };

    Info ans = make_tuple(-BIG, -BIG, -BIG, -BIG);
    for(ll L = 0; L < n; L++) {
        ll vmax = query_sum(L, n - 1) - query_xor(L, n - 1);
        ll left = L - 1; // always bad
        ll right = n - 1; // always good
        while(left + 1 < right) {
            ll middle = fdiv(left + right, 2LL);
            if((query_sum(L, middle) - query_xor(L, middle)) == vmax) right = middle;
            else left = middle;
        }
        ll R = right;
        Info local_ans = make_tuple(vmax, -(R - L + 1), L, R);
        dbg(L, R, vmax, R - L + 1);
        ckmax(ans, local_ans);
    }
    auto [vmax, dmax, L, R] = ans;
    dbg(vmax, dmax, L, R);
    cout << (L + 1) << " " << (R + 1) << "\n";
}
...
int main() {
    ...
    while(0) {
        RAYA;
        ll n = rng_ll(1, 100);
        vl a(n); for(auto& x: a) x = rng_ll(0, 100);
        dbg(n);
        brute(n, a);
    }
