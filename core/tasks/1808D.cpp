...
void slv(ll n, ll k, vl a) {
    ...
    const ll mx = *max_element(all(a)) + 5;
    vector<vector<vector<int>>> wh(2, vector<vector<int>>(mx + 5));
    for(int i = 0; i < n; i++) {
        wh[i & 1][a[i]].eb(i);
    }
    ...
    vl dist(n);
    const ll max_gap = k - 1;
    dist[0] = max_gap;
    for(int i = 1; i < n; i++) {
        dist[i] = max(2LL, dist[i - 1] - 2);
    }
    dbg(dist);
    ...
    for(int i = 0; i < n; i++) {
        auto check = [&](ll middle) -> bool {
            ...
            if(l - half >= 0 && r + half < n) return true;
            return false;
        };

        ll left = -1; // always good
        ll right = ll(1e6); // always bad
        while(left + 1 < right) {
        ...
        const ll l = i + dist[i];
        const ll r = i + dist[i] + 2 * left;

        ll tot = left + 1;
        ll matches = upper_bound(all(wh[i & 1][a[i]]), r) - lower_bound(all(wh[i & 1][a[i]]), l);
        ll bads = tot - matches;
        response += bads;
    }
