...
ll solve(ll n) {
    ...
    else if(n == 3) return lcm(n, n - 1, n - 2);
    else {
        ll val1 = lcm(n, n - 1, n - 2);
        ll val3 = lcm(n - 1, n - 2, n - 3);
        ll val2 = lcm(n, n - 1, n - 3);
...
ll brute(ll n) {
    ...
    for(ll i = 1; i <= n; i++) {
        for(ll j = 1; j <= n; j++) {
            for(ll k = 1; k <= n; k++) {
                if(ckmax(mx, lcm(i, j, k))) {
                    who = make_tuple(i, j, k);
                }
            }
        }
    }
