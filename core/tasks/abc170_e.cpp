...
void solve() {
    int n, q; cin >> n >> q;
    vl ratings(n); // infant rating
    vl where(n);   // infant kindergarden id
    for(int i = 0; i < n; i++) {
        ll rating, kid; cin >> rating >> kid; kid--;
        ratings[i] = rating;
        where[i] = kid;
    }
    vpl queries(q);
    for(auto& [iid, kid]: queries) { cin >> iid >> kid; iid--; kid--; }

    multiset<int> kindergardens[int(2e5) + 5];
    multiset<int> ms;
    auto add = [&](int iid, int kid) -> void {
        where[iid] = kid;
        if(!kindergardens[where[iid]].empty()) safeErase(ms, *kindergardens[where[iid]].rbegin());
        kindergardens[kid].emplace(ratings[iid]);
        ms.emplace(*kindergardens[where[iid]].rbegin());

    };
    auto remove = [&](int iid) -> void {
        safeErase(ms, *kindergardens[where[iid]].rbegin());
        safeErase(kindergardens[where[iid]], ratings[iid]);
        if(!kindergardens[where[iid]].empty()) ms.emplace(*kindergardens[where[iid]].rbegin());
    };
