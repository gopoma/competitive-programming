...
ll solve(ll n, ll d, vl& x) {
    auto work = [&](ll left, ll right) {
        ll length = right - left + 1;
        if(length <= 2) return 0LL;
        else {
            ll go = length - 2;
            return fdiv(go * (go + 1), 2LL);
        }
    };

    ll ans = 0;
    for(ll left = 0, right = 0; right < n; right++) {
        while(left < right && x[right] - x[left] > d) {
            left++;
        }
...
ll brute(ll n, ll d, vl& x) {
    ll ans = 0;
    for(int i = 0; i < n; i++) {
        for(int j = i + 1; j < n; j++) {
            for(int k = j + 1; k < n; k++) {
                ll mn = min({x[i], x[j], x[k]});
                ll mx = max({x[i], x[j], x[k]});
...
//?    while(1) {
//?        const ll n = rng_ll(1, 10);
//?        const ll d = rng_ll(1, 1e3);
//?
//?        vl x;
//?        for(int i = 0; i < n; i++) x.eb(rng_ll(1, 1e3));
//?        sor(x);
//?
//?        ll ans1 = brute(n, d, x);
//?        ll ans2 = solve(n, d, x);
