...
bool check(vector<int>& p) {
    ...
    bool ok = true;
    for(int i = 0; i < n; i++) ok &= (p[i] != (i + 1));
    for(int i = 0; i < n; i++) {
        V<int> nn;
        if(i - 1 >= 0) nn.eb(p[i - 1]);
        if(i + 1 < n) nn.eb(p[i + 1]);

        bool local_ok = false;
        for(auto& e: nn) local_ok |= (abs(p[i] - e) == 1);
        ok &= (local_ok);
    }
    return ok;
}
...
void solve() {
    ...
    V<int> res;
    if(n % 2 == 0) {
        for(int i = n; i >= 1; i--) res.eb(i);
    } else {
        const int until = ((n - 1) / 2 - 1) + 3;
        for(int i = n; i >= until; i--) res.eb(i);
        for(int i = 1; i < until; i++) res.eb(i);
    }
