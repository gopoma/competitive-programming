...
void solve() {
    ...
    auto check = [&](int R) {
        const int MAX_DIGITS = sz(ts(R));
        vi digits_R;
        for(auto& c: ts(R)) digits_R.eb(c - '0');

        const int leading_zero = 10;
        int memo[MAX_DIGITS][11][2][11];
            ...
            if(last == leading_zero) {
                ...
                if(0 < idx) {
                    ...
                    for(int d = 1; d <= digits_R[idx]; d++) {
                        ans += dp(idx + 1, d, d < digits_R[idx], sum + d);
                    }
                }
            } else {
                if(reached_lower) {
