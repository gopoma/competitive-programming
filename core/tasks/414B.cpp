...
void solve() {
    ...
    vector<vb> vis(k + 1, vb(n + 1, false));
    vector<vmi> dp(k + 1, vmi(n + 1));
    function<mi(int, int)> solve = [&](int current_length, int current_num) {
        if(current_length == k) return mi(1);

        if(vis[current_length][current_num]) return dp[current_length][current_num];
        vis[current_length][current_num] = true;

        mi partial_res = 0;
        for(int x = current_num; x <= n; x += current_num) {
            partial_res += solve(current_length + 1, x);
        }

        return dp[current_length][current_num] = partial_res;
    };
