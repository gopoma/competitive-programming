...
void solve() {
    ...
    const int mxTake = m / 2;
    vvi b;
    for(int row = 0; row < n; row++) {
        dbg(row);

        for(int i = m; i >= 0; i--) {
            for(int cnt = 0; cnt <= mxTake; cnt++) {
                for(int mod = 0; mod < k; mod++) {
                    if(i == m) {
                        ...
                        // take
                        if(cnt + 1 <= mxTake) {
                            ckmax(dp[i][cnt][mod], dp[i + 1][cnt + 1][(mod + a[row][i]) % k] + a[row][i]);
                        }

                        // skip
                        ckmax(dp[i][cnt][mod], dp[i + 1][cnt][mod]);
                    }
                }
            }
        }
    ...
    vector vis(n, vector<bool>(k));
    vector memo(n, vector<int>(k));
    auto dp2 = [&](auto&& self, int i, int mod) -> int {
        if(i == n) {
            if(mod == 0) return 0;
            return -INF;
        }
        ...
        int re = -INF;
        for(int adi = 0; adi < k; adi++) {
            ckmax(re, self(self, i + 1, (mod + adi) % k) + b[i][adi]);
        }
        return memo[i][mod] = re;
    };

    int response = dp2(dp2, 0, 0);
    cout << response << "\n";
}
