...
void slv(ll n, vl a, vl b) {
    ...
    if((n % 2 == 0) && !fixedPoints.empty()) {
        cout << "-1\n";
        return;
    }
    if((n % 2 == 1) && ((sz(fixedPoints)) != 1)) {
        cout << "-1\n";
        return;
    }
    ...
    if(n % 2 == 1) {
        ll should = fdiv(n, 2LL);
        if(fixedPoints.ft != should) {
            dbg("fixing...");
            response.eb(should, fixedPoints.ft);
            swap(a[should], a[fixedPoints.ft]);
            swap(b[should], b[fixedPoints.ft]);
        }
    }
    ...
    while(left < right) {
        //* have: a[left], b[left]
        //* need: b[left], a[left]
        pl need = mp(b[left], a[left]);
        //* dbg(left, right, need, where);
        if(mp(a[right], b[right]) == need) {}
        else {
            if(where.count(need)) {
                ll i = right;
                ll j = where[need];
                if(i != j) {
                    response.eb(i, j);
                    swap(where[mp(a[i], b[i])], where[mp(a[j], b[j])]);
                    safeErase(where, mp(a[left], b[left]));
                    swap(a[i], a[j]);
                    swap(b[i], b[j]);
                    safeErase(where, mp(a[right], b[right]));
                }
            } else {
                cout << "-1\n";
                return;
...
int main() {
    ...
    while(0) {
        ...
        dbg("/Greedy");
        ll n = rng_ll(1, 25);
        vl a(n), b(n); iota(all(a), 1); iota(all(b), 1);
        shuf(a); shuf(b);
        dbg(n);
        dbg(a);
        dbg(b);
