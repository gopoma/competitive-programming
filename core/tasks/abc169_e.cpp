...
ll brute(ll n, vpl intervals) {
    const ll id_mid = fdiv(n - 1, 2LL);
    const ll id_mid2 = cdiv(n - 1, 2LL);
    ...
    auto dfs = [&](auto&& self, int i) -> void {
        if(i == n) {
            vl temp = cur;
            sor(temp);
            S.emplace(temp[id_mid] + temp[id_mid2]);
            return;
        }

        for(ll x = intervals[i].f; x <= intervals[i].s; x++) {
            cur.eb(x);
            self(self, i + 1);
            cur.pop_back();
        }
...
ll slv(ll n, vpl intervals) {
    //* return brute(n, intervals);
    vl left, right;
    ...
    if(n & 1LL) {
        chk(id_mid == id_mid2);
        const ll L = left[id_mid];
        const ll R = right[id_mid];
        dbg(L, R);
        ll response = R - L + 1LL;
        return response;
    } else {
        const ll L = left[id_mid];
        const ll R = right[id_mid];
        const ll L2 = left[id_mid2];
        const ll R2 = right[id_mid2];

        ll response = ((R + R2) - (L + L2) + 1LL);
        return response;
    }
}
...
int main() {
    ...
    while(0) {
        ...
        dbg(n);
        dbg(intervals);
        ll ans = brute(n, intervals);
        ll greedy = slv(n, intervals);
        dbg(ans, greedy);
        chk(ans == greedy);
    }
