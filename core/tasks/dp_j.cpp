...
bool vis[300 + 5][300 + 5][300 + 5];
db  memo[300 + 5][300 + 5][300 + 5];
void solve() {
    int n; cin >> n;
    vi a(n); for(auto& x: a) cin >> x;
    vi hist(4); for(auto& x: a) hist[x]++;

    {
        int mx = *max_element(all(hist));
        for(int i = 0; i <= mx; i++)
            for(int j = 0; j <= mx; j++)
                for(int k = 0; k <= mx; k++)
                    vis[i][j][k] = false;
    ...
    auto dp = [&](auto&& self, int x, int y, int z) -> db {
        if(x == 0 && y == 0 && z == 0) return db(0);
        if(x < 0 || y < 0 || z < 0) return db(0);

        if(vis[x][y][z]) return memo[x][y][z];
        vis[x][y][z] = true;

        db X = (db(x) / db(n)) * (self(self, x - 1, y,     z)     + db(1));
        db Y = (db(y) / db(n)) * (self(self, x + 1, y - 1, z) + db(1));
        db Z = (db(z) / db(n)) * (self(self, x,     y + 1, z - 1) + db(1));
        db S = X + Y + Z;

        db v = (db(n - (x + y + z)) / db(n));

        db ans = (v + S) / (db(1) - v);
        return memo[x][y][z] = ans;
    };

    db response = dp(dp, hist[1], hist[2], hist[3]);
    cout << response << "\n";
}
