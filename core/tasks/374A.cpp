...
void solve() {
    ...
    bool ok = false;
    vpl corners = {mp(1, m), mp(n, 1), mp(n, m), mp(1, 1)};
    ll res = BIG;
    for(auto& corner: corners) {
        ll offset_x = abs(corner.f - i);
        ll offset_y = abs(corner.s - j);
        if(!((offset_x % a == 0) && (offset_y % b == 0))) continue;
        if(((offset_x / a) % 2) != ((offset_y / b) % 2)) continue;
        if(offset_x == 0 && offset_y != 0 && !(1 <= (corner.f - a) || (corner.f + a) <= n)) continue;
        if(offset_x != 0 && offset_y == 0 && !(1 <= (corner.s - b) || (corner.s + b) <= m)) continue;

        ckmin(res, max(offset_x / a, offset_y / b));
        ok = true;
    }

    if(ok) cout << res << n_l;
    else cout << "Poor Inna and pony!" << n_l;
    RAYA;
    RAYA;
    RAYA;
}
