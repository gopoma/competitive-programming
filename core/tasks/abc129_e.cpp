...
bool vis[100001 + 1][2 + 1];
mi  memo[100001 + 1][2 + 1];
void solve() {
    str S; cin >> S;
    ...
    auto dp = [&](auto&& self, int i, bool reached_lower) -> mi {
        if(i == n) return mi(1);

        if(vis[i][reached_lower]) return memo[i][reached_lower];
        vis[i][reached_lower] = true;

        mi ans = 0;
        for(int d = 0; d <= (reached_lower? 1 : digits[i]); d++) {
            if(d == 0) {
                ans += self(self, i + 1, reached_lower || (d < digits[i]));
            } else {
                assert(d == 1);
                ans += mi(2) * self(self, i + 1, reached_lower || (d < digits[i]));
            }
        }

        return memo[i][reached_lower] = ans;
    };
    mi response = dp(dp, 0, false);
    cout << response << "\n";
}
