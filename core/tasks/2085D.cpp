...
ll brute(int n, int k, vl a) {
    map<int, map<int, bool>> vis;
    map<int, map<int, ll>> memo;
    auto dp = [&](auto&& self, int i, int j) -> ll {
        ll right = max(0, (n - 1) - (i + 1) + 1);
        if(j + k > right) return 0;
        ...
        ll res = -BIG;
        //* don't take
        ckmax(res, self(self, i + 1, j));
        //* take
        if(j + k <= right) {
            ckmax(res, self(self, i + 1, j + k) + a[i]);
        }
        //* reduce
        if(j >= 1) {
            ckmax(res, self(self, i + 1, j - 1));
        }
...
int main() {
    ...
    while(0) {
        RAYA;
        int k = rng_int(1, int(100));
        int n = rng_int(k, int(100));
