...
struct node {
	...
	node operator + (const node &rhs) const {
        ...
        dbg(L, R, rhs.L, rhs.R, val, rhs.val);
        dbg(nxt.L, nxt.R, nxt.val);
...
void solve() {
    ...
    for(auto& [k, x]: que) {
        ...
        dbg(k, x);
        a[k] = x;
        st.upd(k, node(k, k, a[k]));

        dbg("query");
        node response = st.query(0, n - 1);
        dbg(a);
        dbg(response.L, response.R, response.val);
        dbg("/query");
    }
}
