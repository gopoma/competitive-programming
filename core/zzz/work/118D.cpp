...
bool vis[200][3][11][101][101];
mi   dp[200][3][11][101][101];
...
void solve() {
    ...
    //? last -> '1' | '2'
    function<mi(int, int, int, int, int)> solve = [&](int idx, int last, int count, int tot_1, int tot_2) {
        if(last == 0 && count > k1) return mi(0);
        if(last == 1 && count > k2) return mi(0);
        if(idx == n1 + n2)          return mi(1);

        if(vis[idx][last][count][tot_1][tot_2]) return dp[idx][last][count][tot_1][tot_2];
        vis[idx][last][count][tot_1][tot_2] = true;

        mi res = 0;
        //? choose '1'
        if(tot_1 < n1) {
            if(last == 0) res += solve(idx + 1,   0, count + 1, tot_1 + 1, tot_2);
            else            res += solve(idx + 1, 0, 1, tot_1 + 1, tot_2);
        }

        //? choose '2'
        if(tot_2 < n2) {
            if(last == 1) res += solve(idx + 1, 1, count + 1, tot_1, tot_2 + 1);
            else            res += solve(idx + 1, 1, 1, tot_1, tot_2 + 1);
        }

        return dp[idx][last][count][tot_1][tot_2] = res;
    };

    mi ans = solve(0, 3, 0, 0, 0);
    ps(ans);
}
