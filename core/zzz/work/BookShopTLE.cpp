...
void solve() {
    ...
    const int OFFSET = 1;
    vector<vb> vis(n + OFFSET, vb(x + OFFSET, false));
    vector<vi> dp(n + OFFSET, vi(x + OFFSET));
    int ans = y_combinator([&](auto solve, int idx, int go) -> int {
        if(idx == n) return 0;

        if(vis[idx][go]) return dp[idx][go];
        vis[idx][go] = true;

        int res = solve(idx + 1, go);
        if(go - h[idx] >= 0)
            ckmax(res, solve(idx + 1, go - h[idx]) + s[idx]);

        return dp[idx][go] = res;
	})(0, x);
