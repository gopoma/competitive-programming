...
void solve() {
    ...
    vi values;
    each(x, a) values.eb(x);
    for(auto& [L, R, x]: que) values.eb(x);
    int MAXN = -12;
    { //? Coordinate Compression
        ...
        each(x, a) x = forwards[x];
        each(x, que) {
            get<0>(x)--;
            get<1>(x)--;
            get<2>(x) = forwards[get<2>(x)];
        }
        MAXN = m + 5;
    }
    chk(MAXN != -12);
    ...
    each(x, queries) dbg(x.l, x.r, x.x, x.idx);
    dbg(MAXN);
	...
	for (int i = 0; i < q; i++) {
		...
		ans[queries[i].idx] = (hist[x] > 0);
	}
    dbg(ans);
    str result;
    for(int i = 0; i < q; i++) result.pb(ans[i]?'1':'0');
    dbg(result);
    ps(result);
}
