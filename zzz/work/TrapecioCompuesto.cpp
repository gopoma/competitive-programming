...
void solve() {
    if(isTabular) {
        ...
        auto f = [&](db x) {
            ...
            //?if(x == 0.0)      return 0.0;
            //?else if(x == 0.1) return 6.84;
            //?else if(x == 0.3) return 4.0;
            //?else if(x == 0.5) return 4.2;
            //?else if(x == 0.7) return 5.51;
            //?else if(x == 0.95) return 5.77;
            //?else if(x == 1.2) return 1.0;
            //?else assert(false);

            if(eq(x, -1.0)) return 2.0;
            else if(eq(x, -0.5)) return -3.0;
            else if(eq(x, 0.0)) return 1.5;
            else if(eq(x, 1.0)) return -1.0;
            else if(eq(x, 1.75)) return 0.5;
            else if(eq(x, 2.5)) return 0.75;
            else if(eq(x, 3.25)) return -2.0;
            else assert(false);
        };

        const db a = 0.0;   //! init   [xo]
        const db b = 1.0; //! finish [xf]
        assert(b > a);

        const int n = 1;  //! ¿Cuántos intervalos?
        assert(n % 1 == 0);
