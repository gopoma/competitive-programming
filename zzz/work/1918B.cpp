...
ll brute(vl& a, vl& b) {
    ...
    vi go(n); for(int i = 0; i < n; i++) go[i] = i;
...
void solve() {
    ...
    sor(atmp); for(int i = 0; i < n; i++) bgo.eb(b[atmp[i].s]);
    sor(btmp); for(int i = 0; i < n; i++) ago.eb(a[btmp[i].s]);
    sor(a);
    sor(b);

    ll xd1 = work(a) + work(bgo);
    ll xd2 = work(ago) + work(b);

    ll mn = min(xd1, xd2);
    //?assert(mn == brute(arespaldo, brespaldo));
