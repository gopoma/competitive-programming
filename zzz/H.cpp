...
bool ok(pi pos) {
    return ok(pos.first, pos.second);
}
...
void solve() {
    ...
    // Rook
    if(from.first == to.first || from.second == to.second) cout << "1 ";
    else cout << "2 ";

    // Bishop
    if(((from.first + from.second) % 2) != ((to.first + to.second) % 2)) cout << "0 ";
    else {
        bool just = false;
    ...
    // King
    cout << max(abs(from.first - to.first), abs(from.second - to.second)) << "\n";
