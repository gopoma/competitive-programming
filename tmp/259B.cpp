...
void solve() {
    ...
    auto in_range = [](int x) {
        return (1 <= x && x <= int(1e5));
    };

    auto ok = [&]() {
        set<int> sums;
        sums.emplace(A[0][0]+A[0][1]+A[0][2]);
        sums.emplace(A[1][0]+A[1][1]+A[1][2]);
        sums.emplace(A[2][0]+A[2][1]+A[2][2]);

        sums.emplace(A[0][0]+A[1][0]+A[2][0]);
        sums.emplace(A[0][1]+A[1][1]+A[2][1]);
        sums.emplace(A[0][2]+A[1][2]+A[2][2]);

        sums.emplace(A[0][0]+A[1][1]+A[2][2]);
        sums.emplace(A[2][0]+A[1][1]+A[0][2]);
    ...
    for(int x = 1; x <= int(1e5); x++) {
        ...
        bool in = in_range(A[0][0]) && in_range(A[1][1]) && in_range(A[2][2]);
        bool lok = in && ok();
        if(lok) {
            for(int i = 0; i < n; i++) {
                for(int j = 0; j < n; j++) {
