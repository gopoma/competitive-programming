...
void solve() {
    ...
    const int OFFSET = 12;
    V<vl> dp(n + OFFSET, vl(2));
    V<vb> vis(n + OFFSET, vb(2, false));
    function<ll(ll, ll)> solve = [&](ll pos, ll jumped) {
        if(pos == n) return 0LL;

        if(vis[pos][jumped]) return dp[pos][jumped];
        vis[pos][jumped] = true;

        auto k = where[a[pos]].upper_bound(pos);
        bool go = false;
        ll nxt;
        if(k != where[a[pos]].end()) {
            go = true;
            nxt = *k;
        }

        ll res = solve(pos + 1, 0);
        if(go) {
            if(jumped) ckmax(res, solve(nxt, 1) + nxt - pos);
            else ckmax(res, solve(nxt, 1) + nxt - pos + 1);
        }
        return dp[pos][jumped] = res;
    };
