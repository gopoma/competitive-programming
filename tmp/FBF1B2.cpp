#pragma GCC optimize(3,"Ofast","inline")
#pragma GCC target("sse,sse2,sse3,ssse3,sse4,popcnt,abm,mmx,avx,avx2")

#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>
#include <ext/pb_ds/tree_policy.hpp>

using namespace std;

#define MACRO(code) do {code} while (false)
#define DBG(x) MACRO(cerr << #x << " = " << (x) << endl;)
#define DBGY(x) MACRO(cerr << #x << " = " << (x) << " , ";)
#define DBG2(x,y) MACRO(DBGY(x); DBG(y);)
#define DBG3(x,y,z) MACRO(DBGY(x); DBGY(y); DBG(z);)
#define DBG4(x,y,z,w) MACRO(DBGY(x); DBGY(y); DBGY(z); DBG(w);)
#define RAYA MACRO(cerr << " ================ " << endl;)

#define tcT template<class T

using ll = long long;
using ld = long double;       // or double if tight TL
using u128 = __uint128_t;
using str = string;         // yay python!
tcT> using V = vector<T>;
tcT, size_t SZ> using AR = array<T, SZ>;

#define mp  make_pair
#define F   first
#define S   second

#define sz(x)   int((x).size())
#define bg(x)   begin(x)
#define all(x)  bg(x), end(x)
#define rall(x) x.rbegin(), x.rend()
#define sor(x)  sort(all(x))
#define rsz     resize
#define ins     insert
#define pb      push_back
#define eb      emplace_back
#define ft      front()
#define bk      back()

#define lb lower_bound
#define ub upper_bound

const int MOD = (int)1e9 + 7;   // 998244353;
const ld PI = acos((ld)-1);
mt19937 rng(0); // or mt19937_64
const int dx[4]{1, 0, -1, 0}, dy[4]{0, 1, 0, -1};  // for every grid problem!!
const char n_l = '\n';

template <typename T>
ostream& operator <<(ostream &os, const vector<T>& v) {
    os << "[";

    for(int i = 0; i < sz(v); i++) {
        if (i > 0) os << " ";
        os << v[i];
    }
    return os << "]";
}

bool is_prime(ll x) {
    for (ll d = 2LL; d * d <= x; d++) {
        if (x % d == 0)
            return false;
    }
    return x >= 2LL;
}

ll eks;

// Function to find all factor combinations of a number
void backtrack(ll start, ll target, vector<ll>& factors,
               vector<vector<ll> >& combinations, ll n)
{
    // Base case: if target is 1, we have found a
    // factorization
    if (target == 1LL) {
        // Add a copy of the factors vector to combinations,
        // except if the vector contains only one factor
        // equal to n
        if (factors.size() > 1LL || factors[0] != n) {
            combinations.push_back(factors);
        }
        return;
    }

    // Try all factors from start to target, inclusive
    for (ll i = start; ((target == eks)? (i * i) : i) <= target; i++) {
        if (target % i == 0LL) {
            if(i > 41LL) return;
            factors.push_back(i); // Add i to factors
            backtrack(i, target / i, factors, combinations,
                      n); // Recursively find factors of
                          // target / i
            factors.pop_back(); // Remove i from factors
        }
    }
}

// Function to call backtrack and return the factor
// combinations
vector<vector<ll>> factorCombinations(ll n)
{
    vector<vector<ll> > combinations;
    vector<ll> factors;
    backtrack(2LL, n, factors, combinations, n);
    return combinations;
}

void solve(ll i) {
    // RAYA;

    const ll k = 41; // don't move!
    ll n; cin >> n;
    eks = n;

    vector<ll> ans;
    bool ok = false;
    int minSize = INT_MAX;
    if(n == 1) {
        ok = true;
        for(int _ = 0; _ < k; _++) {
            ans.emplace_back(1);
        }
    } else if(is_prime(n)) {
        ll ktmp = k;
        if(n <= ktmp) {
            ok = true;
            ans.emplace_back(n);
            ktmp -= n;

            while(ktmp--) {
                ans.emplace_back(1LL);
            }
        }
    } else {
        vector<vector<ll>> combinations = factorCombinations(n);
        vector<ll> akx = {n};
        combinations.emplace_back(akx);
        for(auto& v: combinations) {
            ll xd = 1LL;
            for(auto& e: v) {
                xd *= e;
            }
            if(xd != n) continue;


            // DBG(v);
            ll sum = 0;
            for(auto& e: v) {
                sum += e;
            }

            if(sum <= k) {
                ok = true;
                vector<ll> partial;
                for(auto& e: v) {
                    partial.emplace_back(e);
                }
                ll ktmp = k;
                ktmp -= sum;

                while(ktmp--) {
                    partial.emplace_back(1);
                }


                // ll sum = 0LL;
                // for(auto& e: partial) {
                //     sum += e;
                // }
                // ll kxsum = sum;
                // assert(kxsum == 41LL);


                // DBG(partial);
                if(sz(partial) < minSize) {
                    minSize = sz(partial);
                    ans = partial;
                }
            }
        }
    }

    if(!ok) {
        assert(sz(ans) == 0);
        ans.emplace_back(-1);
    } else {
        sor(ans);
    }

    if(ans[0] != -1LL) {
        ll sum = 0LL;
        for(auto& e: ans) {
            sum += e;
        }
        assert(sum == 41LL);
    }

    cout << "Case #" << i << ": ";
    if(ans[0] == -1LL) {
        cout << "-1\n";
    } else {
        cout << sz(ans) << " ";
        for(int xd = 0; xd < sz(ans); xd++) {
            cout << ans[xd] << " \n"[xd == sz(ans) - 1];
        }
    }
}

int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    ll T = 1LL;
    cin >> T;

    for(ll i = 1LL; i <= T; i++) {
        solve(i);
    }

    return 0;
}
