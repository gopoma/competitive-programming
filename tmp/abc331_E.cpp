...
void solve() {
    ...
    vpl side_dishes(M);
    for(int i = 0; i < M; i++) side_dishes[i] = mp(b[i], i);
    sort(rall(side_dishes));

    ll global_ans = -BIG;
    for(int i = 0; i < N; i++) {
        ll local_ans = -BIG;
        for(int j = 0; j < M; j++) {
            if(!banned.count(mp(i, side_dishes[j].s))) {
                local_ans = a[i] + side_dishes[j].f;
                dbg(a[i], b[side_dishes[j].s]);
                break;
            }
        }
        ckmax(global_ans, local_ans);
    }
    cout << global_ans << n_l;
}
