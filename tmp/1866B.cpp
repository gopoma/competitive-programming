...
// here goes the work!
template<>
i64 MLong<0LL>::Mod = 998244353LL;

constexpr i64 P = 998244353LL;
using Z = MLong<P>;
...
void solve() {
    ...
    // DBG("X");
    // for(auto& it: X) DBG2(it.first, it.second);
    // DBG("Y");
    // for(auto& it: Y) DBG2(it.first, it.second);

    // check if Y is a subset of X and each exponent is lower than or equal
    bool ok = true;
    for(auto& it: Y) {
        ok &= ((X.find(it.first) != X.end()) && (it.second <= X[it.first]));
    }
