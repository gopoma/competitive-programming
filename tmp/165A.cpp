...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < n; j++) {
            ...
            left |=  (a[j].x < a[i].x && a[j].y == a[i].y);
            right |= (a[j].x > a[i].x && a[j].y == a[i].y);
            lower |= (a[j].x == a[i].x && a[j].y < a[i].y);
            upper |= (a[j].x == a[i].x && a[j].y > a[i].y);
        }
        bool ok = false;
        ok = (left && right && lower && upper);
        if(ok) DBG2(a[i].x, a[i].y);
        ans += ok;
    }
    cout << ans << n_l;
}
