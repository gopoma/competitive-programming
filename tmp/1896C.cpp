...
void brute() {
    ...
    do {
        int beaty = 0;
        for(int i = 0; i < n; i++) beaty += (a[i] > b[i]);
    ...
    int goal_mx = INT_MIN;
    for(auto& it: aux) {
        ckmin(goal_mn, it.first);
        ckmax(goal_mx, it.first);
    }
    ...
    int mn = n;
    int mx = 0;
    multiset<int> aa; for(auto& x: a) aa.emplace(x);
    multiset<int> bb; for(auto& x: b) bb.emplace(x);

    //? mn
    for(int i = 0; i < n; i++) {
        auto k = bb.lower_bound(a[i]);
        if(k != bb.end()) {
            erase(bb, *k);
            mn--;
        }
...
pair<ll, ll> work(vector<ll>& a, vector<ll>& b) {
    chk(sz(a) == sz(b));
    const int n = sz(a);
    ll mn = n;
    ll mx = 0;

    multiset<ll> aa; for(auto& x: a) aa.emplace(x);
    multiset<ll> bb; for(auto& x: b) bb.emplace(x);
