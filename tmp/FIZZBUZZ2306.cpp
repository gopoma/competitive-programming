...
void solve() {
    ...
    auto ok = [&](ll row, ll column) {
        return (1LL <= row && row <= N) && (1LL <= column && column <= M);
    };

    ll ans = 0LL;
    if(ok(x - 1LL, y)) ans = max(ans, (x - 1LL) * M);
    if(ok(x + 1LL, y)) ans = max(ans, (N - x) * M);
    if(ok(x, y - 1LL)) ans = max(ans, N * (y - 1LL));
    if(ok(x, y + 1LL)) ans = max(ans, N * (M - y));
