...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        int left = i; // is always good
        int right = n; // is always bad
        ...
        while(left + 1 < right) {
            ...
            // DBGY(right);
            // DBGY(middle);
            // DBG(query(pref, fixedLeft, middle));
            if(query(pref, fixedLeft, middle) <= t) {
                left = middle;
            } else {
                right = middle;
            }
        ...
        // DBG4(i, fixedLeft, left, right);
        // RAYA;
        ans = max(ans, left - fixedLeft + 1);
    }
    cout << ans;
}
