...
void solve() {
    ...
    if(idx_last_good == -1) cout << "0\n";
    else {
        dbg(idx_last_good);
        bool firstHalf = is_sorted(a.begin(), a.begin() + idx_last_good);
        bool secondHalf = is_sorted(a.begin() + idx_last_good + 1, a.end());
        bool ok = a.back() <= a.front();
        if(firstHalf && secondHalf && ok) cout << (n - idx_last_good - 1) << n_l;
        else cout << "-1\n";
    }
}
