...
void solve() {
    ...
    ll left = 0; // is always bad
    ll right = BIG; // is always good

    while(left + 1LL < right) {
        ll middle = (left + right) / 2LL;
        ll n = middle;

        ll no_x_pero_si_y = (n / y) - (n / lcm(x, y));
        ll no_y_pero_si_x = (n / x) - (n / lcm(x, y));
        ll ni_x_ni_y = n - ((n / x) + (n / y) - (n / lcm(x, y)));

        ll need_x = max(cnt1 - no_x_pero_si_y, 0LL);
        ll need_y = max(cnt2 - no_y_pero_si_x, 0LL);


        bool ok = (need_x + need_y <= ni_x_ni_y);
        dbg(left, right, middle, need_x, need_y, ni_x_ni_y, cnt1, cnt2, ok);
        if(ok) {
            right = middle;
        } else {
            left = middle;
