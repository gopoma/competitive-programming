...
void solve() {
    ...
    auto work = [&](ll x) -> ll {
        return fdiv((x - 1LL) * x, 2LL);
    };

    for(int u = 0; u < n; u++) dbg(u + 1, subtree_size[u], parent[u] + 1);
    ll response = work(n);
    for(auto& [x, y]: G.bridges) {
        ll u = x;
        ll v = y;
        if(u != parent[v]) swap(u, v);
        chk(u == parent[v]);
        dbg(u + 1, v + 1, subtree_size[u], subtree_size[v]);
        ll A = subtree_size[v];
        ll B = n - A;
        ckmin(response, work(A) + work(B));
    }
    dbg(response);
    cout << response << "\n";
}
