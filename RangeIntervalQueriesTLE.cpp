...
struct node {
	...
	node operator + (const node &rhs) const {
        ...
        const int n = sz(block);
        const int m = sz(rhs.block);
        int i, j;
        i = j = 0;

        while(i < n && j < m) {
            if(block[i] < rhs.block[j]) {
                new_node.block.emplace_back(block[i]);
                i++;
            } else {
                new_node.block.emplace_back(rhs.block[j]);
                j++;
            }
        }
...
int work(const vector<int>& vec, int l, int r, int c, int d) {
    if(l > r) return 0;
    if(vec[r] < c || vec[l] > d) return 0;
    ...
    int m = (l + r) >> 1;
    int s1 = work(vec, l, m, c, d);
    int s2 = work(vec, m + 1, r, c, d);
    return s1 + s2;
}
...
struct Response {
    ...
    Response operator + (const node &rhs) const {
        ...
        response.re += work(rhs.block, 0, sz(rhs.block) - 1, c, d);
...
struct SegmentTree {
    ...
    Response query(int l, int r, int c, int d) {
        Response re;
        re.c = c;
        re.d = d;
...
void solve() {
    ...
    int n, q; cin >> n >> q;
    vi values(n); for(auto& x: values) cin >> x;
