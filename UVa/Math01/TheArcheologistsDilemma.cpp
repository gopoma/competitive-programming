...
const db EPS = 1e-9;
void solve(ll N) {
    auto get_amount_digits = [&](db n) -> ll {
        return ll(floorl(db(1) + floorl(n * log10l(db(2)))));
    };
    auto get = [&](db n, db k) -> ll {
        db E = db(n) * log10l(db(2)) - floorl(db(n) * log10l(db(2))) + k - db(1);
        db ans = powl(db(10), E);
        return ll(floorl(ans));
    };
    const ll tar_digits = sz(ts(N));
    //? int counter = 0;
    for(db E = 7; ; E++) {
        //? counter++;
        //? if(counter > 200000) break;
        ll amount_digits = get_amount_digits(E);
        if((amount_digits >= tar_digits) && (amount_digits - tar_digits > tar_digits) && (get(E, tar_digits) == N)) {
            cout << E << "\n";
            return;
        }
    }
...
//?        map<ll, int> ans;
//?        map<ll, str> tar;
//?        for(int xd = 1; xd <= 200; xd++) {
//?            str S = ts(current);
//?            const int N = sz(S);
//?            str pref_str;
//?            for(int i = 0; i < (N - 1) / 2; i++) {
//?                pref_str.pb(S[i]);
//?                ll pref = stoll(pref_str);
//?                if(pref > 2147483648) break;
//?                if(!vis[pref]) {
//?                    vis[pref] = true;
//?                    ans[pref] = xd;
//?                    tar[pref] = S;
