...
void solve() {
    ...
    print(A); cout << n_l;
    print(B); cout << n_l;
    print(lcm(A, B)); cout << n_l;
    u128 sum1 = fdiv(N, A) + fdiv(N, B) + fdiv(N, C);
    u128 sum2 = fdiv(N, lcm(A, B)) + fdiv(N, lcm(A, C)) + fdiv(N, lcm(B, C));
    u128 sum3 = fdiv(N, lcm(A, lcm(B, C)));

    u128 tmp = sum1 - sum2 + sum3;
    ull ans = ull(tmp);
    cout << ans << n_l;
