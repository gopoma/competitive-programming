...
void solve() {
    ...
    auto encrypt = [&](vector<string>& tokens, int shift) {
        for(auto& token: tokens) {
            for(auto& c: token) {
                if(c == ' ') continue;
                // c = char(((int(c - int('a')) + shift) % 26) + int('a'));

                int tmp = int(c - int('a')) - shift;
                if(tmp < 0) {
                    tmp += 26;
                }
                c = char(tmp + int('a'));
    ...
    auto decrypt = [&](vector<string>& tokens, int shift) {
        for(auto& token: tokens) {
            for(auto& c: token) {
                if(c == ' ') continue;
                // int tmp = int(c - int('a')) - shift;
    ...
    for(int i = 0; i < n; i++) {
        ...
        while(true) {
            ...
            message = message.substr(message.find(' ') + 1, sz(message) - message.find(' ') + 1);
        }
        // DBG2(tokens[sz(tokens) - 1], sz(tokens[sz(tokens) - 1]));
        for(auto& token: tokens) {
            to_encode |= (token == "the");
        }
        ...
        if(to_encode) {
            encrypt(tokens, shift);
        } else {
            decrypt(tokens, shift);
        }

        for(int k = 0; k < sz(tokens); k++) {
            cout << tokens[k];
            if(k < sz(tokens) - 1) {
                cout << " ";
            } else {
