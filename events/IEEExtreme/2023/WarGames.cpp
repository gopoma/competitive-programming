...
// here goes the work!
void solve() {
    vector<string> tmpe = {"2","3","4","5","6","7","8","9","T","J","Q","K","A"};
    map<string, int> mappings;
    for(int i = 0; i < sz(tmpe); i++) {
        mappings[tmpe[i]] = i;
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        deque<string> ppp1 = tokenize(pp1);
        deque<string> ppp2 = tokenize(pp2);

        // for(auto& e: ppp1) DBG(e);
        // RAYA;
        // for(auto& e: ppp2) DBG(e);
        // RAYA;

        int count = 1;
        const int upper_bound = 500000;
        bool ok = true;
        bool win1 = false;
        bool win2 = false;
        while(true) {
            // DBG2(ppp1.front(), ppp2.front());

            string play1 = ppp1.front();
            string play2 = ppp2.front();

            if(mappings[play1] > mappings[play2]) {
                // player 1 wins local
                ppp1.pop_front();
                ppp1.emplace_back(play2);
                ppp2.pop_front();
            } else if(mappings[play2] > mappings[play1]) {
                // player 2 wins local
                ppp2.pop_front();
                ppp2.emplace_back(play1);
                ppp1.pop_front();
            } else {
                ppp1.emplace_back(ppp1.front());
                ppp2.emplace_back(ppp2.front());
                ppp1.pop_front();
                ppp2.pop_front();
            }
