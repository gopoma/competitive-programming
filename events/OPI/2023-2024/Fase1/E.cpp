...
void solve() {
    ...
    deque<int> zero_positions, one_positions, two_positions;
    for(int i = 0; i < n; i++) {
        if(S[i] == '0') zero_positions.emplace_back(i);
        else if(S[i] == '1') one_positions.emplace_back(i);
        else if(S[i] == '2') two_positions.emplace_back(i);
        else assert(false);
    }
    assert(sz(zero_positions) == n / 6);

    V<vi> sequences(n/6);
    for(int i = 0; i < n/6; i++) {
        sequences[i].eb(one_positions.front()); one_positions.pop_front();
        sequences[i].eb(one_positions.front()); one_positions.pop_front();
    }

    for(int i = 0; i < n/6; i++) {
        sequences[i].eb(two_positions.front()); two_positions.pop_front();
    }

    for(int i = 0; i < n/6; i++) {
        sequences[i].eb(zero_positions.front()); zero_positions.pop_front();
    }
