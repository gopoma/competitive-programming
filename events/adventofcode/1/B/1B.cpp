    ...
    while(cin >> token) {
        ...
        for(int i = 0; i < sz(stk1); i++) {
            for(auto& [word, num]: nums) {
                if(stk1.substr(i, sz(word)) == word) {
                    stk1.erase(i, sz(word));
                    stk1.insert(i, num);
                }
            }
        }
        for(int i = sz(stk2); i >= 0; i--) {
            for(auto& [word, num]: nums) {
                if(stk2.substr(i, sz(word)) == word) {
                    stk2.erase(i, sz(word));
                    stk2.insert(i, num);
                }
            }
        }
