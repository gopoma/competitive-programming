...
void solve() {
    str hand; ll bid;
    map<char, ll> score;
    vector<char> skeys = {'A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2'};
    reverse(all(skeys));
    ...
    sort(all(aux), [&](const pair<str, ll>& a, const pair<str, ll>& b) {
        map<char, ll> cnt_a, cnt_b;
        for(auto& x: a.f) cnt_a[x]++;
        for(auto& x: b.f) cnt_b[x]++;
        ...
        for(int i = 0; i < sz(a.f); i++) {
            if(a.f[i] != b.f[i]) {
                return scores[a.f[i]] > scores[b.f[i]];
            }
        }
        return a > b;
    });
