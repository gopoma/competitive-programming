...
void solve() {
    ...
    const int THEORETICAL_MAX_DIGITS = 2 * 50;
    const int MAX_DIGITS = 2 * N;

    bool vis[THEORETICAL_MAX_DIGITS][1005];
    memset(vis, false, sizeof(vis));
    bigint memo[THEORETICAL_MAX_DIGITS][1005];

    function<bigint(int, int)> dp = [&](int idx, int sum) -> bigint {
        if(idx == MAX_DIGITS) return bigint(sum == S);

        if(idx == N && sum != fdiv(S, 2LL)) return bigint(0);
