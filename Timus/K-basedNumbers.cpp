...
ll brute(ll N, ll K) {
    ...
    ll cnt = 0;
    for(ll x = binpow(10LL, N - 1); x < binpow(10LL, N); x++) {
        if(check(x, K)) {
            cnt++;
        }
    }
...
bigint slv(ll N, ll K) {
    //? <>
    vector<vector<bigint>> memo = vector<vector<bigint>>(N + 1, vector<bigint>(K + 5, bigint("-1")));
    function<bigint(ll, ll)> dp = [&](ll idx, ll last) -> bigint {
        if(idx == N) return bigint("1");
        if(memo[idx][last] != -1) return memo[idx][last];

        bigint ans = bigint("0");

        for(ll d = 0; d < K; d++) {
            if(d == 0 && (idx == 0 || last == 0)) continue;
            ans += dp(idx + 1, d);
        }
