...
bool isBalanced(str S) {
    ...
    each(c, S) {
        if(c == '(') {
            ...
            if(stk.empty() || stk.bk == '[') return false;
            stk.pop_back();
        } else {
            if(stk.empty() || stk.bk == '(') return false;
            stk.pop_back();
...
int brute(str S) {
    vs xd;
    const int N = sz(S);
    deque<char> current; str can = "[]()";
    function<void()> dfs = [&]() {
        {
            str act;
            each(x, current) act.pb(x);
            xd.eb(act);
        }
        if(sz(current) == 2 * N) return;
    ...
    int mn = int(1e9) + 5;
    each(x, xd) {
        if(isBalanced(x) && issubsequence(S, x)) {
            ckmin(mn, sz(x));
        }
    }
    // each(x, xd) {
    //     if(good(x) && issubsequence(S, x) && sz(x) == mn) {
    //         dbg(x);
    //     }
    // }
    chk(mn < 1000);
...
void slv(str S) {
    ...
    auto dp = [&](auto&& self, int L, int R) -> Info {
        if(L > R) return make_tuple(0, "", "");
        if(vis[L][R]) return memo[L][R];
        vis[L][R] = true;

        Info ans = make_tuple(INF, "", "");
        auto improve = [&](int nxt_l, int nxt_r, str gl, str gr) -> void {
            auto [mn, p, pp] = self(self, nxt_l, nxt_r);
            Info local_ans = make_tuple(mn + 2, gl + p + pp + gr, "");
            ckmin(ans, local_ans);
        };
        ...
        for(int middle = L; middle < R; middle++) {
            auto [mn, p, pp] = self(self, L, middle);
            auto [mn2, p2, pp2] = self(self, middle + 1, R);
            ckmin(ans, make_tuple(mn + mn2, p + pp, p2 + pp2));
        }

        return memo[L][R] = ans;
    };

    auto [_, p, pp] = dp(dp, 0, N - 1);
    str response = p + pp;
    cout << response << "\n";
    ...
    if(false && isDebugging) {
        dbg(response);
        int should = brute(S);
        dbg(sz(response), should);
        chk(should == sz(response));
        chk(issubsequence(S, response));
    }
}
...
int main() {
    ...
    while(0) {
        RAYA;
        const int N = rng_int(1, 5);
        const str alphabet = "()[]";
        str S; rep(N) S.pb(alphabet[rng_int(0, 3)]);
        dbg(N);
        dbg(S);
