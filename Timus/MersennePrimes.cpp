...
void precompute() {
    ...
    //?
    {
        vl keys{18, 32, 24, 21, 19, 34, 27, 33, 20, 30, 28, 29, 22,};
        vl values{3217, 756839, 19937, 9689, 4253, 1257787, 44497, 859433, 4423, 132049, 86243, 110503, 9941,};
        for(int i = 0; i < sz(keys); i++) ans[keys[i]] = values[i];
    }
    //?

    each(x, ans) dbg(x);
    vl R;
    for(ll idx = 1; idx <= 38; idx++) if(!ans.count(idx)) R.eb(idx);
    dbg("Falta", R);
}
