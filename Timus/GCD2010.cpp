...
tcT> struct SegTree { // cmb(ID,b) = b
    ...
    T ID{0}; T cmb(T a, T b) { return gcd(a, b); }
	int n; V<T> seg;
...
void solve() {
    ...
    SegTree<int> st; st.init(q+5);
    int last_idx_not_used = 0;
    map<int, vi> where;


    for(auto& [type, x]: queries) {
        if(type == '+') {
            st.upd(last_idx_not_used, x);
            where[x].eb(last_idx_not_used);
            last_idx_not_used++;
        } else {
            st.upd(where[x].bk, 0);
            where[x].pop_back();
