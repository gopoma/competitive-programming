...
void solve() {
    ...
    HashRange SH_rev; SH_rev.add(S_rev);
    auto isPal = [&](int L, int R) -> bool {
        return (SH.hash(L, R) == SH_rev.hash((n - 1) - R, (n - 1) - L));
    };
...
//?    vb vis(n + 5);
//?    vpi memo(n + 5);
//?    function<pi(int)> dp = [&](int L) -> pi {
//?        if(L == n) return mp(0, -1);
//?        if(vis[L]) return memo[L];
//?        vis[L] = true;
//?        pi ans = mp(int(1e9) + 5, -1);
//?        for(int R = L; R < n; R++) {
//?            if(isPal(L, R)) {
//?                ckmin(ans, mp(dp(R + 1).f + 1, R + 1));
//?            }
//?        }
//?        return memo[L] = ans;
//?    };
...
//?     }
    ...
    dbg(left);
    {
        ...
        for(int i = 1; i < sz(left); i++) {
            ...
            const int R = left[i] - 1;
            response.eb(S.substr(L, R - L + 1));
        }
        dbg(response);
        ps(sz(response));
