...
void solve() {
    ...
    auto get_dist = [&](int node) -> vi {
        vi dist(n);
        auto dfs = [&](auto&& self, int src, int par, int depth) -> void {
            dist[src] = depth;
            for(auto& nxt: adj[src]) {
                if(nxt == par) continue;
                self(self, nxt, src, depth + 1);
            }
        }; dfs(dfs, node, -1, 0);
        return dist;
    };
    ...
    int A = -79, B = -79;
    {
        ...
        auto get_furthest = [&](int node) -> int {
            vi dist = get_dist(node);
            pi res = mp(-INF, -INF);
            for(int nxt = 0; nxt < n; nxt++) {
                ckmax(res, mp(dist[nxt], nxt));
            }
            return res.s;
        };
    ...
    vi dist_from_A = get_dist(A);
    vi dist_from_B = get_dist(B);

    auto get_nxt = [&](int node) -> vi {
        vi nxt(n);
    ...
    vector parent_A(MAXN, vector<int>(MAXD));
    {
        vi nxt = get_nxt(A);
        for (int i = 1; i <= n; i++) {
            parent_A[i][0] = nxt[i - 1] + 1;
        }
        // Evaluate the parent matrix
	    for(int d = 1; d < MAXD; d++) {
		    for(int i = 1; i <= n; i++) {
			    parent_A[i][d] = parent_A[parent_A[i][d - 1]][d - 1];
		    }
        }
    }
    vector parent_B(MAXN, vector<int>(MAXD));
    {
        vi nxt = get_nxt(B);
        for (int i = 1; i <= n; i++) {
            parent_B[i][0] = nxt[i - 1] + 1;
        }
        // Evaluate the parent matrix
	    for(int d = 1; d < MAXD; d++) {
		    for(int i = 1; i <= n; i++) {
			    parent_B[i][d] = parent_B[parent_B[i][d - 1]][d - 1];
		    }
        }
    }

    auto jump = [&](vector<vector<int>>& parent, int a, int d) -> int {
    ...
    rep(q) {
        ...
        bool ok = false;
        { //* A
            int mx = dist_from_A[v - 1];
            dbg(mx);
            if(0 <= d && d <= mx) {
                dbg("got in A");
                cout << jump(parent_A, v, d) << "\n";
                ok = true;
            }
        }
        ...
        {
            int mx = dist_from_B[v - 1];
            dbg(mx);
            if(0 <= d && d <= mx) {
                dbg("got in B");
                cout << jump(parent_B, v, d) << "\n";
                ok = true;
            }
        }
