...
void solve() {
    ...
    HashRange forwards; forwards.add(S);
    HashRange backwards;
    {
        str S_rev = S;
        reverse(all(S_rev));
    ...
    auto isPalindrome = [&](int L, int R) -> bool {
        int rL = (N - 1) - R;
        int rR = (N - 1) - L;
        return forwards.hash(L, R) == backwards.hash(rL, rR);
    };
    const int BIG = int(1e9);
    vi odd(N, -BIG);
    for(int center = 0; center < N; center++) {
        auto check = [&](int extend) -> bool {
            int L = center - extend + 1;
            int R = center + extend - 1;
            return isPalindrome(L, R);
        };
        int left = 1; //? always good
        int right = min(center - 0 + 1, (N - 1) - center + 1) + 1; //? always bad
        while(left + 1 < right) {
        ...
        int ans = left;
        ckmax(odd[center], ans + (ans - 1));
    }
    vi even(N, -BIG);
    for(int centerL = 0; centerL < N - 1; centerL++) {
        int centerR = centerL + 1;
        if(S[centerL] != S[centerR]) continue;
        auto check = [&](int extend) -> bool {
            int L = centerL - extend + 1;
            int R = centerR + extend - 1;
            return isPalindrome(L, R);
        };
        int left = 1; //? always good
        int right = min(centerL - 0 + 1, (N - 1) - centerR + 1) + 1; //? always bad
        while(left + 1 < right) {
            int middle = (left + right) >> 1;
            if(check(middle)) left = middle;
            else right = middle;
        ...
        int ans = left;
        ckmax(even[centerL], ans + ans);
    }
    int tar = -BIG;
    for(int i = 0; i < N; i++) {
        ckmax(tar, max(odd[i], even[i]));
    }
    for(int center = 0; center < N; center++) {
        if(odd[center] == tar) {
            int extend = (tar + 1) / 2;
