...
bool check(ll x) {
    V<pair<ul,int>> xd = factor(x);
    if(sz(xd) == 2) {
        return (xd.ft.s == 1 && xd.bk.s == 1);
    }
    return false;
}
...
vl slv(ll n) {
    ...
    ans.eb(mbinpow(p, q - 2, q) * p);
    ans.eb(mbinpow(q, p - 2, p) * q);
    sor(ans);
...
//?            vl current = brute(x);
//?            vl greedy = slv(x);
//?            vector<ull> factors{factor(x).ft.f, factor(x).bk.f};
//?            dbg(x, factors, current, greedy);
//?            chk(current == greedy);
//?            ckmax(mx, sz(current));
//?        }
//?        dbg(mx);
//?        dbg(factor(910186311));
//?    }
