...
void solve() {
    //? <>
    vector<vector<vector<str>>> answer;
    while(true) {
        ll n, m; cin >> n >> m; if(n == 0 && m == 0) break;
        V<pair<str, str>> edges_str(m); for(auto& x: edges_str) cin >> x.f >> x.s;
        dbg(n, m);
        dbg(edges_str);

        map<str, ll> forwards;
        map<ll, str> backwards;

        vpl edges;
        SCC G;
        vs uu;
        {
            for(auto& [u, v]: edges_str) {
                uu.eb(u);
                uu.eb(v);
            }
            remDup(uu);
        ...
        map<int, vs> response;
        for(int u = 0; u < sz(uu); u++) {
            response[G.comp[u]].eb(uu[u]);
        }
        vector<vector<str>> current_answer;
        for(auto& [_, members]: response) {
            remDup(members);
            current_answer.eb(members);
        }
        answer.eb(current_answer);
        cout << "\n";
    }
    dbg(answer);
    const int N = sz(answer);
    for(int i = 0; i < N; i++) {
        cout << "Calling circles for data set " << (i + 1) << ":\n";

        const int M = sz(answer[i]);
        for(int j = 0; j < M; j++) {
            const int R = sz(answer[i][j]);
            for(int k = 0; k < R; k++) {
                cout << answer[i][j][k];
                if(k != R - 1) cout << ", ";
            }
