...
void solve() {
    ...
    HashRange S_H; S_H.add(S);

    str S_rev = S; reverse(all(S_rev));
    HashRange S_rev_H; S_rev_H.add(S_rev);
    ...
    auto isPalindrome = [&](int L, int R) -> bool {
        ...
        auto val1 = S_H.hash(L, L + length - 1);

        int RRight = n - L - 1;
        auto val2 = S_rev_H.hash(RRight - length + 1, RRight);
    ...
    auto calc_odd = [&](int extend) -> ll {
        return 1 + 2 * extend;
    };

    auto calc_even = [&](int extend) -> ll {
        return 2 + 2 * extend;
    };
    ...
    for(int center = 0; center < n; center++) {
        ...
        int right = min(center, (n - 1) - center) + 1; //? always bad
        ...
        ckmax(mx, calc_odd(mx_odd[center]));
    }
    ...
    for(int i = 0; i < n - 1; i++) {
        ...
        auto check = [&](int extend) {
            int L = center1 - extend;
            int R = center2 + extend;

            return isPalindrome(L, R);
        };

        if(S[center1] != S[center2]) continue;

        int left = 0; //? always good
        int right = min(center1, (n - 1) - center2) + 1; //? always bad
        ...
        mx_even[center1] = left;

        ckmax(mx, calc_even(mx_even[center1]));
    }
    dbg(mx_odd);
    dbg(mx_even);
    dbg(mx);
