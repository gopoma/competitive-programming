...
void solve() {
    ...
    vvl adj(n);
    for(auto& [u, v]: edges) {
        assert(0 <= G.comp[u] && G.comp[u] < n);
        assert(0 <= G.comp[v] && G.comp[v] < n);
        if(G.comp[u] == G.comp[v]) continue;
        adj[G.comp[u]].eb(G.comp[v]);
    }
    vl memo(n + 5, -1);
    function<ll(ll)> dp = [&](ll src) -> ll {
        if(adj[src].empty()) return values[src];
        ...
        return memo[src] = values[src] + ans;
    };
    ll ans = -BIG;
    each(u, G.comps) {
        ckmax(ans, dp(u));
