...
//?    };
    ...
    auto flip = [&](int y, int x) {
        ll current_val = ftree.query(y, y, x, x);
        //?dbg(y, x, current_val);

        ll new_val = (current_val == 0)? 1 : -1;

        ftree.upd(y, x, new_val);
    };
    ...
    for(int _ = 0; _ < q; _++) {
        ...
        if(type == 1) {
            ...
            ll ans = ftree.query(y1, y2, x1, x2);
            ps(ans);
        } else assert(false);
    }
}
