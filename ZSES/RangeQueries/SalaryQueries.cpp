...
void solve() {
    ...
    each(x, queries) cin >> x.f >> x.s.f >> x.s.s;
    ...
    {
        vi values;
        each(x, p) values.eb(x);
        each(x, queries) {
            if(x.f == '!') {
                //? change the salary of employee k to x
                values.eb(x.s.s);
            } else {
                assert(x.f == '?');
        ...
        each(x, p) x = forwards[x];
        each(x, queries) {
            if(x.f == '!') {
                //? change the salary of employee k to x
                x.s.s = forwards[x.s.s];
            } else {
                assert(x.f == '?');

                //? count the number of employees whose salary is between a ... b
                x.s.f = forwards[x.s.f];
                x.s.s = forwards[x.s.s];
            }
        }
    }
