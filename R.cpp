...
void solve() {
    ...
    V<str> notes = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "B", "H"};
    const int m = sz(notes);
    auto distance = [&](str from, str to) {
        int left = 0;
        for(; left < m; left++)
            if(notes[left] == from)
                break;
    ...
    do {
        ...
        str Y = S[1];
        str Z = S[2];
        if(distance(X, Y) == 4 && distance(Y, Z) == 3 && distance(X, Z) == 7) {
            cout << "major\n";
            return;
        } else if(distance(X, Y) == 3 && distance(Y, Z) == 4 && distance(X, Z) == 7) {
            cout << "minor\n";
            return;
        }
