...
void solve() {
    ...
    ht<int, bool> banned({},{},{},{},{1<<10});
    auto work = [&](int tar) {
        for(int i = 0; i < n; i++)
            if(a[i] == tar && banned.find(i) == end(banned))
                return i;
    ...
    if(!ok) ps("IMPOSSIBLE");
        ...
        vi idx(3);
        idx[0] = work(get<0>(val)); banned[idx[0]] = true;
        idx[1] = work(get<1>(val)); banned[idx[1]] = true;
