...
void solve() {
    ...
    /** @return The sum of all numbers in [from, to] mod MOD. */
    auto work = [&](ll from, ll to) {
        mi right = mi(modMul(to, to + 1LL, MOD)) / 2LL;
        mi left = mi(modMul(from - 1LL, from, MOD)) / 2LL;
    ...
    while(go <= n) {
        ...
        ll last_with_val = fdiv(n, val);

        res += work(go, last_with_val) * val;

        go = last_with_val + 1LL;
    }
    ps(res);
}
