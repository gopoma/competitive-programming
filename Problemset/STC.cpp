...
void solve() {
    ...
    NumArray* tree = new NumArray(v);

    while(q--) {
        char tipo;
        cin >> tipo;

        assert(tipo == '+' || tipo == '-' || tipo == '?');
        if(tipo == '+') {
            int e;
            cin >> e;
            ...
            ll minlr = min(left, right);
            ll maxlr = max(left, right);

            ll sum = tree->sumRange(minlr, maxlr);
            ll sum2 = tree->sumRange(1, minlr) + tree->sumRange(maxlr, n);
            if(sum != 0LL && sum2 != 0LL) {
                cout << "impossible" << nl;
            } else {
                cout << "possible" << nl;
