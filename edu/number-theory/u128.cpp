    ...
    void solve() {
        ...
        if(sz(n) == 1 && (0 <= n[0]-'0' && n[0]-'0' < 8)) {
            cout << n << n_l;
            return;
        }

        // transform n into a u128 num
        auto eval = [&](string S) {
            ull res = ull(0);
            ull ff = ull(1);
            reverse(all(S));
        ...
        // x is in base k
        auto from_base_k_to_base_10 = [&](string x, ull k = ull(8)) {
            reverse(all(x));
        ...
        // x is in base 10
        auto from_base_10_to_base_k = [&](ull x, ull k = ull(9)) {
            string res;
            while(x > ull(0)) {
                res.pb(to_string(int(x % k))[0]);
                x /= k;
            }
            reverse(all(res));
        ...
        for(int _ = 0; _ < k; _++) {
            auto a = from_base_k_to_base_10(n, 8);
            // now a is in base 10

            auto b = from_base_10_to_base_k(a, 9);
            // now b is in base 9
            // DBG(b);
