...
void solve() {
    ...
    //!
    map<int, vector<int>> ccomponents;
    ...
    function<void(int)> dfs = [&](int x) {
        ...
        // process
        ccomponents[current].emplace_back(x);
    ...
    Z ans = 1;
    for(auto& cc: ccomponents) {
        // DBG2(cc.first, cc.second);
        map<int, int> count;
        for(auto& v: cc.second) {
            count[a[v]]++;
        }
        for(auto& it: count) {
            ans *= comb.fac(it.second);
        }
    }
...
// Spooky Sequences
// https://www.codechef.com/problems/SPOOKYSEQ
signed main() {
    // read read read
	setIO();
