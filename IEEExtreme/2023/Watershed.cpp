...
void solve() {
    ...
    vector<vector<long double>> a(n, vector<long double>(m, 1.0));
    ...
    while(!pq.empty()) {
        ...
        int count = 0;
        if(ok(i - 1, j) && (h[i - 1][j] < h[i][j])) count++;
        if(ok(i + 1, j) && (h[i + 1][j] < h[i][j])) count++;
        if(ok(i, j - 1) && (h[i][j - 1] < h[i][j])) count++;
        if(ok(i, j + 1) && (h[i][j + 1] < h[i][j])) count++;

        if(ok(i - 1, j) && (h[i - 1][j] < h[i][j])) a[i - 1][j] += (a[i][j] / count);
        if(ok(i + 1, j) && (h[i + 1][j] < h[i][j])) a[i + 1][j] += (a[i][j] / count);
        if(ok(i, j - 1) && (h[i][j - 1] < h[i][j])) a[i][j - 1] += (a[i][j] / count);
        if(ok(i, j + 1) && (h[i][j + 1] < h[i][j])) a[i][j + 1] += (a[i][j] / count);

        if(count > 0) a[i][j] = 0.0;
    }
