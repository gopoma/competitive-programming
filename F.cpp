...
void solve() {
    ...
    vi rights;
    for(int i = 0; i < n; i++) {
        if(S[i] == 'L') lefts.eb(i + 1);
        else if(S[i] == 'R') rights.eb(i + 1);
    }

    assert(is_sorted(all(lefts)));
    assert(is_sorted(all(rights)));
    if(lefts.empty()) cout << rights.front() << " " << (rights.back() + 1) << n_l;
    else if(rights.empty()) cout << lefts.back() << " " << (lefts.front() - 1) << n_l;
    else {
        assert(rights.back() < lefts.front());
        cout << rights.front() << " " << rights.back() << n_l;
    }
    RAYA;
    RAYA;
    RAYA;
