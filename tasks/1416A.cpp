...
void solve() {
    ...
    vector<vl> where(n + 1);
    for(int x = 0; x <= n; x++) where[x].eb(-1);
    for(int i = 0; i < n; i++) where[a[i]].eb(i);
    for(int x = 0; x <= n; x++) where[x].eb(n);

//?    for(int x = 0; x <= n; x++) {
//?        dbg(x, where[x]);
//?        assert(is_sorted(all(where[x])));
//?    }
    ...
    for(int x = 0; x <= n; x++) {
        ...
        if(m == 2) {
            ...
            ll mx = -BIG;
            for(int j = 0; j < m - 1; j++) {
                ckmax(mx, where[x][j + 1] - where[x][j]);
            }
            assert(mx != -BIG);
    ...
    AngelBeats seg(vl(n + 5, BIG));
    for(int x = 0; x <= n; x++) {
        if(banned[x]) continue;

        seg.range_chmin(d[x], n + 1, x);
    }

    vl ans;
    for(int x = 1; x <= n; x++) {
        ll val = seg.range_sum(x, x + 1);
        if(val == BIG) val = -1;
