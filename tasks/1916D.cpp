...
void solve() {
    ...
    if(n == 1) ps(1);
        ...
        //? 196
        str k2 = "196"; rellena(k2);

        resultados.eb(k2);

        for(int off = 0; off <= offset_mx; off++) {
            //? 169
            str k1 = "1";
            for(int i = 0; i < off; i++) k1.pb('0');
            k1.pb('6');
            for(int i = 0; i < off; i++) k1.pb('0');
            k1.pb('9');
        ...
        assert(sz(resultados) >= n);
        //?sor(resultados);
        //?set<str> aux;
        for(int i = 0; i < n; i++) {
            assert(sz(resultados[i]) == n);
            ps(resultados[i]);

            //?aux.emplace(resultados[i]);
        }
        //?assert(sz(aux) == n);
    }
}
