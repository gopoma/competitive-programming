...
bool  vis[8000 + 1][2 * 8000 + 1];
bool memo[8000 + 1][2 * 8000 + 1];
...
void solve() {
    ...
    for(int i = 0; i < sz(a); i++) {
        if(i % 2 == 0) for_y.eb(a[i]);
        else for_x.eb(a[i]);
    }

    auto work = [&](int start, int tar, vi& adi) -> bool {
        dbg(start, tar, adi);
        //* partial sum should be in [-N, N] want tar
        //* [0, 2 * N] want tar + N

        const int m = sz(adi);
        memset(vis, false, sizeof(vis));
        auto dp = [&](auto&& self, int i, int sum) -> bool {
            if(i == m) return (sum == tar + N);

            if(vis[i][sum]) return memo[i][sum];
            vis[i][sum] = true;

            bool ans = self(self, i + 1, sum + adi[i]) || self(self, i + 1, sum - adi[i]);
            return memo[i][sum] = ans;
        };
        bool response = dp(dp, 0, start + N);
        dbg(response);
        return response;
    };

    bool ok = work(sx, x, for_x) && work(sy, y, for_y);
    cout << (ok?"Yes":"No") << "\n";
}
