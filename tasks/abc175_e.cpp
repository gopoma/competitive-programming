...
bool vis[3000 + 1][3000 + 1][4];
ll  memo[3000 + 1][3000 + 1][4];
void solve() {
    int R, C, K; cin >> R >> C >> K;
    ...
    auto dp = [&](auto&& self, int i, int j, int cnt_row) -> ll {
        if(i == R - 1 && j == C - 1) {
            if(cnt_row < 3) return a[i][j];
            else return 0;
        }

        if(vis[i][j][cnt_row]) return memo[i][j][cnt_row];
        vis[i][j][cnt_row] = true;

        ll ans = -BIG;
        if(i + 1 < R) {
            ckmax(ans, self(self, i + 1, j, 0));
            if(cnt_row < 3)
                ckmax(ans, self(self, i + 1, j, 0) + a[i][j]);
        }
        if(j + 1 < C) {
            ckmax(ans, self(self, i, j + 1, cnt_row));
            if(cnt_row < 3)
                ckmax(ans, self(self, i, j + 1, cnt_row + 1) + a[i][j]);
        }
        return memo[i][j][cnt_row] = ans;
    };
    ll response = dp(dp, 0, 0, 0);
    cout << response << "\n";
}
