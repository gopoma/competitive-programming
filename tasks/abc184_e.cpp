...
void solve() {
    ...
    const pi S = locate('S').ft;
    const pi G = locate('G').ft;
    V<vpi> locations(26);
    for(int i = 0; i < 26; i++) {
        locations[i] = locate(char('a' + i));
    }
    ...
    vb already(26);
    vvb vis(n, vb(m)); vis[S.f][S.s] = true;
    vvl dist(n, vl(m, BIG)); dist[S.f][S.s] = 0;
    deque<pi> q; q.pb(S);
    while(!q.empty()) {
        ...
        if('a' <= a[x][y] && a[x][y] <= 'z' && !already[a[x][y] - 'a']) {
            already[a[x][y] - 'a'] = true;
            for(auto& [nx, ny]: locations[a[x][y] - 'a']) {
                if(!vis[nx][ny]) {
                    vis[nx][ny] = true;
                    dist[nx][ny] = dist[x][y] + 1;
                    q.eb(nx, ny);
    ...
    if(!vis[G.f][G.s]) cout << "-1";
    else cout << dist[G.f][G.s];
    cout << "\n";
}
