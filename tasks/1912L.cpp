...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        int left = loavesLeft + onionsLeft;
        int right = loavesRight + onionsRight;
        dbg(i, left, right, loavesLeft, onionsLeft, loavesRight, onionsRight);

        bool ok1 = min(left, right) >= 1;
        bool ok2 = loavesLeft != loavesRight;
        bool ok3 = onionsLeft != onionsRight;
        if(ok1 && ok2 && ok3) {
            cout << (i + 1) << "\n";
            return;
        }
