...
void solve() {
    ...
    vl guarda_queries = queries;
    remDup(queries);

    auto work = [&](vl A) -> set<ll> {
        ll tot = accumulate(all(A), 0LL);
        set<ll> response;
        for(auto& x: A) response.emplace(tot - x);
        return response;
    };
    set<ll> A = work(a);
    set<ll> B = work(b);
    ...
    for(auto& x: queries) {
        ll x_norm = abs(x);
        bool ok = false;
        for(auto& d: divs[x_norm]) {
            ll o = fdiv(x_norm, d);
            if(x > 0) {
                if((A.count(d) && B.count(o)) || (A.count(-d) && B.count(-o))) {
                    ok = true;
                    break;
                }
            } else if(x < 0) {
        ...
        if(ok) isGood.emplace(x);
    }

    for(auto& x: guarda_queries) {
        cout << (isGood.count(x)?"YES":"NO") << "\n";
    }
}
