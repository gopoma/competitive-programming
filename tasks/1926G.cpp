...
void solve() {
    ...
    auto dp = [&](auto&& dp, int src, int par, bool shouldBeP) -> int {
        if(vis[src][shouldBeP]) return memo[src][shouldBeP];
        vis[src][shouldBeP] = true;

        if(
            (type[src] == 'P' && !shouldBeP) ||
            (type[src] == 'S' && shouldBeP)
        ) {
            return memo[src][shouldBeP] = INF;
        }

        int res = 0;
        for(auto& nxt: adj[src]) {
            if(nxt == par) continue;
            int A = dp(dp, nxt, src, true) + int(!shouldBeP);
            int B = dp(dp, nxt, src, false) + int(shouldBeP);
            res += min(A, B);
        }
        return memo[src][shouldBeP] = res;
    };
    int res = min(dp(dp, 0, -1, true), dp(dp, 0, -1, false));
    cout << res << "\n";
}
