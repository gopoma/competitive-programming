...
void solve() {
    int n, m; cin >> n >> m;
    str sstart; cin >> sstart;

    auto get = [&](str& S) -> int {
        int res = 0;
        for(int bit = 0; bit < n; bit++) {
            if(S[bit] == '1') res |= (1 << bit);
        }
        return res;
    };
    ...
    int start = get(sstart);

    multiset<pi> ms; ms.emplace(0, start);
    const int MAXN = (1 << n) + 5;
    vb vis(MAXN);
    vi dist(MAXN);
    while(!ms.empty()) {
        auto it = *ms.begin();
        auto [current_dist, current_node] = it;
        safeErase(ms, it);
        ...
        for(auto& [cost, from, to]: edges) {
            ...
            for(int bit = 0; bit < n; bit++) {
                if(bool(from & (1 << bit)) && bool(new_node & (1 << bit))) {
                    new_node ^= (1 << bit);
                }
            }
            for(int bit = 0; bit < n; bit++) {
                if(bool(to & (1 << bit)) && !bool(new_node & (1 << bit))) {
                    new_node ^= (1 << bit);
                }
            }
            ms.emplace(current_dist + cost, new_node);
        }
    }
