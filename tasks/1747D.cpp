...
void solve() {
    ...
    auto query_sum = [&](ll L, ll R) -> ll {
        ll sum = pref_sum[R] - pref_sum[L - 1];
        return sum;
    };

    vl pref_xor = a;
    for(int i = 1; i <= n; i++) pref_xor[i] ^= pref_xor[i - 1];
    auto query_xor = [&](ll L, ll R) -> ll {
        ll sum = pref_xor[R] ^ pref_xor[L - 1];
        return sum;
    };
    map<ll, set<ll>> odd, even;
    for(int i = 0; i <= n; i++) {
        if(i & 1) odd[pref_xor[i]].emplace(i);
        else     even[pref_xor[i]].emplace(i);
    }
    ...
    for(auto& [L, R]: queries) {
        dbg(L, R);
        ll length = R - L + 1;
        ll tot_sum = query_sum(L, R);
        ll tot_xor = query_xor(L, R);
        if(length <= 2) {
            if(tot_sum == 0) {
                response.eb(0);
            } else {
            ...
            if(length & 1LL) {
                ...
                if(tot_sum == 0) {
                    ...
                    if(tot_xor != 0) {
                        ...
                        //* 1
                        {
                            if(a[L] == 0 && query_xor(L + 1, R) == 0) {
                                response.eb(1);
                                continue;
                            }
                            if(a[R] == 0 && query_xor(L, R - 1) == 0) {
                                response.eb(1);
                                continue;
                            }
                        }
                        //* 2
                        {
                            ...
                            dbg(odd, even);
                            dbg(pref_xor[L - 1]);
                            assert((pref_xor[L - 1] ^ pref_xor[R]) == 0LL);

                            if(L & 1LL) {
                                auto it = odd[pref_xor[L - 1]].upper_bound(L - 1);
                                if(it != odd[pref_xor[L - 1]].end() && ((*it) < R)) {
                                    response.eb(2);
                                    continue;
                                }
                            } else {
                                auto it = even[pref_xor[L - 1]].upper_bound(L - 1);
                                if(it != even[pref_xor[L - 1]].end() && ((*it) < R)) {
                                    response.eb(2);
                                    continue;
                                }
                            }
