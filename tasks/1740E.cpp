...
void solve() {
    ...
    vvb  vis(n + 1, vb(2));
    vvl memo(n + 1, vl(2));
    auto dp = [&](const auto& dp, int u, int par, bool shouldTake) -> ll {
        if(vis[u][shouldTake]) {
            return memo[u][shouldTake];
        }
        vis[u][shouldTake] = true;

        if(shouldTake) {
            ll sum = 0;
            for(auto& nxt: adj[u]) {
                if(nxt != par) {
                    ll x = dp(dp, nxt, u, true);
                    ll y = dp(dp, nxt, u, false);
                    sum += max(x, y);
                }
            }
            return memo[u][shouldTake] = sum;
        } else {
            ll mx = 0;
            for(auto& nxt: adj[u]) {
                if(nxt != par) {
                    ckmax(mx, dp(dp, nxt, u, false));
                }
            }
            return memo[u][shouldTake] = mx + 1LL;
        }
    };
    ll ans = max(dp(dp, 0, -1, true), dp(dp, 0, -1, false));
    dbg(ans);
    cout << ans << "\n";
}
