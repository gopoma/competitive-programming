...
void solve() {
    ...
    if(N & 1) {
        int center = N / 2;
        int can_extend = (N - 1) - center;
        dbg(center, can_extend);

        for(int d = 0; d <= can_extend; d++) {
            int LC = center - d;
            int RC = center + d;
            if(LC == 0 || RC == N - 1) continue;
            if(sh.hash(0, RC) == sh.hash(LC, N - 1)) {
                ps("YES");
                ps(S.substr(0, RC - 0 + 1));
                return;
            }
        }
    } else {
        int centerR = N / 2;
        int centerL = centerR - 1;
        int can_extend = (N - 1) - centerR;
        dbg(centerL, centerR, can_extend);
