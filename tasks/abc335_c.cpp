...
void solve() {
    ...
    vpi go(N);
    for(int i = 0; i < N; i++) go[i] = mp(N - i, 0);
    dbg(go);

    auto get = [](char C) {
        if(C == 'R')      return mp(1, 0);
        else if(C == 'L') return mp(-1, 0);
        else if(C == 'U') return mp(0, 1);
        else if(C == 'D') return mp(0, -1);
        else assert(false);
    };
    ...
    for(int _ = 0; _ < Q; _++) {
        ...
        if(type == 1) {
            ...
            go.eb(mp(go.bk.f + delta.f, go.bk.s + delta.s));
        } else if(type == 2) {
            def(int, p); p--;
