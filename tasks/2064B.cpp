...
ll calc(vl a) {
    const ll n = sz(a);
    set<ll> U; for(auto& x: a) U.emplace(x);
    return n - (sz(U));
}

pl brute(ll n, vl a) {
    pl ans = mp(calc(a), -n); // max - min
    for(int l = 0; l < n; l++) {
        for(int r = l; r < n; r++) {
            vl b;
            for(int k = 0; k < l; k++) b.eb(a[k]);
            for(int k = r + 1; k < n; k++) b.eb(a[k]);
            ckmax(ans, mp(calc(b), -ll(sz(b))));
        }
    }
    return ans;
}
...
pl slv(ll n, vl a) {
    ...
    vl b(n);
    for(int i = 0; i < n; i++) if(hist[a[i]] == 1) b[i] = 1;
    ...
    if(!could) {
        ...
        Info response = make_tuple(-BIG, BIG, BIG);
        for(auto& [left, right]: intervals) {
            if(b[left] == +1) {
                Info local_response = make_tuple(right - left + 1, left, right);
                ckmax(response, local_response);
            }
        }
        auto [_, left, right] = response;
