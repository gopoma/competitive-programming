...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        for(int x = 1; x <= n; x++) {
            if(taken[x]) continue;
            aux.eb(make_pair(v[x], x));
        }
        sort(rall(aux));

        // delete aux[0]
        int idx_to_delete = aux[0].second;
        for(auto& y: adj[idx_to_delete]) {
            ans += v[y];
        }
        taken[idx_to_delete] = true;

        for(auto& SS: adj) {
            if(SS.count(idx_to_delete)) SS.erase(idx_to_delete);
        }
    }
