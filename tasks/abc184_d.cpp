...
bool vis[100 + 5][100 + 5][100 + 5];
db  memo[100 + 5][100 + 5][100 + 5];
...
void solve() {
    ...
    auto dp = [&](auto&& self, int a, int b, int c) -> db {
        if(a == 100 || b == 100 || c == 100) return db(0);
        if(vis[a][b][c]) return memo[a][b][c];
        vis[a][b][c] = true;

        db ans = 0;
        ans += (db(a) / db(a + b + c)) * (self(self, a + 1, b, c) + 1);
        ans += (db(b) / db(a + b + c)) * (self(self, a, b + 1, c) + 1);
        ans += (db(c) / db(a + b + c)) * (self(self, a, b, c + 1) + 1);
