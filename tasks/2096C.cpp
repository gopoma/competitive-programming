...
void solve() {
    ...
    for(auto& vec: h) for(auto& x: vec) cin >> x;
    vl a(n); for(auto& x: a) cin >> x;
    vl b(n); for(auto& x: b) cin >> x;
    dbg(n);
    for(auto& row: h) dbg(row);
    dbg(a);
    dbg(b);

    auto work = [&](vl& cost) -> ll {
        for(int i = 0; i <= n; i++)
            for(int j = 0; j < 2; j++)
                vis[i][j] = false;

        auto dp = [&](auto&& dp, int row, bool lastWasTaken) -> ll {
            if(row == n) return 0;

            if(vis[row][lastWasTaken]) return memo[row][lastWasTaken];
            vis[row][lastWasTaken] = true;
            ...
            //* don't take
            {
                ...
                if(row > 0) {
                    for(int col = 0; col < n; col++) {
                        if(h[row - 1][col] + ll(lastWasTaken) == h[row][col]) {
                            ok = false;
                            break;
                        }
                    }
            ...
            //* take
            {
                ...
                if(row > 0) {
                    for(int col = 0; col < n; col++) {
                        if(h[row - 1][col] + ll(lastWasTaken) == h[row][col] + 1) {
                            ok = false;
                            break;
                        }
                    }
                ...
                if(ok) {
                    ckmin(res, dp(dp, row + 1, true) + cost[row]);
                }
            }

            return memo[row][lastWasTaken] = res;
        };
        ll res = dp(dp, 0, false);
        return res;
    };
