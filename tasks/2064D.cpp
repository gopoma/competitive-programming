...
vi brute(int n, int q, vi a, vi queries) {
    reverse(all(a));
    vi response;
    for(auto& x: queries) {
        int cnt = 0;
...
vi slv(int n, int q, vi a, vi queries) {
    const int MXBIT = 30;
    vvi cnt(MXBIT, vi(n));
    ...
    for(int i = 0; i < n; i++) {
        for(int bit = 0; bit < MXBIT; bit++) {
            ...
            if(0 <= i - 1) cnt[bit][i] += cnt[bit][i - 1];
        }
    }
    auto query = [&](vi& pref, int L, int R) -> ll {
    ...
    vvi nxt(MXBIT, vi(n, n + 5));
    for(int i = n - 1; i >= 0; i--) {
        for(int bit = 0; bit < MXBIT; bit++) {
            if(a[i] & (1 << bit)) {
                ckmin(nxt[bit][i], i);
            }
            if(i + 1 < n) ckmin(nxt[bit][i], nxt[bit][i + 1]);
        }
    }
    vi response;
    for(auto& x: queries) {
        auto check = [&](int middle) -> bool {
            ...
            for(int bit = MXBIT - 1; bit >= 0; bit--) {
                int count = query(cnt[bit], middle, right);
                int tot = query(cnt[bit], middle, n - 1);
                int o = tot - count;
                int on = int(bool(x & (1LL << bit))) + o;
                on %= 2;
                ...
                if(on) {
                    if(count == 0) return true;
                        ...
                        assert(count == 1);
                        // int l = middle; // always good
                        // int r = n; // always bad
                        // while(l + 1 < r) {
                        //     int mid = (l + r) >> 1;
                        //     if(query(cnt[bit], mid, right) == 1) l = mid;
                        //     else r = mid;
                        // }
                        right = nxt[bit][middle];
                        continue;
                    }
                } else {
...
void solve() {
    ...
    vi queries(q); for(auto& x: queries) cin >> x;
    dbg(n);
    dbg(a);
    dbg(queries);
    for(auto& x: a) dbg(bitset<20>(x));

    vi response = slv(n, q, a, queries);
...
int main() {
    ...
    while(0) {
        ...
        vi a(n), queries(q);
        for(auto& x: a) x = rng_int(1, (1 << 30) - 1);
        for(auto& x: queries) x = rng_int(1, (1 << 30) - 1);
        dbg(n, q);
        dbg("a");
        // for(auto& x: a) dbg(x, bitset<10>(x));
        dbg("queries");
        // for(auto& x: queries) dbg(x, bitset<10>(x));
        vi ans = brute(n, q, a, queries);
        vi greedy = slv(n, q, a, queries);
        dbg(ans, greedy);
        chk(ans == greedy);
    }
