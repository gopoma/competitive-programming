...
using Program = tuple<int, int, int>;
void solve() {
    int N, tot_programs; cin >> N >> tot_programs;
    V<Program> __programs(N);
    for(auto& [S, T, C]: __programs) {
        cin >> S >> T >> C;
    }
    dbg(N, tot_programs);
    ...
    for(int id = 0; id < MAXN; id++) {
        auto programs = blocks[id];
        const int m = sz(programs);
        int i = 0;
        while(i < m) {
            auto [start, T] = programs[i];
            int finish = T;

            int j = i + 1;
            while(j < m && finish == programs[j].f) {
                finish = programs[j].s;
                j++;
            }

            intervals.eb(start, finish);
