...
void precompute() {
    const int mx = 2025;
    vector<vector<ll>> table(mx, vector<ll>(mx));

    auto check = [&](int xx, int yy) {
        return (0 <= xx && xx < mx) && (0 <= yy && yy < mx);
    };

    auto apply = [&](pi& act) -> void {
        act = mp(act.f - 1, act.s + 1);
    };
    ...
    for(int row = 0; row < mx; row++) {
        ...
        while(check(start.f, start.s)) {
            table[start.f][start.s] = cnt * cnt;
            mappings[cnt] = mp(start.f, start.s);
            cnt++;
