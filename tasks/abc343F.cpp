...
struct node {
	...
	node(long long _val) { hist[_val]++; }

	node operator + (const node &rhs) const {
        node new_node;
        map<ll, ll> new_hist;
        each(x, hist) new_hist[x.f] += x.s;
        each(x, rhs.hist) new_hist[x.f] += x.s;

        while(sz(new_hist) > 2) {
            ll xd = new_hist.begin()->f;
            safeErase(new_hist, xd);
        }
        new_node.hist = new_hist;
...
void solve() {
    ...
    rep(Q) {
        ...
        if(type == 1) {
            ...
            map<ll, ll> hist = st.query(l, r).hist;
            if(sz(hist) <= 1) ps("0");
            else {
                assert(sz(hist) == 2);
                ps(hist.begin()->s);
