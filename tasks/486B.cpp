...
void solve() {
    ...
    for(int i = 0; i < m; i++) {
        ...
        for(int j = 0; j < n; j++)
            could &= B[i][j];
        if(could) could_rows.eb(i);
    }
    for(int i = 0; i < n; i++) {
        bool could = true;
        for(int j = 0; j < m; j++)
            could &= B[j][i];
        if(could) could_columns.eb(i);
    }
    dbg(could_rows);
    dbg(could_columns);

    vector<vi> A(m, vi(n, 0));
    for(auto& row: could_rows) for(auto& col: could_columns) A[row][col] = 1;
    for(auto& x: A) dbg(x);

    set<int> rows_on, cols_on;
    for(int i = 0; i < m; i++) {
        for(int j = 0; j < n; j++) {
            if(A[i][j]) {
                rows_on.emplace(i);
    ...
    vector<vi> BBB(m, vi(n));
    for(int i = 0; i < m; i++) {
        for(int j = 0; j < n; j++) {
            if(rows_on.count(i) || cols_on.count(j)) {
                BBB[i][j] = 1;
            }
        }
    }
