...
bool vis[200 + 5][400 + 5];
int memo[200 + 5][400 + 5];
void solve() {
    int n; cin >> n;
    vi a(n); for(auto& x: a) cin >> x;
    sor(a);
    for(int i = 0; i < n; i++) for(int j = 0; j <= 2 * n; j++) vis[i][j] = false;
    auto dp = [&](auto&& self, int i, int j) -> int {
        ...
        int ans = INF;
        //* skip
        ckmin(ans, self(self, i, j + 1));
        //* take
        ckmin(ans, self(self, i + 1, j + 1) + abs(j - a[i]));

        return memo[i][j] = ans;
    };
    int response = dp(dp, 0, 1);
    cout << response << "\n";
}
