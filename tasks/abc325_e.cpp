...
void solve() {
    ...
    Dijkstra<ll, true> by_car; by_car.init(int(N));
    Dijkstra<ll, true> by_train; by_train.init(int(N));
    ...
    for(int i = 0; i < N; i++) {
        for(int j = 0; j < N; j++) {
            ...
            by_car.ae(i, j, D * A);
            by_train.ae(j, i, D * B + C);
        }
    }

    by_car.gen(0);
    by_train.gen(int(N - 1));

    ll ans = BIG;
    for(int u = 0; u < N; u++) {
        ckmin(ans, by_car.dist[u] + by_train.dist[u]);
    }
    assert(ans != BIG);
    dbg(ans);
    ps(ans);
