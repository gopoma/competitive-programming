...
bool slv(ll n, vl a) {
    ...
    SegTreeMax<ll> st_a; st_a.init(n + 5);
    SegTreeMin<ll> st_pref_min; st_pref_min.init(n + 5);
    SegTreeMax<ll> st_pref_max; st_pref_max.init(n + 5);
    for(int i = 0; i <= n; i++) {
        st_a.upd(i, a[i]);
        st_pref_min.upd(i, pref[i]);
        st_pref_max.upd(i, pref[i]);
    }
    for(int i = 1; i <= n; i++) {
        // dbg("ga");
        // dbg(i, a[i]);
        ll mn_pref = -BIG;
        {
            ...
            while(left + 1 < right) {
                ...
                if(st_a.query(middle, i) <= a[i]) right = middle;
                else left = middle;
            }
            // dbg(left, right);
            mn_pref = st_pref_min.query(right - 1, i - 1);
        }
        chk(mn_pref != -BIG);
        ll mx_pref = -BIG;
        {
            int left = i; // always good
            int right = n + 1; // always bad
            while(left + 1 < right) {
                int middle = (left + right) >> 1;
                if(st_a.query(i, middle) <= a[i]) left = middle;
                else right = middle;
            }
            // dbg(left, right);
            mx_pref = st_pref_max.query(i, left);
        }
        chk(mx_pref != -BIG);
        // dbg(mx_pref, mn_pref);
        if(mx_pref - mn_pref > a[i]) return false;
    }
    return true;
}
...
int main() {
    ...
    while(0) {
        RAYA;
        ll n = rng_ll(1, 100);
        vl a(n); for(auto& x: a) x = rng_ll(-10, 10);
        dbg(n);
        dbg(a);
        bool ans = brute(n, a);
        bool greedy = slv(n, a);
