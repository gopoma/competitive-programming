...
//?    ll mn = BIG;
//?    ll mx = -BIG;
//?    for(int i = 1; i < sz(go); i++) {
//?        ckmin(mn, go[i] - go[i - 1]);
//?        ckmax(mx, go[i] - go[i - 1]);
//?    }

    //?dbg(go);
    //?dbg(*max_element(all(go)), mn, mx);

    def(int, N); vi nums(N); re(nums); dbg(nums);

    sort(nums.begin(), nums.end());
    const int n = int(nums.size());
    int median = -1;
    if(n % 2 == 0) median = (nums[n / 2] + nums[n / 2 - 1]) / 2;
    ...
    int right = median + 1;
    while(!ok(left)) left--;
    while(!ok(right)) right++;
    left = max(left, 1);

    auto cost = [&](int x) {
        long long res = 0;
        for(auto& e: nums) res += abs(x - e);
        return res;
    };

    long long ans = min(cost(left), cost(right));
    if(ok(median)) ans = min(ans, cost(median));
