...
void solve() {
    ...
    for(int i = 0; i < n - 1; i++) {
        segments.eb(make_pair(min(x[i], x[i + 1]), max(x[i], x[i + 1])));
    }
    sor(segments);
    // for(auto& it: segments) DBG(it);
    bool ok = false;
    for(int i = 0; i < sz(segments); i++) {
        for(int j = 0; j < sz(segments); j++) {
            if(i == j) continue;
            ll x1 = segments[i].first;
            ll y1 = segments[i].second;
            ll x2 = segments[j].first;
            ll y2 = segments[j].second;

            ok |= (x1 < x2 && x2 < y1 && y1 < y2);
        }
    }
    cout << (ok?"yes":"no") << n_l;
}
