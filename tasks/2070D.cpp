...
void solve() {
    ...
    for(int u = 0; u < n; u++) dbg(u, adj[u], parent[u], depth[u]);
    vvl buckets(n);
    for(int u = 0; u < n; u++) {
        buckets[depth[u]].eb(u);
    }

    vmi dp(n);
    dp[ROOT] = mi(1);
    for(auto& nxt: adj[ROOT]) dp[nxt] = mi(1);
    for(int dep = 2; dep < n; dep++) {
        if(buckets[dep].empty()) break;

        mi tot = 0;
        for(auto& node: buckets[dep - 1]) {
            tot += dp[node];
        }
        for(auto& node: buckets[dep]) {
            dp[node] = tot - dp[parent[node]];
        }
    }
