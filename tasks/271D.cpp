...
void slv(int n, str& S, str& isGood, int k) {
    for(int i = 0; i < n; i++) {
        if(isGood[S[i] - 'a'] == '0') pref_bads[i]++;
    }

    for(int i = 1; i < n; i++) pref_bads[i] += pref_bads[i - 1];

    auto query = [&](int L, int R) -> int {
        int sum = pref_bads[R];
        if(0 <= L - 1) sum -= pref_bads[L - 1];
        return sum;
    };
