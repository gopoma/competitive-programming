...
bool intersect(pl a, pl b) { // check if a is inside b
    return !(a.s < b.f || b.s < a.f);
}

pl intersection(pl a, pl b) {
...
void solve() {
    int n; cin >> n;
    vpl segments(n); for(auto& x: segments) cin >> x.f >> x.s;

    auto check = [&](ll k) {
        pl current = mp(0, 0);
        for(int i = 0; i < n; i++) {
            current = mp(current.f - k, current.s + k);

            if(!intersect(current, segments[i])) return false;

            current = intersection(current, segments[i]);
        }
        return true;
    };

    ll left = -1; // always bad
    ll right = 1e12; // always good
    while(left + 1 < right) {
