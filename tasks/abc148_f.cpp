...
void solve() {
    ...
    auto work = [&](int src) -> vi {
        vb vis(n); vis[src] = true;
        vi dist(n); dist[src] = 0;
        deque<int> q; q.eb(src);
        while(!q.empty()) {
    ...
    vi dist_from_T = work(T);
    vi dist_from_A = work(A);
    for(int u = 0; u < n; u++) {
        //* cerr << "u: " << (u + 1) << " " << dist_from_T[u] << " " << dist_from_A[u] << "\n";
    }

    int response = 0;
    for(int u = 0; u < n; u++) {
        if(dist_from_T[u] < dist_from_A[u]) {
            int adi = dist_from_A[u] - dist_from_T[u];
            ckmax(response, dist_from_T[u] + adi - 1);
        }
    }
    cout << response << "\n";
}
