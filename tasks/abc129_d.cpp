...
const pi UP = mp(-1, 0);
const pi DOWN = mp(+1, 0);
const pi LEFT = mp(0, -1);
const pi RIGHT = mp(0, +1);
...
void solve() {
    ...
    for(int row = 0; row < n; row++) {
        for(int col = 0; col < m; col++) {
            ...
            if(0 <= col - 1) dp_left[row][col] += dp_left[row][col - 1] + 1;
            else dp_left[row][col] = 1;
        }

        for(int col = m - 1; col >= 0; col--) {
            if(S[row][col] == '#') continue;
            if(col + 1 < m) dp_right[row][col] += dp_right[row][col + 1] + 1;
            else dp_right[row][col] = 1;
        }
    }
    ...
    for(int col = 0; col < m; col++) {
        for(int row = 0; row < n; row++) {
            if(S[row][col] == '#') continue;
            if(0 <= row - 1) dp_up[row][col] += dp_up[row - 1][col] + 1;
            else dp_up[row][col] = 1;
        }

        for(int row = n - 1; row >= 0; row--) {
            if(S[row][col] == '#') continue;
            if(row + 1 < n) dp_down[row][col] += dp_down[row + 1][col] + 1;
            else dp_down[row][col] = 1;
        }
    }
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            if(S[i][j] == '.') {
                int local_response = 1;
                if(check(i + UP.f, j + UP.s)) local_response += dp_up[i + UP.f][j + UP.s];
                if(check(i + DOWN.f, j + DOWN.s)) local_response += dp_down[i + DOWN.f][j + DOWN.s];
                if(check(i + LEFT.f, j + LEFT.s)) local_response += dp_left[i + LEFT.f][j + LEFT.s];
                if(check(i + RIGHT.f, j + RIGHT.s)) local_response += dp_right[i + RIGHT.f][j + RIGHT.s];
