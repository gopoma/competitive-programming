...
void brute() {
    ...
    map<string, int> cnt;
    map<string, vector<vector<int>>> keep;
    do {
        set<int> aux; aux.emplace(p[0]);
        string lre;
        ...
        cnt[lre]++;
        keep[lre].emplace_back(p);
    } while(next_permutation(all(p)));


    map<int, int> cnt_ways;
    V<pair<int, string>> stk;
    for(auto& [S, count]: cnt) {
        dbg(S, count, keep[S]);
        cnt_ways[count]++;
        stk.emplace_back(count, S);
    }
    dbg(sz(cnt));
    dbg(cnt_ways);
    sor(stk);
