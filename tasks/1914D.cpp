...
void solve() {
    def(ll, n);
    vl a(n), b(n), c(n); re(a); re(b); re(c);

    vi go(3); for(int i = 0; i < 3; i++) go[i] = i;
    ll ans = 0;
    auto get = [&](ll idx, ll wh) {
        if(wh == 0) return a[idx];
        else if(wh == 1) return b[idx];
        else return c[idx];
    };
    do {
        const int OFFSET = 5;
        vector<vector<bool>> vis(n + OFFSET, vector<bool>(3 + OFFSET, false));
        vector<vector<ll>> dp(n + OFFSET, vector<ll>(3 + OFFSET));
        function<ll(ll, ll)> solve = [&](ll idx, ll at) {
            if(idx == n) return 0LL;

            if(vis[idx][at]) return dp[idx][at];
            vis[idx][at] = true;

            ll res = solve(idx + 1, at);
            for(ll i = at; i <= 2; i++) {
                ckmax(res, solve(idx + 1, at + 1) + get(idx, go[at]));
            }
            return dp[idx][at] = res;
        };
        dbg(go, solve(0, 0));
        ckmax(ans, solve(0, 0));
