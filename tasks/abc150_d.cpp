...
ll slv(ll N, ll M, vl a) {
    ...
    each(x, a) {
        g = lcm(g, x);
        if(g > ll(1e10)) return 0LL;
    }
    chk(N >= 1);

    pl xd = CRT(mp(0, 1), mp(fdiv(a.ft, 2LL), a.ft));
    for(int i = 1; i < N; i++) {
        if(xd == mp(-1LL, -1LL)) return 0LL;

        xd = CRT(mp(xd.f, xd.s), mp(fdiv(a[i], 2LL), a[i]));
    }
    if(xd == mp(-1LL, -1LL)) return 0LL;

    dbg(xd);
    if(xd.f > M) return 0LL;

    auto check = [&](ll x) {
        return (M >= xd.s * x + xd.f);
    };
...
signed main() {
	...
	while(0) {
		...
		ll N = rng_ll(1, 10);
		ll M = rng_ll(1, 1000);
		vl a(N); each(x, a) x = 2LL * rng_ll(2, 10000);

        bool modify = rng_ll(0, 1);
        if(modify) {
            bool even = rng_ll(0, 1);
            dbg(even);
