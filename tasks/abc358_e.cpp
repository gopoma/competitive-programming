...
void solve() {
    ...
    function<mi(int, int)> dp = [&](int idx, int tam) -> mi {
        if(idx == 26) {
            if(tam == 0) return 0;
            else return 1;
        }
        if(vis[idx][tam]) return memo[idx][tam];
        vis[idx][tam] = true;

        mi ans = 0;
        for(int x = 0; x <= hist[idx]; x++) {
            if(tam + x > K) break;
            ans += dp(idx + 1, tam + x) * F.C(tam + x, x);
        }
        return memo[idx][tam] = ans;
    };
    mi ans = dp(0, 0);
    dbg(ans);
    ps(ans);
