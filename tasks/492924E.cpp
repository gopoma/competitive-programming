...
struct node {
	...
	node operator + (const node &rhs) const {
        ...
        each(x, hist)     go[x.f] += x.s;
        each(x, rhs.hist) go[x.f] += x.s;
...
void solve() {
    ...
    for(int i = 0; i < n; i++) tree.upd(i, node(a[i]));

    rep(q) {
        def(int, l, r); l--; r--;
        //?dbg(l, r);

        const node result = tree.query(l, r);

        db length = r - l + 1.0;
        db denom = ((length - 1.0) * (length)) / 2.0;
        db numer = 0.0;
        for(auto& [val, cnt]: result.hist) {
            numer += ((db(cnt) - 1.0) * db(cnt)) / 2.0;
        }
