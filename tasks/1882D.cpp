...
void solve() {
    ...
    for(int u = 0; u < n; u++) dbg(u + 1, subtree_size[u]);
    ...
    auto work = [&](vb& b) -> vl {
        ...
        auto dfs2 = [&](auto&& self, int src, int par) -> void {
            ...
            for(auto& nxt: adj[src]) {
                if(nxt == par) continue;
                //* make nxt as root
                ll lst_dp_nxt = dp[nxt];
                ll lst_dp_src = dp[src];

                //* dbg("Transition", src + 1, nxt + 1, dp[nxt]);

                dp[src] -= dp[nxt];
                if(b[src] != b[nxt]) dp[src] -= subtree_size[nxt];

                //* dbg("Transition", src + 1, nxt + 1, dp[src]);

                dp[nxt] += dp[src];
                if(b[src] != b[nxt]) {
                    dp[nxt] += n - subtree_size[nxt];
                }
