...
void check(vl res) {
    set<ll> C; for(auto& x: res) C.emplace(x);
    chk(sz(C) == sz(res));
    ll tot = 0;
    ll pot = +1;
...
vl rng_vec_unique(ll N, ll L, ll R) {
	set<ll> so_far; vl res;
	F0R(_,N) {
		ll x; do { x = rng_int(L,R); } while (so_far.count(x));
...
int main() {
    ...
    while(0) {
        RAYA;
        ll n = rng_ll(1, 30);
        vl b = rng_vec_unique(2 * n, 1, ll(100));
        dbg(n);
        dbg(b);
        slv(n, b);
    }
