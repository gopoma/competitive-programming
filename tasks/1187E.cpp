...
void solve() {
    ...
    auto dfs1 = [&](auto&& self, int src, int par) -> void {
        subtree_size[src] = 1;
        ...
        dp[src] += subtree_size[src];
    }; dfs1(dfs1, 1, -1);

    for(int i = 1; i <= n; i++) {
        dbg(i, subtree_size[i], dp[i]);
    }
    ...
    auto dfs2 = [&](auto&& self, int src, int par) -> void {
        dbg(src, dp[src]);
        response = max(response, dp[src]);

        for(auto& nxt: adj[src]) {
            if(nxt == par) continue;

            const ll temp_subtree_size_src = subtree_size[src];
            const ll temp_subtree_size_nxt = subtree_size[nxt];

            const ll temp_dp_src = dp[src];
            const ll temp_dp_nxt = dp[nxt];

            dp[src] -= subtree_size[src];
            dp[src] -= dp[nxt];

            dp[nxt] -= subtree_size[nxt];

            subtree_size[src] -= subtree_size[nxt];
            subtree_size[nxt] += subtree_size[src];
            ...
            subtree_size[src] = temp_subtree_size_src;
            subtree_size[nxt] = temp_subtree_size_nxt;
            dp[src] = temp_dp_src;
            dp[nxt] = temp_dp_nxt;
        }
    }; dfs2(dfs2, 1, -1);
