    ...
    rep(m) {
        ...
        for(ll x = 0; x < Mod; x++) {
            for(ll y = 0; y < Mod; y++) {
                if((x * C + y) % Mod == k) {
                    if(x != y) {
                        if(!adj[x].empty() && !adj[y].empty()) {
                            ckmin(response, mp(adj[x].ft, adj[y].ft));
                        }
                    } else {
                        if(sz(adj[x]) >= 2) {
                            ckmin(response, mp(adj[x][0], adj[y][1]));
                        }
                    }
                }
            }
        }
        if(response.f == BIG) cout << "-1 -1";
        else cout << (response.f + 1) << " " << (response.s + 1);
        cout << "\n";
    }
}
