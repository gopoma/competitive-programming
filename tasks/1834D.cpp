...
void solve() {
    ...
    ll re = 0;
    {
        ...
        for(int i = 0; i < n; i++) {
            // forwards from L
            if(R[i] < Lmax) {
                re = max(re, 2LL * (R[i] - L[i] + 1));
            } else {
                re =  max(re, 2LL * (Lmax - L[i]));
            }
...

    {
        ...
        { // Coordinate Compression
            vl tot;
            for(int i = 0; i < n; i++) {
                tot.emplace_back(L[i]);
                tot.emplace_back(R[i]);
            }
            remDup(tot);
        ...
        for(auto& [L, R]: G) {
            // calc
            ll mn = st.query(forwards[L], forwards[R]);
            if(mn != BIG) {
                dbg(L, R, mn);
                re = max(re, 2LL * ((R - L + 1) - mn));
            }

            // upd
            st.upd(forwards[L], min(st.query(forwards[L], forwards[L]), R - L + 1));
        }
    }
    cout << re << "\n";
}
