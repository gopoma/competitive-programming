...
void solve() {
    ...
    V<vmi> pref_left(n, vmi(m));
    V<vmi> pref_up(n, vmi(m));
    V<vmi> pref_diagonal(n, vmi(m));
    V<vmi> dp(n, vmi(m));
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            if(i == 0 && j == 0) {
                dp[0][0]            = 1;
                pref_left[0][0]     = dp[0][0];
                pref_up[0][0]       = dp[0][0];
                pref_diagonal[0][0] = dp[0][0];
            } else {
                if(S[i][j] == '#') {
                    dp[i][j]            = 0;
                    pref_left[i][j]     = 0;
                    ...
                    //* up
                    if(0 <= i - 1)
                        dp[i][j] += pref_up[i - 1][j];
                    //* left
                    if(0 <= j - 1)
                        dp[i][j] += pref_left[i][j - 1];
                    //* upleft
                    if(0 <= i - 1 && 0 <= j - 1)
                        dp[i][j] += pref_diagonal[i - 1][j - 1];

                    pref_up[i][j]       = dp[i][j];
                    pref_left[i][j]     = dp[i][j];
                    pref_diagonal[i][j] = dp[i][j];
                    ...
                    //* left
                    if(0 <= j - 1)
                        pref_left[i][j]     += pref_left[i][j - 1];
                    //* upleft
                    if(0 <= i - 1 && 0 <= j - 1)
                        pref_diagonal[i][j] += pref_diagonal[i - 1][j - 1];
                }
            }
        }
    }

    mi response = dp[n - 1][m - 1];
    cout << response << "\n";
}
