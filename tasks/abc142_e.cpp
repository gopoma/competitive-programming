...
using Info = pair<ll, vl>; //* cost - boxes
bool vis[1000 + 1][(1 << 12) + 1];
ll  memo[1000 + 1][(1 << 12) + 1];
void solve() {
    ll n, m; cin >> n >> m;
    V<Info> info;
    rep(m) {
    ...
    auto dp = [&](auto&& self, int i, int mask) -> ll {
        if(i == m) {
        ...
        //* skip
        ckmin(ans, self(self, i + 1, mask));

        //* take
        int new_mask = mask;
        for(auto& id: info[i].s) {
            new_mask |= (1 << id);
        }
        ckmin(ans, self(self, i + 1, new_mask) + info[i].f);

        return memo[i][mask] = ans;
    };
    ll response = dp(dp, 0, 0);
    if(response >= BIG) response = -1;
    cout << response << "\n";
}
