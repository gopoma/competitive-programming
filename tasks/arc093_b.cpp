...
void solve() {
    ...
    //* A <= B
    vector response(100, vector<char>(100, '$'));
    ...
    for(int row = 0; row <= 100; row += 2) {
        for(int col = 0; col <= 48; col += 2) {
            blacks.eb(row, col);
        }

        for(int col = 51; col <= 99; col += 2) {
            whites.eb(row, col);
        }
    }

    for(int i = 0; i < B; i++) {
        auto [row, col] = blacks[i];
        response[row][col] = '#';
    }
    for(int i = 0; i < A; i++) {
        auto [row, col] = whites[i];
        response[row][col] = '.';
    }
