...
void solve() {
    vs S(3); for(auto& x: S) cin >> x;
    for(auto& x: S) dbg(x);
    vector<char> U; for(auto& it: S) for(auto& c: it) U.eb(c);
    remDup(U);
    dbg(U);
    if(sz(U) > 10) {
        cout << "UNSOLVABLE\n";
    ...
    do {
        reverse(all(p));
        vi forwards(26);
        for(int i = 0; i < sz(U); i++) forwards[U[i] - 'a'] = p[i];
        vs N(3);
        for(int i = 0; i < 3; i++) for(auto& c: S[i]) N[i].pb(char('0' + forwards[c - 'a']));
        bool ok = true;
        for(auto& it: N) ok &= (sz(it) == sz(ts(stoll(it))));
        ll N1 = stoll(N[0]);
        ll N2 = stoll(N[1]);
        ll N3 = stoll(N[2]);
        if(ok && N1 > 0 && N2 > 0 && N3 > 0 && (N1 + N2 == N3)) {
            for(auto& x: N) cout << x << "\n";
            return;
        }
        reverse(all(p));
