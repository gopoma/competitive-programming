...
void solve() {
    ...
    for(int src = 0; src < n; src++) {
        ...
        if(!ok && sz(adj[src]) >= 4) {
            ...
            #define CHECK { if(should_finish) { return; } }
            ...
            auto dfs = [&](auto&& self, int u, int par) -> void {
                ...
                for(auto& nxt: adj[u]) {
                    if(nxt != par && nxt == src) haveReturned = true;
                }
                ...
                if(haveReturned) {
                    ...
                    response.eb(src, extra.ft);
                    response.eb(src, extra.bk);
                    for(int j = 0; j < sz(cyc); j++) {
                        response.eb(cyc[j], cyc[(j + 1) % sz(cyc)]);
                    }
                    CHECK
                } else {
                    for(auto& nxt: adj[u]) {
