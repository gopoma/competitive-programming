...
void solve() {
    ...
    vl a(n);
    for(auto& x: a) cin >> x;
    for(auto& x: a) assert(-20 <= x && x <= 20);

    bool all_negs = true;
    for(auto& x: a) all_negs &= (x <= 0);
    vpi ops;
    if(all_negs) {
        for(int i = n - 1; i >= 1; i--) {
            ops.eb(mp(i - 1, i));
        ...
        assert(idx_positive != -1);
        assert(a[idx_positive] > 0);

        ll val = a[idx_positive];
        while(val <= 20) {
            ops.eb(mp(idx_positive, idx_positive));
            val *= 2LL;
        }

        for(int i = 0; i < n; i++) {
            if(a[i] < 0) {
                ops.eb(mp(i, idx_positive));
            }
        }
