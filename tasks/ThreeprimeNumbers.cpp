...
using E = mi;
bool vis[10001][11][11][2];
E   memo[10001][11][11][2];
void solve() {
    int cnt = 0;
    for(int x = 100; x <= 999; x++) {
        if(st.is_prime[x]) {
            ...
            str S = ts(x);
            assert(sz(S) == 3);
            good[int(S[0]-'0')][int(S[1]-'0')][int(S[2]-'0')] = true;
        }
    }
    dbg(cnt);
    ...
    //? last - last2 - current
    const int NONE = 10;
    function<E(int, int, int, bool)> dp = [&](int idx, int last, int last2, bool ok) -> E {
        if(idx == n) {
            return ok;
        }
        if(vis[idx][last][last2][ok]) return memo[idx][last][last2][ok];
        vis[idx][last][last2][ok] = true;

        E ans = 0;
        for(int d = ((idx == 0)? 1 : 0); d <= 9; d++) {
            if(idx >= 2) {
                bool current_ok = good[last][last2][d];
                ans += dp(idx + 1, last2, d, ok && current_ok);
            } else {
                ans += dp(idx + 1, last2, d, true);
            }
        }

        return memo[idx][last][last2][ok] = ans;
    };
    E ans = dp(0, NONE, NONE, true);
    dbg(ans);
    ps(ans);
}
