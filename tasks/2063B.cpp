...
void solve() {
    ...
    vl left, right, middle;
    for(int i = 0; i < l; i++) left.eb(a[i]);
    for(int i = l; i <= r; i++) middle.eb(a[i]);
    for(int i = r + 1; i < n; i++) right.eb(a[i]);

    auto work = [](vl side, vl mid) -> ll {
        vl tot;
        for(auto& x: side) tot.eb(x);
        for(auto& x: mid) tot.eb(x);
        sor(tot);
        ll ans = 0;
        for(int i = 0; i < sz(mid); i++) ans += tot[i];
        return ans;
    };

    ll ans = work(left, middle);
    ckmin(ans, work(right, middle));
    cout << ans << "\n";
}
