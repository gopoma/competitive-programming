...
using Info = tuple<char, ll, char, ll>;

ll brute(ll n, V<Info> info) {
    ll response = 0;
    for(int mask = 0; mask < (1 << n); mask++) {
        ...
        pl currentState = mp(1, 1);
        for(int i = 0; i < n; i++) {
            auto [op, a, op2, a2] = info[i];
            auto [left, right] = currentState;

            ll adi_left = 0;
            if(op == '+') adi_left += a;
            else adi_left += (a - 1LL) * left;

            ll adi_right = 0;
            if(op2 == '+') adi_right += a2;
            else adi_right += (a2 - 1LL) * right;

            if(takeLeft[i]) left += adi_left + adi_right;
            else right += adi_left + adi_right;

            currentState = mp(left, right);
        }

        ll local_response = currentState.f + currentState.s;
...
ll slv(ll n, V<Info> info) {
    vvl nxt(2, vl(n));
    for(ll i = 0; i < n; i++) {
        nxt[0][i] = nxt[1][i] = n;
        for(ll j = i + 1; j < n; j++) {
            auto [op, a, op2, a2] = info[j];

            if(op == 'x') ckmin(nxt[0][i], j);
            if(op2 == 'x') ckmin(nxt[1][i], j);
        }
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        //* if(i + 1 < n) dbg(suff[0][i + 1], suff[1][i + 1]);
        ll id_left = nxt[0][i];
        ll id_right = nxt[1][i];
        ll val_left = 0;
        ll val_right = 0;
        while(true) {
            if(id_left == n || id_right == n) break;
            if(id_left != id_right) break;

            auto [op3, a3, op4, a4] = info[id_left];
            //* dbg(op3, a3, op4, a4);
            ...
            id_left = nxt[0][id_left];
            id_right = nxt[1][id_right];
        }
        //* dbg(id_left, id_right);

        if(id_right == n || id_left < id_right || (id_left == id_right && val_left > val_right)) {
            //* dbg("left");
            left += adi_left + adi_right;
        } else {
            //* dbg("right");
            right += adi_left + adi_right;
        }

        currentState = mp(left, right);
    }
    //* dbg(currentState);
    ll response = currentState.f + currentState.s;
    return response;
}
