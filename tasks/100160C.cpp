...
void solve() {
    ...
    auto ok = [&](ll go) {
        ...
        for(ll i = 0; i < 51; i++) {
            if((i + 1LL) % x == 0) {
                ll groups = fdiv(go, 1LL << i);
                ll groups_with_one = fdiv(groups, 2LL);

                count += (1LL << i) * groups_with_one;

                ll rem = go % (1LL << i);
                if(groups % 2 == 1) count += rem;
            }
        }
        return count <= k;
    };
    ...
    ll right = 1e15 + 5; // always bad
    while(left + 1 < right) {
        ll middle = fdiv(left + right, 2LL);

        if(ok(middle)) left = middle;
        else right = middle;
    }
    ll ans = left;
    //? return ans
