...
void solve() {
    ...
    {
        for(int i = 0; i < n; i++) {
            b[i] = mp(a[i] + min(i + 1LL, abs((n + 1LL) - (i + 1LL))), i);
        }
        sor(b);
        for(int i = 0; i < n; i++) {
            where[b[i].s] = i;
    ...
    vl pref(n);
    {
        for(int i = 0; i < n; i++) pref[i] = b[i].f;
    ...
    ll res = 0;
    for(int i = 0; i < n; i++) {
        ll current_cost = a[i] + i + 1;
        ll tot = C;
        if(current_cost > tot) continue;
        tot -= current_cost;
        dbg(i, a[i], tot);

        auto check = [&](ll middle) -> bool {
            ll S = 0;
            if(where[i] > middle) S = pref[middle - 1];
            else S = pref[middle] - b[where[i]].f;
            dbg(middle, S, tot);
            return S <= tot;
        };
