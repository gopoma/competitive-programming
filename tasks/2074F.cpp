...
void solve() {
    ...
    auto work = [&](ll offset, ll L, ll R) -> ll {

        ll oL = -1;
        {
            auto check = [&](ll middle) -> bool {
                return (0LL + offset * middle) >= L;
            };
        ...
        if(oL < L || oR > R || oL > oR) return 0LL;
        else return fdiv(oR - oL, offset);
    };
    ...
    for(ll bit = 30; bit >= 0; bit--) {
        ll offset = (1LL << bit);
        ll cnt = work(offset, l1, r1);
        ll cnt2 = work(offset, l2, r2);
        dbg(offset, bit, cnt, cnt2);

        response += cnt * cnt2;
        response -= 4LL * lstcnt * lstcnt2;
