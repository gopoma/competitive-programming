...
void solve() {
    ...
    for(int i = 0; i < N; i++) {
        ...
        int current = A[i];
        while(st.sp[current] != 0) {
            int val = st.sp[current];
            int count = 0;
            while(st.sp[current] == val) {
                count++;
                current /= st.sp[current];
            }
            if(already[val]) {
                response = response + mi(count) * response * F.invs[hist[val] + 1];
            } else {
                response = response * mi(count + 1);
            }
        }
        cout << response << " ";
    }
