...
void solve() {
    ...
    rep(m) {
        int u, v, color; cin >> u >> v >> color; u--; v--;
        dbg(u, v, color);
    ...
    while(!pq.empty()) {
        const auto [cdist, node, last_color] = pq.top();
        pq.pop();
        ...
        for(auto [neighbor, color]: adj[node]) {
            ...
            int inc = last_color == color? 0 : 1;
            if(cdist + inc < dist[neighbor]) {
                dist[neighbor] = cdist + inc;
                pq.push({dist[neighbor], neighbor, color});
            }
        }
    }
