...
void solve() {
    ...
    vb inComponent(n + 1, false);
    {
        function<void(int)> dfs = [&](int src) {
            if(inComponent[src]) return;

            inComponent[src] = true;
    ...
    TopoSort xd; xd.init(n + 1);
    for(int u = 1; u <= n; u++) {
        each(xx, adj[u]) {
            if(inComponent[u] && inComponent[xx.f]) {
                xd.ae(u, xx.f);
            }
        }
    }

    bool hasCycle = !xd.sort();
    if(hasCycle) ps("YES");
    else {
        //? Graph has no Cycle
        vb vis(n + 1, false);
