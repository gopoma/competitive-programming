...
void solve() {
    ...
    function<ll(ll)> solve = [&](ll x) {
        if(x < 0)       return BIG;
        else if(x == 0) return 0LL;

        if(vis[x]) return dp[x];
        vis[x] = true;

        ll res = 1LL + min(solve(x - 4), solve(x - 7));
        return dp[x] = res;
    };
    ...
    if(count >= BIG) {
        ...
        for(ll how_7 = count; how_7 >= 0; how_7--) {
            ll how_4 = count - how_7;

            if(4LL * how_4 + 7LL * how_7 == n) {
                str ans = string(how_4, '4') + string(how_7, '7');
                ps(ans);
                return;
            }
        }
