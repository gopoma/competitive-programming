...
mi slv(ll n, ll x, vl a) {
    if(n == 1) return mi(1);
    else {
        ll sum = accumulate(all(a), 0LL);

        ll mn = BIG;
        each(e, a) ckmin(mn, sum - e);
        chk(mn != BIG);
        ...
        ll add = 0;
        {
            ...
            for(auto [exponent, cnt]: exponents) {
                ll groups_nxt = fdiv(cnt, x);
                ll remain = cnt % x;
                ...
                if(remain) {
                    ...
                    exponents[exponent + 1] += groups_nxt;
                }
            }
        }

        mi ans = pow(mi(x), min(sum, mn + add));
        return ans;
    }
}
