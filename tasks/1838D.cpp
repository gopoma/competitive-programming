...
void solve() {
    ...
    auto update_open_closed = [&](int i) -> void {
        for(int d = -2; d <= +2; d++) {
            int j = i + d;
            if(0 <= j && j < n) {
                open.erase(j);
    ...
    auto single = [&](int i) -> ll {
        return st.query(i, i).sum;
    };
    ...
    auto answer = [&]() -> bool {
        ...
        if(!open.empty()) {
            ...
            ll w = single(i + 1);
            if(v < 0 || w < 0) {
                dbg("Add Bad Break", i, v, i + 1, w);
                return false;
            }

            ll mn = st.query(i, n - 1).mn;
            dbg("Add", mn);
            if(mn < 0) {
                mn = custom_abs(mn);
                ll steps = cdiv(mn, 2);
                ...
                add_id = i;
                add_amount = 2LL * steps;
                dbg("Should add", add_id + 1, add_amount);
                st.upd(add_id, n - 1, add_amount);
            }
        }
        ...
        if(!closed.empty()) {
            auto i = *closed.rbegin();
            ll v = single(i);
            ll w = single(i + 1);
            ll lst = single(n - 1);

            if(v >= 0 && w >= 0 && lst > 0) {
                ll steps = fdiv(min(v, w), 2LL);
                ll need = fdiv(lst, 2LL);

                rem_id = i;
                rem_amount = 2LL * min(need, steps);
                dbg("Should rem", rem_id + 1, rem_amount);

                st.upd(rem_id, n - 1, -rem_amount);
            }
        }

        auto node = st.query(0, n - 1);
        bool ok = (node.mn >= 0) && (single(n - 1) == 0);
