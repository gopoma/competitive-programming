...
tcT> struct SegTree { // cmb(ID,b) = b
    ...
    T ID{0LL}; T cmb(T a, T b) { return a+b; }
	int n; V<T> seg;
...
void solve() {
    ...
    SegTree<ll> odds; odds.init(n + 5);
    SegTree<ll> evens; evens.init(n + 5);
    ...
    rep(m) {
        ...
        if(type == 0) {
            ...
            ll ans = -BIG;
            if(l % 2 == 1) {
                ans = odds.query(l, r) - evens.query(l, r);
            } else {
                ans = evens.query(l, r) - odds.query(l, r);
            }
            assert(ans != -BIG);
