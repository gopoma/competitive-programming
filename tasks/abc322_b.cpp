...
void solve() {
    ...
    bool isPrefix = true;
    for(int i = 0; i < N; i++) isPrefix &= (S[i] == T[i]);

    bool isSuffix = true;
    reverse(all(S));
    reverse(all(T));
    for(int i = 0; i < N; i++) isSuffix &= (S[i] == T[i]);

    if(isPrefix && isSuffix) cout << "0\n";
    else if(isPrefix && !isSuffix) cout << "1\n";
    else if(!isPrefix && isSuffix) cout << "2\n";
    else cout << "3\n";
}
