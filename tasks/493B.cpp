...
void solve() {
    ...
    ll sum_fs = accumulate(all(fs), 0LL);
    ll sum_sd = accumulate(all(sd), 0LL);
    assert(sum_fs >= 0);
    assert(sum_sd >= 0);

    if(sum_fs > sum_sd) cout << "first\n";
    else if(sum_sd > sum_fs) cout << "second\n";
    else {
        if(fs == sd) {
            if(a.back() > 0) cout << "first\n";
            else if(a.back() < 0) cout << "second\n";
            else assert(false);
        } else {
            for(int i = 0; i < min(sz(fs), sz(sd)); i++) {
                dbg(fs[i], sd[i]);
                if(fs[i] > sd[i]) {
                    cout << "first\n";
                    return;
            ...
            if(sz(fs) > sz(sd)) cout << "first\n";
            else if(sz(sd) > sz(fs)) cout << "second\n";
            else assert(false);
        }
    }
}
