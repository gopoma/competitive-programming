        ...
        for(int y0 = 0; y0 < m; y0++) {
            for(int x1 = 0; x1 < n; x1++) {
                for(int y1 = 0; y1 < m; y1++) {
                    ...
                    ll d = abs(x0 - x1) + abs(y0 - y1);
                    ckmin(ans, C * d + a[x0][y0] + a[x1][y1]);
                }
            }
        }
    }
        ...
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(i == 0 && j == 0) {
                    ...
                    ll val = min(dp[i][j] + C, a[i][j] + C);
                    if(j + 1 < m) ckmin(dp[i][j + 1], val);
                    if(i + 1 < n) ckmin(dp[i + 1][j], val);
                }
            }
        }
        return response;
...
int main() {
    ...
    while(0) {
        ...
        ll n = rng_ll(2, 10);
        ll m = rng_ll(2, 10);
        ll C = rng_ll(1, ll(1e9));
        vvl a(n, vl(m)); for(auto& vec: a) for(auto& x: vec) x = rng_ll(1, ll(1e9));
        dbg(n, m, C);
        ll ans = brute(n, m, C, a);
        ll greedy = slv(n, m, C, a);
        dbg(ans, greedy);
        chk(ans == greedy);
    }
