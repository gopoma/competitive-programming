...
int main() {
    ...
    while(0) {
        ...
        vi b_ans = get_b(n, a, k);
        // dbg(b_ans);
        dbg("Greedy");
        auto [k_gre, a_gre] = slv(n, b_ans);
        dbg("/Greedy");
        vi b_gre = get_b(n, a, k);
        dbg(k_gre, a_gre);
        // dbg(b_ans);
        //dbg(b_gre);
        assert(b_ans == b_gre);
