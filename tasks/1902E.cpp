 ...
 */
 ...
 struct vertex {
    ...
    vertex(char a) : alphabet(a), exist(false), cnt(0) {
    ...
    void insert(string word) { // insert a word into trie
        ...
        for (int i = 0; i < (int)word.size(); ++i) { // O(n)
            ...
            if (cur->child[alphaNum] == NULL) {
                // add new branch if NULL
                cur->child[alphaNum] = new vertex(word[i]);
            }
            cur->child[alphaNum]->cnt++;
            cur = cur->child[alphaNum];
        }
        cur->exist = true;
    }

    ll query(string word) { // true if word in trie
        vertex *cur = root;
...
void solve() {
    ...
    ll response = 0;
    for(auto& x: S) {
        response += n * ll(sz(x)) + sum;
        reverse(all(x));
        response -= 2LL * st.query(x);
    }
    cout << response << "\n";
}
