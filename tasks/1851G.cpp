...
void solve() {
    ...
    vpi edges(m); for(auto& [u, v]: edges) { cin >> u >> v; u--; v--; }
    int q; cin >> q;
    using Query = tuple<int, int, int>;
    V<Query> queries(q);
    for(auto& [a, b, e]: queries) { cin >> a >> b >> e; a--; b--; }
    dbg(n, m);
    dbg(h);
    dbg(edges);
    dbg(q);
    ...
    //* EDGE:  val - type - u - v
    //* CHECK: mx  - type - idQuery
    V<Edge> E;
    const int EDGE = 0;
    const int CHECK = 1;
    for(auto& [u, v]: edges) {
        E.eb(max(h[u], h[v]), EDGE, u, v);
    }
    for(int i = 0; i < q; i++) {
        auto [a, b, e] = queries[i];
        E.eb(h[a] + e, CHECK, i, 79);
    }
    sor(E);
    DSU dsu; dsu.init(n);
    int currentMx = 0;
    for(auto& edge: E) {
        int __type = get<1>(edge);
        if(__type == EDGE) {
            auto [val, type, u, v] = edge;
            dbg("EDGE", val, type, u + 1, v + 1);
            if(dsu.unite(u, v)) {
                ckmax(currentMx, val);
            }
        } else {
            auto [mx, type, idQuery, _] = edge;
            auto [u, v, e] = queries[idQuery];
            dbg("CHECK", mx, type, idQuery, u + 1, v + 1);
            if(dsu.sameSet(u, v)) {
                response[idQuery] = true;
            }
        }
    }
