...
ll brute(ll n, vl a) {
    ...
    for(int mask = 0; mask < (1 << n); mask++) {
        ll local_response = -BIG;

        ll other_sum = 0;
        ll cur_sum = 0;
        for(int bit = 0; bit < n; bit++) {
            if(mask & (1 << bit)) {
                ckmax(local_response, cur_sum);
                cur_sum = 0;
                other_sum += a[bit];
            } else {
                cur_sum += a[bit];
        ...
        ckmax(local_response, cur_sum);
        ckmax(local_response, other_sum);
...
ll slv(ll n, vl a) {
    ...
    auto check = [&](ll middle) -> bool {
        if(MX > middle) return false;

        SegTree<ll> st; st.init(n + 5); st.upd(n, a[n - 1]);
        vl dp(n + 2, BIG); dp[n] = 0; dp[n + 1] = 0;
        dp[n - 1] = 0; st.upd(n - 1, a[n - 1] + dp[n]);
        for(int i = n - 2; i >= 0; i--) {
            ll prv = (i == 0)? 0 : pref[i - 1];
            ll nxt = upper_bound(all(pref), prv + middle) - pref.begin();

            ll res = st.query(i + 1, nxt);
            dp[i] = min(a[i] + dp[i + 1], res);
            if(nxt < n) {
                ckmin(dp[i], a[nxt] + dp[nxt + 1]);
            } else {
                ckmin(dp[i], dp[nxt]);
            }
            st.upd(i, a[i] + dp[i + 1]);
...
int main() {
    ...
    while(0) {
        RAYA;
        ll n = rng_ll(1, 16);
        vl a(n); for(auto& x: a) x = rng_ll(1, ll(10));
        dbg(n);
        dbg(a);
        ll ans = brute(n, a);
        ll greedy = slv(n, a);
