...
void solve() {
    ...
    {
        vector<int> init;
        for(int i = 0; i < n; i++) init.emplace_back(i);
        ll __cnt = query(init);
    ...
    for(int i = 0; i + 1 < n; i += 2) {
        ...
        ll cnt = query(cur);
        assert(G.count(cnt));
        for(auto& c: G[cnt]) re.push_back(c);
    }
    if(n & 1) {
        vector<int> cur{idOpen, n - 1};
        ll cnt = query(cur);
        if(cnt == 1) re.push_back(')');
        else re.push_back('(');
    }
    answer(re);
}
