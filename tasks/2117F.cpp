...
void solve() {
    ...
    vector<int> leafs;
    for(int u = 1; u < n; u++) {
        if(deg[u] == 1) leafs.emplace_back(u);
    }
    ...
    auto get = [&](int node) -> vector<int> {
        ...
        assert(res.back() == root);
        reverse(all(res));
        return res;
    };

    vector<int> A = get(leafs.front());
    vector<int> B = get(leafs.back());
    ...
    auto get2 = [&](int cnt, int sum) -> mi {
        int twos = sum - cnt;
        return F.C(cnt, twos);
    };

    mi res = pow(mi(2), n);
    mi factor = pow(mi(2), i);
    dbg(int(res), int(factor));
    for(int sum = 1; sum <= 2 * n + 5; sum++) {
        bool ok = (sz_A <= sum && sum <= 2 * sz_A);
        bool ok2 = (sz_B <= sum && sum <= 2 * sz_B);
        if(ok && ok2) {
            mi for_A = get2(sz_A, sum);
            mi for_B = get2(sz_B, sum);

            dbg(int(sum), int(for_A), int(for_B));

            res -= factor * for_A * for_B;
        }
    }
    cout << res << "\n";
}
