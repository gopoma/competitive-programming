...
void slv(int n, vi p, bool isDebug) {
    int total_cost = 0;
    ...
    auto answer = [&](int i) -> void {
        if(isDebug) {
            bool ok = p[i] == n;
            dbg(ok, "Given", i, p[i], p);
            assert(ok);

            dbg(total_cost, 5 * n * n);
            assert(total_cost <= 5 * n * n);
        } else {
            cout << "! " << (i + 1) << endl;
        }
    };
    ...
    auto work = [&](auto&& self, int l, int r) -> int {
        ...
        int id_mx_left = self(self, l, mid);
        int id_mx_right = self(self, mid + 1, r);

        int invs = ask(l, id_mx_right);
        int invs2 = ask(l, id_mx_right - 1);
        int delta = invs - invs2;

        if(delta == 0) return id_mx_right;
        return id_mx_left;
    };

    int response = work(work, 0, n - 1);
    answer(response);
}
