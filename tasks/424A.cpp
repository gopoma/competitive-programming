...
void solve() {
    ...
    int standing = count_if(all(S), [](const char& c) { return c == 'X'; });
    int sitting = count_if(all(S), [](const char& c) { return c == 'x'; });

    char tar = (standing > sitting)? 'X' : 'x';
    int need = abs(standing - sitting) / 2;
    ps(need);
