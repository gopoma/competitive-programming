...
struct vertex {
    ...
    vertex(int a) : alphabet(a), exist(false) {
...
public:
    Trie() { root = new vertex(-1); }

    void insert(vi word) { // insert a word into trie
        vertex *cur = root;
    ...
    int work(vi word) { // true if word in trie
        vertex *cur = root;
        int response = 0;
...
void solve() {
    ...
    for(auto& p: P) {
        vi where(m); for(int i = 0; i < m; i++) where[p[i]] = i;
        vi tar(m); for(int i = 0; i < m; i++) tar[i] = where[i];
        dbg(p, tar);
        st.insert(tar);
    }
    ...
    vi response(n);
    for(int i = 0; i < n; i++) response[i] = st.work(P[i]);
