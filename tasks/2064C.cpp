...
ll slv(ll n, vl a) {
    ...
    vl pos(N), neg(N);
    for(int i = 0; i < N; i++) {
        auto [csgn, csum] = groups[i];
        if(csgn == +1) pos[i] = csum;
        else neg[i] = csum;
    }
    for(int i = 1; i < N; i++) {
        pos[i] += pos[i - 1];
    ...
    ll ans = -BIG;
    for(int i = 0; i < N; i++) {
        auto [csgn, _] = groups[i];
        ll local_ans = 0;
        if(csgn == +1) {
            local_ans += query(pos, 0, i) + query(neg, i + 1, N - 1);
        } else {
            local_ans += query(pos, 0, i - 1) + query(neg, i, N - 1);
        }
        ckmax(ans, local_ans);
    }
