...
int get(str S) {
    ...
    for(int i = 0; i < n; i++) {
        for(int j = i; j < n; j++) {
            ...
            ckmax(mx, abs(cnt[0] - cnt[1]));
        }
    }
    return mx;
}

str brute(int n, int m, int k) {
    str response;
...
str slv(int _n, int _m, int k) {
    auto _get = [&](str& S, char c) -> int {
        const int n = sz(S);
        vi a(n);
        for(int i = 0; i < n; i++) {
            if(S[i] == c) a[i] = +1;
        ...
        for(int i = 0; i < n; i++) {
            pref += a[i];
            ckmax(mx, pref - mn_pref);
            ckmin(mn_pref, pref);
        }
        return mx;
    };
    auto check = [&] (str& S) -> bool {
        dbg(S, _get(S, '0'), _get(S, '1'));
        return max(_get(S, '0'), _get(S, '1')) == k;
    };
    auto get = [&](int n, int m, bool invert) -> str {
        chk(n >= k);
        str S = str(k, '0');
        n -= k;
        while(n > 0 && m > 0) {
    ...
    str response;
    if(_n >= k) response = get(_n, _m, false);
    if(response.empty() && _m >= k) response = get(_m, _n, true);
    return response;
}

void solve() {
    int n, m, k; cin >> n >> m >> k;
    str response = slv(n, m, k);
...
int main() {
    ...
    while(0) {
        ...
        dbg(n, m, k);
        str ans = brute(n, m, k);
        str greedy = slv(n, m, k);
        dbg(ans, greedy);
        if(ans.empty()) {
            chk(greedy.empty());
        } else {
            chk(get(greedy) == k);
            vi cnt(2); for(auto& c: greedy) cnt[c - '0']++;
            chk(cnt[0] == n && cnt[1] == m);
        }
    }
