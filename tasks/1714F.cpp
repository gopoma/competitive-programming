...
void solve() {
    ...
    auto complete = [&](vi& a) -> void {
        vi stk; for(int x = 4; x <= n; x++) stk.eb(x);
        for(auto& x: a) {
            if(x == -1) {
                x = stk.bk;
                stk.pop_back();
    ...
    auto printResponse = [&](vpi& res) -> void {
        chk(sz(res) == n - 1);
        cout << "YES\n";
        for(auto& [u, v]: res) {
            cout << u << " " << v << " " << "\n";
    ...
    //* left
    {
        ...
        for(int location_3 = where[2] + 1; location_3 < n; location_3++) {
            int new_d23 = abs(location_3 - where[2]);
            int new_d31 = abs(location_3 - where[1]);

            if(new_d23 == d23 && new_d31 == d31) {
                a[location_3] = 3;
                complete(a);
                vpi res = getTree(a);
                printResponse(res);
    ...
    { //* right
        ...
        for(int location_3 = where[1] - 1; location_3 >= 0; location_3--) {
            int new_d23 = abs(location_3 - where[2]);
            int new_d31 = abs(location_3 - where[1]);
    ...
    { //* middle
        ...
        vi where(3);
        where[1] = 0; where[2] = sz(a) - 1;

        int mx_adi = n - (d12 + 1);
        for(int location_3 = 1; location_3 + 1 < sz(a); location_3++) {
            int new_d23 = abs(location_3 - where[2]);
            int new_d31 = abs(location_3 - where[1]);
            if(new_d23 <= d23 && new_d31 <= d31) {
                int fadi23 = d23 - new_d23;
                int fadi31 = d31 - new_d31;

                if(fadi23 == fadi31 && fadi23 <= mx_adi) {
                    vi stk;
                    for(int x = 4; x <= n; x++)
                        stk.eb(x);

                    for(int j = 1; j + 1 < sz(a); j++) {
                        if(j == location_3) continue;
                        else {
                            a[j] = stk.bk;
                            stk.pop_back();
                        }
