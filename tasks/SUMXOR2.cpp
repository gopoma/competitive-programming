...
void solve() {
    ...
    for(int block = 1; block <= N; block++) {
        for(int bit = 0; bit < 32; bit++) {
            for(int zeros = 0; zeros <= block; zeros++) {
                int ones = block - zeros;

                int have_ones = dp[N - 1][bit];
                int have_zeros = N - have_ones;

                if(have_ones >= ones && have_zeros >= zeros) {
                    if(ones & 1) {
                        dbg(block, bit, zeros, ones, have_ones, have_zeros);
                        ans[block] += mi(1LL << bit) * F.C(have_zeros, zeros) * F.C(have_ones, ones);
                    }
                }
            }
        }
    ...
    auto query = [&](int L, int R) -> mi {
        mi sum = ans[R];
        if(0 <= L - 1) sum -= ans[L - 1];
        return sum;
    };
