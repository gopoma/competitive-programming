...
bool vis[64 + 1][64 + 1][64 + 1];
ll  memo[64 + 1][64 + 1][64 + 1];

void precompute() {
    memset(vis, false, sizeof(vis));
    auto dp = [&](auto&& self, int bit, int x, int y) -> ll {
        if(x < 0 || y < 0) return BIG;
        if(bit > MXBIT) return BIG;
        if(x == 0 && y == 0) return 0;

        if(vis[bit][x][y]) return memo[bit][x][y];
        vis[bit][x][y] = true;

        ll ans = BIG;
        //* not take
        ckmin(ans, self(self, bit + 1, x, y));
        //* take for x
        ckmin(ans, self(self, bit + 1, x - bit, y) | (1LL << bit));
        //* take for y
        ckmin(ans, self(self, bit + 1, x, y - bit) | (1LL << bit));

        return memo[bit][x][y] = ans;
    };
...
ll slv(ll x, ll y) {
    ll response = BIG;
    for(ll offset_x = 0; offset_x < MXBIT; offset_x++) {
        for(ll offset_y = 0; offset_y < MXBIT; offset_y++) {
            if((x >> offset_x) == (y >> offset_y)) {
                ckmin(response, memo[0][offset_x][offset_y]);
            }
        }
    }
    return response;
...
ll brute(ll x, ll y) {
    ...
    auto getBitRepresentation = [&](ll mask) -> vl {
        vl res;
        for(ll bit = 0; bit <= MXBIT; bit++) {
            if(mask & (1LL << bit)) res.eb(1);
            else res.eb(0);
        }
        res.eb(0);
        return res;
    ...
    vl X = getBitRepresentation(x);
    vl Y = getBitRepresentation(y);
    //* dbg(x, X);
    //* dbg(y, Y);



    auto getValueRepresentation = [&](int start, vl& arr) -> ll {
        ll res = 0;
        ll pot = 1;
    ...
    auto check = [&](int i, int j) -> bool {
        ll xx = getValueRepresentation(i, X);
        ll yy = getValueRepresentation(j, Y);
        return (xx == yy);
    };

    vvb good(sz(X), vb(sz(Y)));
    ...
    for(int bit = MXBIT + 1; bit >= 0; bit--) {
        for(int i = 0; i < sz(X); i++) {
            for(int j = 0; j < sz(Y); j++) {
                ...
                dp[bit][i][j] = BIG;
                //* not take
                ckmin(dp[bit][i][j], dp[bit + 1][i][j]);
                //* for i
                ckmin(dp[bit][i][j], dp[bit + 1][min(sz(X) - 1, i + bit)][j] + p2[bit]);
                //* for j
                ckmin(dp[bit][i][j], dp[bit + 1][i][min(sz(Y) - 1, j + bit)] + p2[bit]);
            }
        }
    }

    ll response = dp[1][0][0];
    return response;
}
...
int main() {
    ...
    //? Stress Testing
    while(0) {
        ll x = rng_ll(ll(0), ll(1e17));
        ll y = rng_ll(ll(0), ll(1e17));
