...
void solve() {
    ...
    rep(n) {
        ...
        HashRange g_h, w_h;
        g_h.add(g); w_h.add(w);
        G.eb(g_h.hash(0, sz(g) - 1));
        W.eb(w_h.hash(0, sz(w) - 1));
    }
    // dbg(n);
    // dbg(G);
    // dbg(W);
    ...
    vvb  vis(n, vb(1 << n));
    vvi memo(n, vi(1 << n));
    auto dp = [&](auto&& self, int src, int mask) -> int {
        if(__builtin_popcount(mask) > 0 && src == 0) return 0;
        if(vis[src][mask]) return memo[src][mask];
        vis[src][mask] = true;
        int ans = -INF;
        for(int nxt = 0; nxt < n; nxt++) {
            if(adj[src][nxt] && !bool(mask & (1 << nxt))) {
                ckmax(ans, self(self, nxt, mask | (1 << nxt)) + (nxt != 0));
            }
        }
        return memo[src][mask] = ans;
    };
    int d = dp(dp, 0, 0);
    n--;
    int response = n - d;
