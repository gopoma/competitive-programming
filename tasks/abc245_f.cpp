...
void solve() {
    ...
    map<int, vi> groups;
    for(int u = 0; u < n; u++) groups[G.comp[u]].eb(u);
    ...
    for(auto& [id, group]: groups) {
        for(auto& u: group) {
            dsu.unite(u, group.ft);
        }
        if(sz(group) > 1) {
            start.eb(dsu.get(group.ft));
            for(auto& u: group) {
                isGood[u] = true;
            }
        }
    ...
    auto dfs = [&](auto&& self, int src) -> void {
        ...
        vis[src] = true;
        isGood[src] = true;
        for(auto& nxt: adj[src]) self(self, nxt);
    };
    for(auto& u: start) dfs(dfs, u);

    int ans = 0;
    for(int u = 0; u < n; u++) ans += isGood[u];
    dbg(ans);
    cout << ans << "\n";
}
