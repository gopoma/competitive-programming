...
void solve() {
    ...
    V<pair<pair<int, int>, int>> prices(n);
    for(int i = 0; i < n; i++) {
        int a, g;
        cin >> a >> g;

        prices[i] = mp(mp(a, g), i);
    }
    sor(prices);
    // for(auto& it: prices) DBG2(it.first, it.second);

    vector<char> res(n);
    int A = 0, G = 0;
    for(int i = 0; i < n; i++) {
        auto& [a, g] = prices[i].first;
        if(abs((A + a) - G) <= 500) {
            A += a;
            res[prices[i].second] = 'A';
        } else {
            G += g;
            res[prices[i].second] = 'G';
        }
    }
    assert(abs(A - G) <= 500);
