...
void solve() {
    ...
    auto dp = [&](auto&& self, int i, int j) -> int {
        ...
        if(i == n) return memo[i][j] = self(self, i, j + 1) + 1;
        if(j == m) return memo[i][j] = self(self, i + 1, j) + 1;

        int ans = INF;
        ckmin(ans, self(self, i + 1, j) + 1);
        ckmin(ans, self(self, i, j + 1) + 1);
        ckmin(ans, self(self, i + 1, j + 1) + (a[i] != b[j]));
