...
void solve() {
    ...
    for(int a = 0; a <= N + 1; a++) for(int b = 0; b <= N + 1; b++) for(int c = 0; c < 2; c++) vis[a][b][c] = false;
    auto dp = [&](auto&& self, int i, int i_lst, bool isFTurn) -> ll {
        if(i == N) {
            ll lst = a[i_lst];
            return abs(a[i] - lst);
        }
        if(vis[i][i_lst][isFTurn]) return memo[i][i_lst][isFTurn];
        vis[i][i_lst][isFTurn] = true;
        ll ans = -1;
        if(isFTurn) {
            ans = -BIG;
            ckmax(ans, self(self, i + 1, i_lst, isFTurn));
            ckmax(ans, self(self, i + 1, i, !isFTurn));
        } else {
            ans = BIG;
            ckmin(ans, self(self, i + 1, i_lst, isFTurn));
            ckmin(ans, self(self, i + 1, i, !isFTurn));
        }
        return memo[i][i_lst][isFTurn] = ans;
    };
    ll ans = dp(dp, 1, 0, true);
    dbg(ans);
    cout << ans << "\n";
}
