...
void solve() {
    ...
    vector<char> color(n);
    vector<char> current_color(n);
    color[0] = '0';
    current_color[0] = '0';
    for(int i = 1; i < n; i++) {
        if(S[i - 1] <= S[i]) {
            color[i] = current_color[i - 1];
            current_color[i] = color[i];
        } else {
            color[i] = (current_color[i - 1] == '0')? '1' : '0';
            current_color[i] = color[i];

            int j = i;
            while(0 <= j - 1 && S[j - 1] > S[j]) {
                if(current_color[j - 1] == current_color[j]) {
                    cout << "NO\n";
                    return;
                }
                swap(S[j - 1], S[j]);
                swap(current_color[j - 1], current_color[j]);
                j--;
            }
        }
    }
    ...
    cout << "YES\n";
    str response; for(auto& c: color) response.pb(c);
    dbg(response);
    cout << response << "\n";
}
