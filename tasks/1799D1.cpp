...
void solve() {
    ll n, tot_programs; cin >> n >> tot_programs;
    vl a(n + 1);
    for(int i = 1; i <= n; i++) cin >> a[i];

    vl cold(tot_programs + 1);
    for(int i = 1; i <= tot_programs; i++) cin >> cold[i];

    vl hot(tot_programs + 1);
    for(int i = 1; i <= tot_programs; i++) cin >> hot[i];

    vector dp(n + 1, vector<ll>(2, BIG));
    vector nxt_dp(n + 1, vector<ll>(2, BIG));
    for(int i = n + 1; i >= 2; i--) {
        for(int lst = 0; lst <= n; lst++) {
            for(int lstWasA = 0; lstWasA < 2; lstWasA++) {
                if(i == n + 1) {
                    dp[lst][lstWasA] = 0;
                } else {
                    //* choose A
                    ...
                    {
                        if(lstWasA) {
                            ckmin(dp[lst][lstWasA], nxt_dp[i - 1][false] + ((a[i] == a[lst])? hot[a[i]] : cold[a[i]]));
                        } else {
                            ckmin(dp[lst][lstWasA], nxt_dp[lst][false]   + ((a[i] == a[i - 1])? hot[a[i]] : cold[a[i]]));
                        }
                    }
                    ...
                    {
                        if(lstWasA) {
                            ckmin(dp[lst][lstWasA], nxt_dp[lst][true]    + ((a[i] == a[i - 1])? hot[a[i]] : cold[a[i]]));
                        } else {
                            ckmin(dp[lst][lstWasA], nxt_dp[i - 1][true]  + ((a[i] == a[lst])? hot[a[i]] : cold[a[i]]));
                        }
                    }
                }
            }
    ...
    ll response = nxt_dp[0][false] + cold[a[1]];
    dbg(response);
    cout << response << "\n";
}
