...
void solve() {
    ...
    auto good = [&](int l, int d, int s) {
        ...
        for(auto& c: text) {
            ...
            if(isdigit(c)) need_digits++;
            else if(signs.count(c)) need_signs++;
            else need_letters++;
        }

        return (need_letters <= l && need_digits <= d && need_signs <= s);
    };

    bool ok = false;
    do {
        ok |= good(colors[0], colors[1], colors[2]);
