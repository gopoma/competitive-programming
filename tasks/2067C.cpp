...
ll brute(ll n) {
    map<ll, bool> vis; vis[n] = true;
    map<ll, int> dist; dist[n] = 0;
    deque<ll> q; q.eb(n);

    auto check = [&](ll x) -> bool {
        str S = ts(x);
        for(auto& c: S) if(c == '7') return true;
        return false;
    };
...
ll slv(ll n) {
    ...
    for(ll times = 0; times <= 9; times++) {
        for(int i = 0; i < 25; i++)
            for(int j = 0; j < 10; j++)
                for(int k = 0; k < 10; k++)
                    vis[i][j][k] = false;
        auto dp = [&](auto&& self, int i, int j, int carry) -> bool {
            if(i == sz(digits)) {
                return check(carry);
            }
            ...
            bool ans = false;
            for(int k = 0; k <= j; k++) {
                ll d = digits[i] + 9 * k + carry;
                ans |= self(self, i + 1, k, d / 10) || check(d);
            }
            return memo[i][j][carry] = ans;
        };
        bool ok = dp(dp, 0, times, 0);
        if(ok) {
            return times;
        }
    }
...
int main() {
    ...
    while(0) {
        RAYA;
        ll n = rng_ll(ll(1e8), ll(1e9) + 5000);
        dbg(n);
        ll ans = brute(n);
        ll greedy = slv(n);
        dbg(ans, greedy);
