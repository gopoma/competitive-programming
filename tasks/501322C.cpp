...
//?    auto right = upper_bound(all(go), R);
//?    auto left = lower_bound(all(go), L);
//?
//?    ll count = right - left;
//?    ps(count);
}
...
void precompute() {
    ...
    function<void(str)> gen = [&](str&& current) {
        if(sz(current) > MAX_DIGITS) return;
