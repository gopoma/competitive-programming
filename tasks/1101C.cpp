...
void solve() {
    ...
    map<int, int> forwards;
    {
        ...
        for(int i = 0; i < n; i++) {
            sgmts[i] = mp(mp(forwards[sgmts[i].f.f], forwards[sgmts[i].f.s]), sgmts[i].s);
        }
    }
    ...
    LazySeg st1, st2;
    {
        int mx = sz(forwards) + 5;
        int m = 1;
        while(m < mx) m *= 2;

        vector<LazySeg::T> arr(m, LazySeg::T(0));
        st1.init(arr); st2.init(arr);
    }

    vi group1, group2;

    group1.eb(sgmts.ft.s);
    st1.upd(sgmts.ft.f.f, sgmts.ft.f.s, 1);

    for(int i = 1; i < n; i++) {
        auto [l, r] = sgmts[i].f;
        int idx = sgmts[i].s;

        ll sum1 = st1.query(l, r).sum;
        ll sum2 = st2.query(l, r).sum;
    ...
    if(group1.empty() || group2.empty()) {
        ...
        vi ans(n);
        each(x, group1) ans[x] = 1;
        each(x, group2) ans[x] = 2;
