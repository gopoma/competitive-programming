...
void solve() {
    ...
    auto get = [&](int vari, int N) -> mi {
        return F.C(vari - 1 + N, vari - 1);
    };

    {
        mi contrib = get(a + 1, o) * get(b + 1, g);
        dbg(int(contrib));
    ...
    for(int x = 1; x <= b; x++) {
        int queda = b - x;
        mi left = get(a + x + 1, o - 1);
        mi right = get(queda + 1, g);
        mi contrib = left * right;
        // dbg(x, queda, int(left), int(right), int(contrib));
        re += contrib;
    }
