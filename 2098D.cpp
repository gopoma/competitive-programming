...
void solve() {
    ...
    auto dist = [&](pl A, pl B) -> ll {
        auto [x, y] = A;
        auto [x2, y2] = B;
        return custom_abs(x2 - x) + custom_abs(y2 - y);
    };

    const int N = sz(G);

    vector hist(n, vector(m, vector<ll>()));
    vl cnt(N);
    for(int i = 0; i + 1 < N; i++) {
        auto [x, y] = G[i];
        //* auto [x2, y2] = G[i + 1];
        ...
        for(int xd = 0; xd < 4; xd++) {
            ...
            if(check(nxt_x, nxt_y) && dist(G[i + 1], mp(nxt_x, nxt_y)) == 1 && !U_G.count(mp(nxt_x, nxt_y))) {
                nxt.eb(nxt_x, nxt_y);
            }
        }
        if(nxt.empty()) {
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            if(hist[i][j].empty()) continue;
            dbg(i + 1, j + 1, hist[i][j]);

            vl tmp;
            for(auto& id: hist[i][j]) {
                tmp.eb(id);
            }
    ...
    for(int i = N - 2; i >= 0; i--) {
        ...
        ll g = 0;
        for(auto& x: adj[i]) g += (x > i);

        dp[i] += cnt[i] * dp[i + 1];
        for(auto& x: adj[i]) if(x > i) dp[i] -= dp[x];
