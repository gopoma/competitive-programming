...
void solve() {
    ...
    const int OFFSET = 5;
    vector<vb> vis(n + OFFSET, vb(n + OFFSET, false));
    vector<vmi> dp(n + OFFSET, vmi(n + OFFSET));
    function<mi(int, int)> solve = [&](int x, int y) {
        if(x == 0 && y == 0) {
            if(grid[x][y] != '*') return mi(1);
            else return mi(0);
        }
        else if(x < 0 || y < 0)    return mi(0);
        else if(grid[x][y] == '*') return mi(0);

        if(vis[x][y]) return dp[x][y];
        vis[x][y] = true;

        mi res = solve(x, y - 1) + solve(x - 1, y);
