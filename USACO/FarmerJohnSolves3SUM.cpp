...
void solve() {
    ...
    vector<tuple<int, int, int>> gozu;
    for(int i = 0; i < N; i++) {
        hash_map<ll, vi> go({},{},{},{}, {1 << 13});
        ...
        for(int j = 0; j < N; j++) {
            ...
            if(go.find(tar) != go.end()) {
                for(auto& x: go[tar]) {
                    if(x != i && x != j && i != j) {
                        vi xd{i, j, x};
                        sor(xd);

                        gozu.eb(make_tuple(xd[0], xd[1], xd[2]));
                    }
                }
            }
        }
    ...
    remDup(gozu); each(x, gozu) dbg(x);

    vector<vi> pref(N + 1, vi(N + 1, 0));
    each(x, gozu) {
        pref[get<0>(x) + 1][get<2>(x) + 1]++;
    }
