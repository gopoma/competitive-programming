...
void solve() {
    ...
    vb vis(n + 1, false); vis[from] = true;
    deque<int> q; q.emplace_back(from);
    vi dist(n + 1, -1); dist[from] = 0;
    ...
    if(dist[to] == -1) ps("IMPOSSIBLE");
        ...
        vi route;
        while(dist[current_node] != 0) {
            route.eb(current_node);

            for(auto& v: adj[current_node]) {
                if(dist[v] + 1 == dist[current_node]) {
                    current_node = v;
                    break;
                }
            }
