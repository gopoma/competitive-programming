...
ll brute(vl __cnt) {
    ...
    map<vl, bool> vis;
    map<vl, ll> memo;
    auto dp = [&](const auto& dp, vl count) -> ll {
        if(vis[count]) return memo[count];
        vis[count] = true;
...
ll slv(vl cnt) {
    ...
    vector<vector<double>> subjects(3, vector<double>(sz(combs)));
    for(int i = 0; i < sz(combs); i++) {
        for(int j = 0; j < 3; j++) {
            subjects[j][i] = double(combs[i][j]);
        }
    }
    vector<double> B;
    for(int i = 0; i < 3; i++) B.eb(double(cnt[i]));

    vector<double> A(sz(combs), double(1));

    auto response = Simplex::simplex(subjects, B, A);
    return ll(floorl(response.f));
}
...
//? https://codeforces.com/contest/375/submission/250545852
...
signed main() {
    ...
    while(0) {
        RAYA;
        vl cnt(3); each(x, cnt) x = rng_ll(0, 100);
        dbg(cnt);
        dbg("Brute");
        ll ans = brute(cnt);
        dbg("/Brute");
