...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        while(left + 1 < right) {
            ...
            if(a[middle].f - a[fixedLeft].f <= k) {
                left = middle;
            } else right = middle;
        }

        update_chmax(i + 1, left + 1, left - i + 1);
    }

    vl ans(n);
    for(int i = 0; i < n; i++) {
        ans[a[i].s] = query_sum(i + 1, i + 1);
    }
...
//? https://codeforces.com/group/1yEJWoOmIY/contest/492920/problem/F
signed main() {
    setIO();
