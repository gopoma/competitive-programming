...
tcT> struct SegTree { // cmb(ID,b) = b
	const T ID{LLONG_MIN}; T cmb(T a, T b) { return max(a, b); }
	int n; V<T> seg;
...
void solve() {
    ...
    vi ans(m);
    for(int i = 0; i < m; i++) {
        ll global_max = st.query(0, n - 1);
        if(r[i] > global_max) ans[i] = 0;
        else {
            assert(r[i] <= global_max);
            int fixedLeft = 0;
            ...
            while(left + 1 < right) {
                ...
                if(st.query(fixedLeft, middle) >= r[i]) {
                    right = middle;
                } else {
                    left = middle;
                }
            ...
            int idx = right;
            st.upd(idx, st.query(idx, idx) - r[i]);
