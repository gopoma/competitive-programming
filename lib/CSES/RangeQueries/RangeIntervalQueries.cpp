...
void init(const V<int>& _seg) {
    for (n = 1; n < sz(_seg); ) n *= 2;

    seg.rsz(2 * n);

    F0R(i,n) {
        if(i < sz(_seg)) {
            seg[n + i].emplace_back(_seg[i]);
        }
    }
	ROF(i,1,n) pull(i);
}
...
void work(Response& response, vector<int>& block) {
    int cnt = upper_bound(all(block), response.d) - lower_bound(all(block), response.c);
    response.re += cnt;
}

Response query(int l, int r, int c, int d) {	// zero-indexed, inclusive
    Response response;
    response.c = c;
    response.d = d;
