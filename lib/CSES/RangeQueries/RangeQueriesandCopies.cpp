...
class PersistentSegtree {
    ...
    void _extend(int id) {
        while(id >= sz(tree)) tree.push_back(Node{});
    }
    ...
    public:
	PersistentSegtree(int n) : n(n), tree(5) {}
...
void solve() {
    ...
    rep(q) {
        ...
        if(type == 1) {
            ll k, pos, x; cin >> k >> pos >> x;
            dbg(type, k, pos, x);
            k--;
            pos--;

            roots[k] = st.set(roots[k], pos, x);
        } else if(type == 2) {
            ll k, l, r; cin >> k >> l >> r;
            dbg(type, k, l, r);
            k--;
            ...
            ll re = st.range_sum(roots[k], l, r);
            cout << re << "\n";
        } else {
            ll k; cin >> k;
            k--;
