...
void solve() {
    ...
    //? each(x, queries) dbg(x.l, x.r, x.idx);
    ...
    //? Coordinate Compression
    {
        ...
        each(x, arr) x = forwards[x];
    }
    assert(mx != -79);



    int block_size = 2000; //? TODO: fino
    ...
    vector<int> hist(mx + 5);

    int cnt = 0;

    auto remove = [&](int idx) -> void {
        if(hist[arr[idx]] == 1) cnt--;

        hist[arr[idx]]--;
	};
	auto add = [&](int idx) -> void {
        if(hist[arr[idx]] == 0) cnt++;
