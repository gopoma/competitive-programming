...
void solve() {
	...
	vi ans(n);
	for(int i = 0; i < n; i++) {
		//? int tot = st.query(0, n - 1);
		//? assert(p[i] <= tot);

		const int fixedLeft = 0;
		...
		while(left + 1 < right) {
			...
			if(st.query(fixedLeft, middle) >= p[i]) { //* shortcut? YES
				right = middle;
			} else {
				left = middle;
			}
		...
		int idx_at_least = right;
		st.upd(idx_at_least, st.query(idx_at_least, idx_at_least) - 1);

		ans[i] = x[idx_at_least];
	}
