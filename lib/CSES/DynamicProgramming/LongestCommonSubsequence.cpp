...
void solve() {
    ...
    auto dp = [&](auto&& self, int i, int j) -> int {
        ...
        if(a[i] == b[j]) {
            // take
            ckmax(re, self(self, i + 1, j + 1) + 1);
        } else {
            // skip
            ckmax(re, self(self, i + 1, j));
            ckmax(re, self(self, i, j + 1));
        }
    ...
    auto dfs = [&](auto&& self, int i, int j) -> void {
        ...
        if(a[i] == b[j]) {
            ...
            tuple<int, int, int> nxt = {-INF, -INF, -INF};

            ckmax(nxt, {dp(dp, i + 1, j), i + 1, j});
            ckmax(nxt, {dp(dp, i, j + 1), i, j + 1});

            auto [_, nxt_i, nxt_j] = nxt;

            self(self, nxt_i, nxt_j);
        }
    }; dfs(dfs, 0, 0);
