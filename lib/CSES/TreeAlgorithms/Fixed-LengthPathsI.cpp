...
void work(int centroid) {
    {
        vector<int> block;
        auto dfs = [&](auto&& dfs, int src, int par, int cur_depth) -> void {
            depth[src] = cur_depth;

            response += ll(depth[src] == k);
            block.emplace_back(src);

            for(auto& nxt: adj[src]) {
                if(nxt == par || is_removed[nxt]) continue;
                dfs(dfs, nxt, src, cur_depth + 1);
            }
        };

        depth[centroid] = 0;
        response += ll(depth[centroid] == k);
        ...
        for(auto& nxt: adj[centroid]) {
            ...
            block.clear();
            dfs(dfs, nxt, centroid, 1);
            for(auto& x: block) history.emplace_back(x);

            // calc
            for(auto& x: block) {
                if(depth[x] > k) continue;

                const int tar = k - depth[x];
                response += cnt[tar];
            }
