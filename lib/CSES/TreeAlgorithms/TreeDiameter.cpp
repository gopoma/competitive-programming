...
void solve() {
    ...
    auto get_dist = [&](int node) -> vector<int> {
        vector<int> dist(n + 1);
    ...
    int A = get_furthest(1);
    int B = get_furthest(A);

    vector<int> dist_from_A = get_dist(A);
    vector<int> dist_from_B = get_dist(B);

    vector<int> response(n + 1);
    int re = 0;
    for(int u = 1; u <= n; u++) {
        response[u] = max(dist_from_A[u], dist_from_B[u]);
        re = max(re, response[u]);
    }
