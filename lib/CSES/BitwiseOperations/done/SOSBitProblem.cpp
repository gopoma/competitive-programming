...
int dp1[1 << MXBIT];
int prev_dp1[1 << MXBIT];

int dp2[1 << MXBIT];
int prev_dp2[1 << MXBIT];
...
void solve() {
    ...
    vector<int> hist(1 << MXBIT);
    for(auto& x: a) hist[x]++;

    {
        for(int i = MXBIT; i >= 0; i--) {
            for(int mask = 0; mask < (1 << MXBIT); mask++) {
                if(i == MXBIT) {
                    dp1[mask] = hist[mask];
                } else {
                    dp1[mask] = 0;

                    dp1[mask] += prev_dp1[mask];
                    if(mask & (1 << i)) dp1[mask] += prev_dp1[mask ^ (1 << i)];
                }
            }
            for(int mask = 0; mask < (1 << MXBIT); mask++) {
                prev_dp1[mask] = dp1[mask];
            }
        }
    }
        ...
        for(int i = MXBIT; i >= 0; i--) {
            for(int mask = 0; mask < (1 << MXBIT); mask++) {
                if(i == MXBIT) {
                    ...
                    dp2[mask] += prev_dp2[mask];
                    if(mask & (1 << i)) {

                    } else {
                        dp2[mask] += prev_dp2[mask ^ (1 << i)];
                    }
                }
            }
            for(int mask = 0; mask < (1 << MXBIT); mask++) {
                prev_dp2[mask] = dp2[mask];
            }
        }
    }
