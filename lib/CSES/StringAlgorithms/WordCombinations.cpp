...
void solve() {
    ...
    for(int idx = n; idx >= 0; idx--) {
        if(idx == n) dp[idx] = 1;
            ...
            //? for(int R = idx; R < n; R++) {
                //? HashRange current;
                //? current.add(S.substr(L, R - L + 1));
                //? if(hsh_words.count(current.hash(0, R - L))) dp[idx] += dp[R + 1];
            //? }
        }
    }
    mi ans = dp[0];
...
//?        HashRange current;
//?        current.add(x);
//?        hsh_words.emplace(current.hash(0, sz(x) - 1));
//?    }

//?    vb vis(n + 5);
//?    vmi memo(n + 5);
//?    function<mi(int)> dp = [&](int idx) -> mi {
...
//?            HashRange current;
//?            current.add(S.substr(L, R - L + 1));
//?            if(hsh_words.count(current.hash(0, R - L))) ans += dp(R + 1);
//?        }
//?
//?        return memo[idx] = ans;
//?    };
