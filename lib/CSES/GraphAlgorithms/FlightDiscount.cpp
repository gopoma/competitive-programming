...
void solve() {
    ...
    Dijkstra<ll, true> forwards; forwards.init(n);
    Dijkstra<ll, true> backwards; backwards.init(n);
    ...
    rep(m) {
        ...
        forwards.ae(int(a), int(b), c);
        backwards.ae(int(b), int(a), c);
    }
    ...
    each(e, edges) {
        auto [u, v, w] = e;
        ll L = forwards.dist[u];
        ll R = backwards.dist[v];

        if(L >= BIG || R >= BIG) continue;

        ckmin(ans, L + fdiv(w, 2LL) + R);
    }
    assert(ans != BIG);
