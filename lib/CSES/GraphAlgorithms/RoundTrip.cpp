...
void solve() {
    ...
    const int BEING_PROCESSED = 1;
    const int PROCESSED       = 2;
    vi color(n + 1, NOT_PROCESSED);
    vb vis(n + 1, false);
    vi go;
    bool found = false;
    vi res;
    function<void(int, int)> dfs = [&](int u, int parent) {
        ...
        color[u] = BEING_PROCESSED;
        go.eb(u);

        for(auto& v: adj[u]) {
            if(color[v] == NOT_PROCESSED) {
                dfs(v, u);
            } else if(color[v] == BEING_PROCESSED) {
            ...
            } else if(color[v] == PROCESSED) assert(false);
            else assert(false);
        }
    ...
    for(int u = 1; u <= n; u++) {
        if(!found && color[u] == NOT_PROCESSED) {
            dfs(u, -1);
        }
    }
