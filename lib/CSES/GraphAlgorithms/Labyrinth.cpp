...
void solve() {
    ...
    auto convert = [](int dir_x, int dir_y) {
        // dx[4]{1, 0, -1, 0}, dy[4]{0, 1, 0, -1};
        if(dir_x == 1 && dir_y == 0) return 'D';
        else if(dir_x == 0 && dir_y == 1) return 'R';
        else if(dir_x == -1 && dir_y == 0) return 'U';
        else if(dir_x == 0 && dir_y == -1) return 'L';
        else assert(false);
    };

    vs labyrinth(n); re(labyrinth); for(auto& x: labyrinth) dbg(x);
    ...
    auto work = [&](int i, int j) {
        ...
        while(!q.empty() && !local_ok) {
            pi current = q.front(); q.pop();
            int current_x = current.f;
            int current_y = current.s;

            for(int k = 0; k < 4; k++) {
                int new_x = current_x + dx[k];
                int new_y = current_y + dy[k];

                if(ok(new_x, new_y) && !vis[new_x][new_y] && (labyrinth[new_x][new_y] == '.' || labyrinth[new_x][new_y] == 'B')) {
                    vis[new_x][new_y] = true;
                    q.push(mp(new_x, new_y));

                    dists[new_x][new_y] = dists[current_x][current_y] + 1;
    ...
    if(!global_ok) ps("NO");
        ...
        ps("YES");
        for(auto& x: dists) dbg(x);
        dbg(B_location, dists[B_location.f][B_location.s]);

        int current_dist = dists[B_location.f][B_location.s];
        pi current = B_location;
        str res = "";
        while(current_dist != 0) {
            for(int k = 0; k < 4; k++) {
                int new_x = current.f + dx[k];
                int new_y = current.s + dy[k];

                if(ok(new_x, new_y) && dists[new_x][new_y] != -1 && dists[new_x][new_y] + 1 == current_dist) {
                    res.pb(convert(-dx[k], -dy[k]));
