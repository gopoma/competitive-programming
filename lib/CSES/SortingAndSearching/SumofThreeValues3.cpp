...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        int tar = x - a[i];
        __gnu_pbds::gp_hash_table<ll, __gnu_pbds::null_type, chash> go({},{},{},{}, {1 << 16});

        for(int j = i + 1; j < n; j++) {
            if(i == j) continue;

            if(go.find(tar - a[j]) != go.end()) {
                ok = true;
                val = make_tuple(a[i], a[j], tar - a[j]);
            }
            else go.ins(a[j]);
        }
    }

    __gnu_pbds::gp_hash_table<int, __gnu_pbds::null_type, chash> banned({},{},{},{}, {1 << 10});
    auto work = [&](int tar) {
        for(int i = 0; i < n; i++)
            if(a[i] == tar && banned.find(i) == banned.end())
                return i;
    ...
    if(!ok) ps("IMPOSSIBLE");
        ...
        vi idx(3);
        idx[0] = work(get<0>(val)); banned.ins(idx[0]);
        idx[1] = work(get<1>(val)); banned.ins(idx[1]);
        idx[2] = work(get<2>(val));
        sor(idx);

        auto ans = make_tuple(idx[0] + 1, idx[1] + 1, idx[2] + 1);

        ps(get<0>(ans), get<1>(ans), get<2>(ans));
    }
}
