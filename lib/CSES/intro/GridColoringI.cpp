...
void solve() {
    ...
    set<char> cur;
    vector response(n, vector<char>(m));
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            for(char c = 'A'; c <= 'D'; c++) cur.emplace(c);

            cur.erase(S[i][j]);
            if(check_bounds(i - 1, j)) cur.erase(response[i - 1][j]);
            if(check_bounds(i, j - 1)) cur.erase(response[i][j - 1]);

            response[i][j] = *cur.begin();
        }
    }
