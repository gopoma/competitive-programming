...
void solve() {
    ...
    delta.emplace_back(-1, 0); // UP
    delta.emplace_back(0, -1); // LEFT

    vector<bool> already(2 * n + 5);
    vector response(n, vector<int>(n));
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < n; j++) {
            vector<int> current;
            ...
            for(auto& [dx, dy]: delta) {
                ...
                while(true) {
                    x += dx;
                    y += dy;
                    if(!check_bounds(x, y)) break;
                    current.emplace_back(response[x][y]);
                }
            }

            for(int k = 0; k < sz(already); k++) {
                already[k] = false;
            }
