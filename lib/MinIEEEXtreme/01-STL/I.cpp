...
void solve() {
    ...
    unsigned long long upper_bound = (1ULL << 32ULL) - 1ULL;
    // DBG(upper_bound * upper_bound);
    unsigned long long ans = 0ULL;
    bool ok = true;
    bool overflow = false;
    unsigned long long cumulative = 1ULL;
    unsigned long long count = 0ULL;
    stack<unsigned long long> SS;

    auto check = [&](unsigned long long x) {
        return (x <= upper_bound);
    };

    auto add = [&](unsigned long long x) {
        if(check(ans + x)) {
            ans = ans + x;
        } else {
            ok = false;
    ...
    for(int _ = 0; _ < l; _++) {
        ...
        if(token == "add") {
            ...
            // if cumulative has overflow, then answer is overflow
            if(overflow) {
                ok = false;
            } else {
                add(cumulative);
            ...
            assert(!overflow && !SS.empty());
            unsigned long long temp = SS.top();
            SS.pop();
