...
str norm(str S) {
    reverse(all(S));
    while(!S.empty() && S.bk == '0') S.pop_back();
    reverse(all(S));
    return S;
}

str compare(str A, str B) {
    A = norm(A);
    B = norm(B);
...
void slv(int n, int k, vs S) {
    vi lost(n);
    for(int i = 0; i < n; i++) {
        str current = S[i];
        reverse(all(current));

        while(!current.empty() && current.bk == '0') {
            lost[i]++;
            current.pop_back();
        }
        reverse(all(current));
