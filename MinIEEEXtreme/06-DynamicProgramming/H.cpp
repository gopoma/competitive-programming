...
void solve() {
    ...
    V<long long> values(n);
    V<long long> weights(n);
    ...
    // solve(x): maximum value reachable from x weight remaining
    function<long long(long long)> solve = [&](long long x) {
        ...
        long long ans = -INF;
        for(int i = 0; i < n; i++) {
            ans = max(ans, solve(x - weights[i]) + (x - weights[i] >= 0LL? values[i] : 0LL));
        }
