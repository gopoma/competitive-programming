...
map<pair<int, int>, set<pair<int, int>>> costs;
ll ans;
...
void printArr(int arr[], int n) {
    ...
    for (int i = 1; i < n; ++i) {
        if(arr[i] != -1) {
            pair<int, int> indexed_pair = mp(min(arr[i], i), max(arr[i], i));
            ll minimum = ll((*costs[indexed_pair].begin()).S);
...
void solve() {
    ...
    for(int i = 0; i < M; i++) {
        ...
        addEdge(graph, v, u, cost);

        costs[mp(u, v)].ins(mp(L, C));
    }
