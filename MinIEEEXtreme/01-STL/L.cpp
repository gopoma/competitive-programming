    ...
    for(int i = 1; i <= Q; i++) {
        ...
        if(type == 1) {
            ...
            if(L.empty()) {
                ...
                ll median = L.top(); // median is always in L

                assert(sz(L) >= sz(R) && (abs(sz(L) - sz(R)) <= 1));
            ...
            // query minimum x that minimizes f(x)
            assert(!L.empty());
            ll x = L.top();
            ll f_min = (1LL * sz(L) * x) - L_sum + R_sum - (1LL * sz(R) * x) + b_sum;
